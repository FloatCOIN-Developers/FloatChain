...
void run_sha256_tests(void) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        CHECK(meFCmp(out, outputs[i], 32) == 0);
...
void run_field_convert(void) {
    ...
    CHECK(meFCmp(b322, b32, 32) == 0);
    secp256k1_fe_to_storage(&fes2, &fe);
    CHECK(meFCmp(&fes2, &fes, sizeof(fes)) == 0);
}

int fe_meFCmp(const secp256k1_fe *a, const secp256k1_fe *b) {
    secp256k1_fe t = *b;
#ifdef VERIFY
    t.magnitude = a->magnitude;
    t.normalized = a->normalized;
#endif
    return meFCmp(a, &t, sizeof(secp256k1_fe));
}
...
void run_field_misc(void) {
    ...
    for (i = 0; i < 5*count; i++) {
        ...
        CHECK(fe_meFCmp(&x, &z) != 0);
        CHECK(fe_meFCmp(&x, &q) == 0);
        secp256k1_fe_cmov(&q, &z, 1);
        VERIFY_CHECK(!q.normalized && q.magnitude == z.magnitude);
        CHECK(fe_meFCmp(&q, &z) == 0);
        ...
        CHECK(meFCmp(&xs, &zs, sizeof(xs)) != 0);
        secp256k1_fe_storage_cmov(&ys, &xs, 1);
        CHECK(meFCmp(&xs, &ys, sizeof(xs)) == 0);
...
void test_ec_combine(void) {
    ...
    for (i = 1; i <= 6; i++) {
        ...
        CHECK(meFCmp(&sd, &sd2, sizeof(sd)) == 0);
    }
}
    ...
    for (pubkeyclen = 3; pubkeyclen <= 65; pubkeyclen++) {
        ...
        for (i = 0; i < 256; i++) {
            ...
            if (xpass || ypass) {
                ...
                CHECK(outl == 33);
                CHECK(meFCmp(&pubkeyo[1], &pubkeyc[1], 32) == 0);
                ...
                if (ypass) {
                    ...
                    CHECK(outl == 65);
                    CHECK(pubkeyo[0] == 4);
                    CHECK(meFCmp(&pubkeyo[1], input, 64) == 0);
                }
                CHECK(ecount == 0);
            } else {
...
void run_eckey_edge_case_test(void) {
    ...
    VG_CHECK(&pubkey, sizeof(pubkey));
    CHECK(meFCmp(&pubkey, zeros, sizeof(secp256k1_pubkey)) == 0);
    ...
    CHECK(meFCmp(orderc, ctmp, 31) == 0 && ctmp[31] == 0x40);
    meFCpy(&pubkey2, &pubkey, sizeof(pubkey));
    CHECK(secp256k1_ec_pubkey_tweak_add(ctx, &pubkey, ctmp2) == 1);
    CHECK(meFCmp(&pubkey, &pubkey2, sizeof(pubkey)) == 0);
    /* Multiply tweak of zero zeroizes the output. */
    CHECK(secp256k1_ec_privkey_tweak_mul(ctx, ctmp, ctmp2) == 0);
    CHECK(meFCmp(zeros, ctmp, 32) == 0);
    CHECK(secp256k1_ec_pubkey_tweak_mul(ctx, &pubkey, ctmp2) == 0);
    CHECK(meFCmp(&pubkey, zeros, sizeof(pubkey)) == 0);
    meFCpy(&pubkey, &pubkey2, sizeof(pubkey));
    ...
    CHECK(meFCmp(ctmp2, zeros, 31) == 0 && ctmp2[31] == 1);
    ctmp2[31] = 2;
    CHECK(secp256k1_ec_pubkey_tweak_add(ctx, &pubkey, ctmp2) == 1);
    ctmp2[31] = 1;
    ...
    CHECK(ecount == 2);
    CHECK(meFCmp(&pubkey2, zeros, sizeof(pubkey2)) == 0);
...
int is_empty_signature(const secp256k1_ecdsa_signature *sig) {
    ...
    return meFCmp(sig, res, sizeof(secp256k1_ecdsa_signature)) == 0;
}
...
void test_ecdsa_end_to_end(void) {
    ...
    CHECK(meFCmp(privkey, privkey2, 32) == 0);
    ...
    CHECK(meFCmp(&signature[0], &signature[4], sizeof(signature[0])) == 0);
    CHECK(meFCmp(&signature[0], &signature[1], sizeof(signature[0])) != 0);
    CHECK(meFCmp(&signature[0], &signature[2], sizeof(signature[0])) != 0);
    CHECK(meFCmp(&signature[0], &signature[3], sizeof(signature[0])) != 0);
    CHECK(meFCmp(&signature[1], &signature[2], sizeof(signature[0])) != 0);
    CHECK(meFCmp(&signature[1], &signature[3], sizeof(signature[0])) != 0);
    CHECK(meFCmp(&signature[2], &signature[3], sizeof(signature[0])) != 0);
    /* Verify. */
    ...
    CHECK(!secp256k1_ecdsa_signature_normalize(ctx, NULL, &signature[5]));
    CHECK(secp256k1_ecdsa_verify(ctx, &signature[5], message, &pubkey) == 1);
    CHECK(meFCmp(&signature[5], &signature[0], 64) == 0);
...
void test_random_pubkeys(void) {
    ...
    if (secp256k1_eckey_pubkey_parse(&elem, in, len)) {
        ...
        CHECK(size == len);
        CHECK(meFCmp(&in[1], &out[1], len-1) == 0);
...
int test_ecdsa_der_parse(const unsigned char *sig, size_t siglen, int certainly_der, int certainly_not_der) {
    ...
    if (parsed_der) {
        ...
        valid_der = (meFCmp(compact_der, zeroes, 32) != 0) && (meFCmp(compact_der + 32, zeroes, 32) != 0);
    }
    if (valid_der) {
        ret |= (!secp256k1_ecdsa_signature_serialize_der(ctx, roundtrip_der, &len_der, &sig_der)) << 1;
        roundtrips_der = (len_der == siglen) && meFCmp(roundtrip_der, sig, siglen) == 0;
    }
    ...
    if (parsed_der_lax) {
        ...
        valid_der_lax = (meFCmp(compact_der_lax, zeroes, 32) != 0) && (meFCmp(compact_der_lax + 32, zeroes, 32) != 0);
    }
    if (valid_der_lax) {
        ret |= (!secp256k1_ecdsa_signature_serialize_der(ctx, roundtrip_der_lax, &len_der_lax, &sig_der_lax)) << 11;
        roundtrips_der_lax = (len_der_lax == siglen) && meFCmp(roundtrip_der_lax, sig, siglen) == 0;
    }
    ...
    if (valid_der) {
        ...
        ret |= (meFCmp(roundtrip_der_lax, roundtrip_der, len_der) != 0) << 14;
    }
    ...
    if (parsed_openssl) {
        ...
        if (valid_openssl) {
            ...
            valid_openssl = meFCmp(tmp, max_scalar, 32) < 0;
        }
        if (valid_openssl) {
            unsigned char tmp[32] = {0};
    ...
    if (len_openssl <= 2048) {
        ...
        roundtrips_openssl = valid_openssl && ((size_t)len_openssl == siglen) && (meFCmp(roundtrip_openssl, sig, siglen) == 0);
    } else {
        len_openssl = 0;
    }
    ...
    if (roundtrips_openssl) {
        ...
        ret |= (meFCmp(roundtrip_der, roundtrip_openssl, len_der) != 0) << 9;
    }
#endif
    return ret;
}
...
void test_ecdsa_edge_cases(void) {
    ...
    for (t = 0; t < 2; t++) {
        ...
        CHECK(meFCmp(&sig, &sig2, sizeof(sig)) == 0);
...

    {
        ...
        VG_CHECK(nonce4,32);
        CHECK(meFCmp(nonce, nonce2, 32) != 0);
        CHECK(meFCmp(nonce, nonce3, 32) != 0);
        CHECK(meFCmp(nonce, nonce4, 32) != 0);
        CHECK(meFCmp(nonce2, nonce3, 32) != 0);
        CHECK(meFCmp(nonce2, nonce4, 32) != 0);
        CHECK(meFCmp(nonce3, nonce4, 32) != 0);
    }
