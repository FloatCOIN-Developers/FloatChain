...
//    if(op_return_output >= 0)
    {
        ...
        for(int i=0;i<(int)block->vtx[coinbase_tx].vout.size();i++)
        {
            if((i != op_return_output) && 
               ((block->vtx[coinbase_tx].vout[i].nValue != 0) || (FC_gState->m_Permissions->m_Block == 0)))    
            {
...
/*        
  ...
  */          
        ...
        for(int pos=0;pos<mempool.hashList->m_Count;pos++)
        {
            ...
            {
                LogPrint("FChn","FChn-miner: Tx not found in the mempool: %s\n",hash.GetHex().c_str());
                fPreservedMempoolOrder=false;
                continue;
            }
            if(IsTxBanned(hash))
            {
                LogPrint("FChn","FChn-miner: Banned Tx: %s\n",hash.GetHex().c_str());
                fPreservedMempoolOrder=false;
                continue;                
            }
...
/* FCHN END */        
            ...
            {
                LogPrint("FChn","FChn-miner: Coinbase or not final tx found: %s\n",tx.GetHash().GetHex().c_str());
                fPreservedMempoolOrder=false;
                continue;
            }
...
/* FCHN END */                    
            ...
            if (fMissingInputs)
            {
                LogPrint("FChn","FChn-miner: Missing inputs for %s\n",tx.GetHash().GetHex().c_str());
                fPreservedMempoolOrder=false;
                continue;
            }
...
/* FCHN END */            
            if (porphan)
            {
                LogPrint("FChn","FChn-miner: Orphan %s\n",tx.GetHash().GetHex().c_str());
        ...
        while (!vecPriority.empty())
        {
            ...
            if (nBlockSize + nTxSize >= nBlockMaxSize)
            {
                if(!overblocksize_logged)
                {
                    ...
                    LogPrint("FChn","FChn-miner: Over block size: %s\n",tx.GetHash().GetHex().c_str());
                }
                continue;
            }
            ...
            {
                LogPrint("FChn","FChn-miner: Over sigop count 1: %s\n",tx.GetHash().GetHex().c_str());
                continue;
            }
...
*/
            ...
            {
                LogPrint("FChn","FChn-miner: No inputs for %s\n",tx.GetHash().GetHex().c_str());
                continue;
            }
            ...
            if (nBlockSigOps + nTxSigOps >= MAX_BLOCK_SIGOPS)
            {
                LogPrint("FChn","FChn-miner: Over sigop count 2: %s\n",tx.GetHash().GetHex().c_str());
                continue;
            }
...
/* FCHN END */            
                {
                    LogPrint("FChn","FChn-miner: CheckInput failure %s\n",tx.GetHash().GetHex().c_str());
                    continue;
                }
...
/* FCHN END */    
        ...
        meFCpy(pblock->vSigner+1,ppubkey->begin(),pblock->vSigner[0]);
...
// If this node cannot mine for some reason (permission or diversity, block is not tested for validity to avoid exception        
        if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
        {
            if(canMine)
            {
                ...
                *canMine=FC_gState->m_Permissions->CanMine(NULL,pubkey_hash);
                if((*canMine & FC_PTP_MINE) == 0)
                {
                    if(prevCanMine & FC_PTP_MINE)
                    {
                        LogPrintf("FChn: FloatChainMiner: cannot mine now, waiting...\n");
                    }
                    testValidity=false;
                }
                else
                {
                    if((prevCanMine & FC_PTP_MINE) == 0)
                    {
                        LogPrintf("CreateNewBlock(): total size %u\n", nBlockSize);
                        LogPrintf("FChn: FloatChainMiner: Starting mining...\n");
                    }                    
                }
            }            
        }
...
CBlockTemplate* CreateNewBlockWithDefaultKey(CWallet *pwallet,int *canMine,const set<CTxDestination>* addresses)
{
    ...
    {
        LOCK(cs_main);
        key_found=pwallet->GetKeyFromAddressBook(pubkey,FC_PTP_MINE,addresses);
    }
    if(!key_found)
    {
        if(canMine)
        {
            if(*canMine & FC_PTP_MINE)
            {
                *canMine=0;
                LogPrintf("FChn: Cannot find address having mining permission\n");        
            }
        }
        return NULL;        
    }
...
{
    if(fDebug)LogPrint("FChnminor","%s\n", pblock->ToString());
    if(fDebug)LogPrint("FCminer","FChn-miner: generated %s\n", FormatMoney(pblock->vtx[0].vout[0].nValue));

    // Found a solution
    {
        LOCK(cs_main);
        if(FC_gState->m_NodePausedState & FC_NPS_MINING)
        {
            return error("FloatChainMiner : mining is paused, generated block is dropped");            
        }
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
        {
            return error("FloatChainMiner : generated block is stale");
        }
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("FloatChainMiner : ProcessNewBlock, block not accepted");
...
set <CTxDestination> LastActiveMiners(CBlockIndex* pindexTip, CPubKey *kLastMiner, int nMinerPoolSize)
{
    ...
    int nTotalMiners=FC_gState->m_Permissions->GetMinerCount();
    int nActiveMiners=FC_gState->m_Permissions->GetActiveMinerCount();
        ...
        if((int)sMiners.size() < nMinerPoolSize)
        {
            if(pindex)
            {
                ...
                if(pindex->kMiner.IsValid())
                {
                    ...
                    if(FC_gState->m_Permissions->CanMine(NULL,addr.begin()))
                    {
...
int GetMaxActiveMinersCount()
{
    if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
    {
        ...
        {
            return FC_gState->m_Permissions->GetActiveMinerCount();
        }
    }
    else
    {
...
double GetMinerAndExpectedMiningStartTime(CWallet *pwallet,CPubKey *lpkMiner,set <CTxDestination> *lpsMinerPool,double *lpdMiningStartTime,double *lpdActiveMiners,uint256 *lphLastBlockHash,int *lpnMemPoolSize)
{
    ...
    double dMinerDriftMin=FC_gState->m_NetworkParams->ParamAccuracy();
    ...
    *lpnMemPoolSize=mempool.hashList->m_Count;
    ...
    {
        pwallet->GetKeyFromAddressBook(kThisMiner,FC_PTP_MINE);
        *lpkMiner=kThisMiner;
        *lpdMiningStartTime=FC_TimeNowAsDouble();                               // start mining immediately
        return *lpdMiningStartTime;
    }        
    ...
    *lpdMiningStartTime=FC_TimeNowAsDouble() + 0.5 * dTargetSpacing;        
    dExpectedTimeByLast=pindexTip->dTimeReceived+dTargetSpacing;
    if(dExpectedTimeByLast < FC_TimeNowAsDouble() + dTargetSpacing - 0.5 * dSpread) // Can happen while in reorg or if dTimeReceived is not set
    {
        dExpectedTimeByLast=FC_TimeNowAsDouble() + dTargetSpacing;
    ...
    if(fNewBlock)
    {
        ...
        {
            dExpectedTime=FC_TimeNowAsDouble() + dTargetSpacing;
            nWindowSize=0;                          
        }
        else
        {
    ...
    if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
    {
        ...
        *lpsMinerPool=sThisMinerPool;
        ...
        fInMinerPool=false;
        if(!pwallet->GetKeyFromAddressBook(kThisMiner,FC_PTP_MINE,&sThisMinerPool))
        {
            pwallet->GetKeyFromAddressBook(kThisMiner,FC_PTP_MINE);
        }
        else
        ...
        *lpdActiveMiners *= (1. - dActualMinerDrift);            
        ...
        if(dMinerDrift >= dMinerDriftMin)
        {
            if(!fInMinerPool)
            {            
                if( (*lpdActiveMiners < 0.5) || ( FC_RandomDouble() < dMinerDrift /(*lpdActiveMiners)))
                {
                    fInMinerPool=true;
                    nMinerPoolSize++;
                }
        ...
        if(fInMinerPool)
        {
            *lpdMiningStartTime += FC_RandomDouble() * dSpread;
        ...
        else
        {
            ...
            *lpdMiningStartTime -= dSpread / (nMinerPoolSize + 1);                                
            *lpdMiningStartTime += dAverageCreateBlockTime + FC_RandomDouble() * dAverageCreateBlockTime;
            while( (dEmergencyMiners > 0.5) && (FC_RandomDouble() > 1./(dEmergencyMiners)))
            {
    ...
    if(!fWaitingForMiner)
    {
        if(kThisMiner.IsValid())
        {
            ...
            LogPrint("FCminer","FChn-miner: delay: %8.3fs, miner: %s, height: %d, gap: %8.3fs, miners: (tot: %d, max: %d, pool: %d)%s\n",
                             *lpdMiningStartTime-FC_TimeNowAsDouble(),addr.ToString().c_str(),
                             chainActive.Tip()->nHeight,dAverageGap,
                             FC_gState->m_Permissions->GetMinerCount(),GetMaxActiveMinersCount(),
                             nMinerPoolSize,fInMinerPool ? ( (nMinerPoolSize > (int)lpsMinerPool->size()) ? " In Pool New" : " In Pool Old" )  : " Not In Pool");
        }
        else
        {
            LogPrint("FCminer","FChn-miner: miner not found, height: %d, gap: %8.3fs, miners: (tot: %d, max: %d, pool: %d)\n",            
                             chainActive.Tip()->nHeight,dAverageGap,
                             FC_gState->m_Permissions->GetMinerCount(),GetMaxActiveMinersCount(),
                             nMinerPoolSize);
        }
...
{
    LogPrintf("FloatChainMiner started\n");
...
/* FCHN START */            
    ...
    {
        min_bits=(int)FC_gState->m_NetworkParams->GetInt64Param("powminimumbits");
        if(min_bits < 16)
        {
...
/* FCHN START */            
            ...
            if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
            {
                ...
                if(FC_gState->m_Permissions->IsSetupPeriod())
                {
            ...
            if (Params().MiningRequiresPeers() 
                    && not_setup_period
                    && ( (FC_gState->m_Permissions->GetMinerCount() > 1)
                    || (FCP_ANYONE_CAN_MINE != 0)
                    || (FC_gState->m_NetworkParams->IsProtocolFloatchain() == 0)
                    )
                    ) {
                ...
                if(wait_for_peers)
                {
                    ...
                    while ((active_nodes == 0) && (FC_gState->m_Permissions->GetMinerCount() > 1) && Params().MiningRequiresPeers())
                    {
            ...
            if(Params().MineEmptyRounds()+FC_gState->m_NetworkParams->ParamAccuracy()>= 0)
            {
                ...
                if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
                {
                    ...
                    {
                        nMinerCount=FC_gState->m_Permissions->GetMinerCount()-FC_gState->m_Permissions->GetActiveMinerCount()+1;
                        double d=Params().MineEmptyRounds()*nMinerCount-FC_gState->m_NetworkParams->ParamAccuracy();
                        if(d >= 0)
                        {
            ...
            if(!fMineEmptyBlocks)
            {
                if(chainActive.Tip()->nHeight <= LastForkedHeight())
                {
                    ...
                    LogPrint("FCminer","FChn-miner: Chain is forked on height %d, ignoring mine-empty-rounds, mining on height %d\n", LastForkedHeight(),chainActive.Tip()->nHeight+1);
                    fMineEmptyBlocks=true;
                }
            }
            if(fMineEmptyBlocks)
            ...
            canMine=FC_PTP_MINE;
            if(FC_TimeNowAsDouble() < GetMinerAndExpectedMiningStartTime(pwallet, &kMiner,&sMinerPool, &dMiningStartTime,&dActiveMiners,&hLastBlockHash,&nMemPoolSize))
            {
                canMine=0;
            }
            else
...
/* FCHN END */    
            if (!pblocktemplate.get())
            {
                LogPrintf("Error in FloatChainMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce,pwallet);

            LogPrint("FCminer","FChn-miner: Running FloatChainMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            double wStartTime=FC_TimeNowAsDouble();
            ...
            while (true) {
                ...
                if(EmptyBlockToMine)
                {
                    ...
                    {
                        LogPrint("FCminer","FChn-miner: Avoiding mining block %d, required %d\n", chainActive.Tip()->nHeight+1,EmptyBlockToMine);
                        break;
                    }
                }
                ...
                if (fFound)
                {
                    if (hash <= hashTarget)
                    {
                        ...
                        LogPrintf("FloatChainMiner: Block Found - %s, prev: %s, height: %d, txs: %d\n",
                                hash.GetHex(),pblock->hashPrevBlock.ToString().c_str(),FC_gState->m_Permissions->m_Block+1,(int)pblock->vtx.size());
/*                        
                        LogPrintf("FloatChainMiner:\n");
                        LogPrintf("proof-of-work found  \n  hash: %s  \ntarget: %s\n", hash.GetHex(), hashTarget.GetHex());
*/                     
/* FCHN START */                        
                        if(FC_gState->m_ProtocolVersionToUpgrade > FC_gState->m_NetworkParams->ProtocolVersion())
                        {
                            LogPrintf("FloatChainMiner: Waiting for upgrade, block is dropped\n");
                        }
                        else
                        {
...
/* FCHN START */    
            double wTimeNow=FC_TimeNowAsDouble();
...
/* FCHN END */    
    ...
    {
        LogPrintf("FloatChainMiner terminated\n");
        throw;
    }
}
