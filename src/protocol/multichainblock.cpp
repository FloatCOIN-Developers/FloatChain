...
bool ReplayMemPool(CTxMemPool& pool, int from,bool accept)
{
    ...
    LogPrint("FChn", "FChn: Replaying memory pool (%d new transactions, total %d)\n",total_txs-from,total_txs);
    FC_gState->m_Permissions->MempoolPermissionsCopy();
            ...
            else
            {
                ...
                if(FC_gState->m_Features->Streams())
                {
                    ...
                    if(entry.FullReplayRequired())
                    {
                        ...
                        if(!AcceptFloatChainTransaction(tx,view,-1,accept,reason,NULL))
                        {
                    ...
                    {
                       if(FC_gState->m_Permissions->MempoolPermissionsCheck(entry.ReplayPermissionFrom(),entry.ReplayPermissionTo()) == 0) 
                       {
                            removed_type="rejected";                                               
                       }                        
                    }
    ...
    if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
    {
        for (unsigned int i = 0; i < block->vtx.size(); i++)
        {
            ...
            if (tx.IsCoinBase())
            {
                ...
                {
                    FC_gState->m_TmpScript1->Clear();

                    const CScript& script1 = tx.vout[j].scriptPubKey;        
                    CScript::const_iterator pc1 = script1.begin();

                    FC_gState->m_TmpScript1->SetScript((unsigned char*)(&pc1[0]),(size_t)(script1.end()-pc1),FC_SCR_TYPE_SCRIPTPUBKEY);

                    for (int e = 0; e < FC_gState->m_TmpScript1->GetNumElements(); e++)
                    {
                        if(block->vSigner[0] == 0)
                        {
                            FC_gState->m_TmpScript1->SetElement(e);                        
                            *sig_size=255;
                            key_size=255;    
                            if(FC_gState->m_TmpScript1->GetBlockSignature(sig,sig_size,hash_type,block->vSigner+1,&key_size) == 0)
                            {
    ...
    if(block->vSigner[0])
    {
        switch(hash_type)
        {
            ...
            case BLOCKSIGHASH_NO_SIGNATURE_AND_NONCE:
                ...
                break;
            default:
                LogPrintf("FChn: Invalid hash type received in block signature\n");
...
//        printf("V: %s -> %s\n",hash_to_verify.GetHex().c_str(),block->GetHash().GetHex().c_str());
        ...
        {
            LogPrintf("FChn: Invalid pubkey received in block signature\n");
            block->nSigHashType=BLOCKSIGHASH_INVALID;
            return false;
        }
        if(!pubKeyOut.Verify(hash_to_verify,vchSigOut))
        {
            LogPrintf("FChn: Wrong block signature\n");
            block->nSigHashType=BLOCKSIGHASH_INVALID;
            return false;
        }        
    }
    else
    {
        if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
        {
            ...
            {
                LogPrintf("FChn: Block signature not found\n");                
                block->nSigHashType=BLOCKSIGHASH_INVALID;
                return false;
            }
        }
...
{
    if( (FC_gState->m_NetworkParams->IsProtocolFloatchain() == 0) ||
        (FC_gState->m_Features->CachedInputScript() == 0) ||
        (FC_gState->m_NetworkParams->GetInt64Param("supportminerprecheck") == 0) ||
        (FCP_ANYONE_CAN_MINE) )                               
    {
    ...
    int32_t offsets[FC_PLS_SIZE_OFFSETS_PER_ROW];
    uint256 block_hash;
    ...
    LogPrint("FChn","VerifyBlockMiner: Block: %d, Fork: %d, Chain: %d\n",pindexNew->nHeight,pindexFork->nHeight,FC_gState->m_Permissions->m_Block);
    ...
    last_after_fork=0;
    fRolledBack=true;
    FC_gState->m_Permissions->RollBackBeforeMinerVerification(pindexFork->nHeight);
    LogPrint("FChn","VerifyBlockMiner: Rolled back to block %d\n",FC_gState->m_Permissions->m_Block);

    for(pos=0;pos<branch_size;pos++)
    {
        LogPrint("FChn","VerifyBlockMiner: Verifying block %d\n",FC_gState->m_Permissions->m_Block+1);
        pindex=branch[pos];
        ...
        if(!fVerify && (FC_gState->m_Permissions->GetBlockMiner(&block_hash,(unsigned char*)&miners[pos],&admin_miner_count) == FC_ERR_NOERROR) )
        {
            LogPrint("FChn","VerifyBlockMiner: Verified block %s (height %d)\n",pindex->GetBlockHash().ToString().c_str(),pindex->nHeight);
            if(miners[pos] == miners[branch_size - 1])
            {
                last_after_fork=pindex->nHeight;
            }
            record=1;
            while(FC_gState->m_Permissions->GetBlockAdminMinerGrants(&block_hash,record,offsets) == FC_ERR_NOERROR)
            {
                for(i=0;i<FC_PLS_SIZE_OFFSETS_PER_ROW;i++)
                {
                    if(offsets[i])
                    {
                        CTransaction tx;
            ...
            FC_gState->m_Permissions->IncrementBlock(admin_miner_count);
        }
        else
        {
            if(pblock == NULL)
            {
                LogPrint("FChn","VerifyBlockMiner: Unverified block %s (height %d)\n",pindex->GetBlockHash().ToString().c_str(),pindex->nHeight);
                ...
                if(FC_gState->m_Permissions->CanMineBlockOnFork(&miners[pos],pindex->nHeight,last_after_fork) == 0)
                {
            ...
            {
                LogPrint("FChn","VerifyBlockMiner: New block %s (height %d)\n",pindex->GetBlockHash().ToString().c_str(),pindex->nHeight);
                if(FC_gState->m_Permissions->CanMineBlockOnFork(&miners[pos],pindex->nHeight,last_after_fork) == 0)
                {
            ...
            for (unsigned int i = 0; i < pblock->vtx.size(); i++)
            {
                ...
                string reason;
                uint32_t result;
                int mempool_size=FC_gState->m_Permissions->m_MemPool->GetCount();
                ...
                if(mempool_size != FC_gState->m_Permissions->m_MemPool->GetCount())
                {
                    LogPrint("FChn","VerifyBlockMiner: Grant tx %s in block %s (height %d)\n",tx.GetHash().ToString().c_str(),pindex->GetBlockHash().ToString().c_str(),pindex->nHeight);
                }
            ...
            FC_gState->m_Permissions->StoreBlockInfo(&miners[pos],&block_hash);
        }
...
exitlbl:
    ...
    if(fRolledBack)
    {
        LogPrint("FChn","VerifyBlockMiner: Restoring chain, block %d\n",FC_gState->m_Permissions->m_Block);
        FC_gState->m_Permissions->RestoreAfterMinerVerification();
        LogPrint("FChn","VerifyBlockMiner: Restored on block %d\n",FC_gState->m_Permissions->m_Block);
    }
...
bool CheckBlockPermissions(const CBlock& block,CBlockIndex* prev_block,unsigned char *lpMinerAddress)
{
    ...
    LogPrint("FChn","FChn: Checking block signature and miner permissions...\n");
    ...
    FC_gState->m_Permissions->CopyMemPool();
    ...
    if(checked)
    {
        if(prev_block)
        {
            ...
            else
            {
                ...
                {    
                    ...
                    meFCpy(lpMinerAddress,pubKeyHash.begin(),20);
                    if(!FC_gState->m_Permissions->CanMine(NULL,pubKeyHash.begin()))
                    {
                //                FC_DumpSize("Connection address",pubKeyHash.begin(),20,20);
                        LogPrintf("FChn: Permission denied for miner %s received in block signature\n",CBitcoinAddress(pubKeyHash).ToString().c_str());
                        checked = false;
                    }
                }                
            }
        ...
        {
            genesis_key=(unsigned char*)FC_gState->m_NetworkParams->GetParam("genesispubkey",&key_size);
    ...
    FC_gState->m_Permissions->RestoreMemPool();
