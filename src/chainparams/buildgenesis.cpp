...
void FC_GetCompoundHash160(void *result,const void  *hash1,const void  *hash2)
{
    ...
    meFCpy(conc_subkey,hash1,20);
    meFCpy(conc_subkey+20,hash2,20);
    *(uint160*)result= Hash160(conc_subkey,conc_subkey+40);    
}

int FC_RandomEncodedBase58String(char * dest,int size)
{
...
int FC_GenerateConfFiles(const char *network_name)
{
    ...
    fileHan=FC_OpenFile(NULL,"floatchain",".conf","r",FC_FOM_RELATIVE_TO_DATADIR | FC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=FC_OpenFile(NULL,"floatchain",".conf","w",FC_FOM_RELATIVE_TO_DATADIR);
    }
    if(fileHan)
    {
    ...
    fileHan=FC_OpenFile(network_name,"floatchain",".conf","r",FC_FOM_RELATIVE_TO_DATADIR | FC_FOM_CREATE_DIR);
    if(fileHan == NULL)
    {
        fileHan=FC_OpenFile(network_name,"floatchain",".conf","w",FC_FOM_RELATIVE_TO_DATADIR);
        if(fileHan)
        {
            fprintf(fileHan,"rpcuser=floatchainrpc\n");
            FC_RandomEncodedBase58String(rpcpwd,32);
...
int FC_FloatchainParams::Build(const unsigned char* pubkey, int pubkey_size) 
{
    if(m_Status == FC_PRM_STATUS_VALID)
    {
        return FC_ERR_NOERROR;
    }
    ...
    {                
        ...
        if(IsProtocolFloatchain())
        {
            ...
            if(FC_gState->m_Features->Streams())
            {
                lpScript->SetPermission(FC_PTP_GLOBAL_ALL,0,0xffffffff,timestamp);
            }
            else
            {
                lpScript->SetPermission(FC_PTP_ALL,0,0xffffffff,timestamp);                
            }
    ...
    FC_HexToBin(hash,genesis.GetHash().ToString().c_str(),32);
