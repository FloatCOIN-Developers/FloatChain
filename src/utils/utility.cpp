...
#define FC_DCT_BUF_ALLOC_ITEMS          256
#define FC_DCT_LIST_ALLOC_MIN_SIZE      32768
#define FC_DCT_LIST_ALLOC_MAX_SIZE      268435456
...
{
    meFCpy(dest,src,dest_size);                                                 // Assuming all systems are little endian
}
...
void FC_MemoryDump(void *ptr,                                                   // Pointer to the memory to dump
...
void FC_MemoryDumpCharSizeToFile(
                FILE *fHan,
                const void *ptr, 
                int from,         
                int len,
...
void FC_MemoryDumpCharSize(const void *ptr, 
                int from,         
                int len,
                int row_size)          
{
...
void FC_MemoryDumpChar(const void *ptr, 
                int from,         
                int len)
{
    FC_MemoryDumpCharSize(ptr,from,len,16);
}
...
{
    printf("%s\n",message);
    FC_MemoryDumpCharSize(ptr,0,size,row_size);
    printf("\n");
}

void FC_Dump(const char * message,const void *ptr,int size)
{
    FC_DumpSize(message,ptr,size,16);
}
...
int FC_Buffer::Initialize(int KeySize,int RowSize,uint32_t Mode)
{
    int err;
    ...
    if(m_Mode & FC_BUF_MODE_MAP)
    {
        m_lpIndex=new FC_MapStringIndex;
    }
        
    
    m_AllocSize=FC_AllocSize(1,FC_DCT_BUF_ALLOC_ITEMS,m_RowSize);
    
    m_lpData=(unsigned char*)FC_New(m_AllocSize);
    if(m_lpData==NULL)
    {
        Zero();
        err=FC_ERR_ALLOCATION;
        return err;
    }
...
int FC_Buffer::Realloc(int Rows)
{
    ...
    {
        NewSize=FC_AllocSize(m_Count+Rows,FC_DCT_BUF_ALLOC_ITEMS,m_RowSize);
        ...
        meFCpy(lpNewBuffer,m_lpData,m_AllocSize);
        FC_Delete(m_lpData);
...
int FC_Buffer::Add(const void *lpKey,const void *lpValue)
{
    int err;
    ...
    if(m_KeySize)
    {
        meFCpy(m_lpData+m_Size,lpKey,m_KeySize);
    }
    
    if(m_KeySize<m_RowSize)
    {
        meFCpy(m_lpData+m_Size+m_KeySize,lpValue,m_RowSize-m_KeySize);
    }
...
int FC_Buffer::Add(const void *lpKeyValue)
{
    return Add(lpKeyValue,(unsigned char*)lpKeyValue+m_KeySize);
}

int FC_Buffer::PutRow(int RowID,const void *lpKey,const void *lpValue)
{
    unsigned char *ptr;
    ...
    if(m_KeySize)
    {
        meFCpy(ptr,lpKey,m_KeySize);
    }
    
    if(m_KeySize<m_RowSize)
    {
        meFCpy(ptr+m_KeySize,lpValue,m_RowSize-m_KeySize);
    }
...
int FC_Buffer::Seek(void *lpKey)
{
    ...
    if(m_lpIndex)
    {
        ...
        {
            ptr=GetRow(row);
            if(meFCmp(ptr,lpKey,m_KeySize)==0)
            {
                return row;
            }
        }
...
unsigned char *FC_Buffer::GetRow(int RowID)
{
    return m_lpData+m_RowSize*RowID;
}

int FC_Buffer::GetCount()
{
    return m_Count;
}

int FC_Buffer::SetCount(int count)
{    
    int i;
...
void FC_Buffer::CopyFrom(FC_Buffer *source)
{
    Clear();
    int i;
    unsigned char *ptr;
...
int FC_Buffer::Sort()                                                           
{
    ...
    for(i=0;i<m_Count-1;i++)                                                    // This is never used, so we can consider entire function as stub
    {
        for(j=i;j>=0;j--)
        {
            if(meFCmp(GetRow(j),GetRow(j+1),m_RowSize) > 0)
            {
                meFCpy(GetRow(t),GetRow(j+1),m_RowSize);
                meFCpy(GetRow(j+1),GetRow(j),m_RowSize);
                meFCpy(GetRow(j),GetRow(t),m_RowSize);
            }
        }
    }
...
int FC_List::Put(unsigned char *ptr, int size)
{
    ...
    while(m_Size+true_size+(int)sizeof(int)>m_AllocSize)
    {
        if(m_AllocSize>0)
        {
            ...
            if(NewSize> FC_DCT_LIST_ALLOC_MAX_SIZE)
            {
        ...
        {
            NewSize=FC_DCT_LIST_ALLOC_MIN_SIZE;
        }
                
        NewBuffer=(unsigned char*)FC_New(NewSize);
        if(NewBuffer == NULL)
        {
            return FC_ERR_ALLOCATION;
        }
        else
        {
            if(m_lpData)
            {
                if(m_Size)
                {
                    meFCpy(NewBuffer,m_lpData,m_Size);
                }
                FC_Delete(m_lpData);
            }
            m_lpData=NULL;                    
    ...
    *(int *)(m_lpData+m_Size)=true_size;
    ...
    if(true_size)
    {
        meFCpy(m_lpData+m_Size,ptr,true_size);
    }
    m_Size+=true_size;
...
unsigned char *FC_List::First()
{
    m_Pos=0;
    return Next();
}

unsigned char *FC_List::Next()
{
    unsigned char *ptr;
    if(m_Pos>=m_Size)
    {
...
int FC_VarIntSize(unsigned char byte)
{
...
int64_t FC_GetVarInt(const unsigned char *buf,int max_size,int64_t default_value,int* shift)
{
    ...
    return FC_GetLE((void*)(buf+1),size);
}
...
int FC_PutVarInt(unsigned char *buf,int max_size,int64_t value)
{
    ...
    FC_PutLE(buf+shift,&value,varint_size);    
...
int FC_TerminalInput::LoadDataFromLog(const char* fileName)
{
    int fHan;
    char *raw;
    ...
    raw=(char*)FC_New(FC_DCT_TERM_BUFFER_SIZE);
    err=FC_ERR_NOERROR;
...
int FC_TerminalInput::IsAlphaNumeric(char c)
{
    if( (c >= 0x30) && (c <= 0x39) )
    {
        return 1;
...
int FC_TerminalInput::SetPrompt(const char* prompt)
{
    strcpy(m_Prompt,prompt);
    return FC_ERR_NOERROR;
}

int FC_TerminalInput::Prompt()
{
    printf("%s: ",m_Prompt);
    fflush(stdout);
    return FC_ERR_NOERROR;
...
int FC_TerminalInput::TerminalCols()
{
    struct winsize max;
...
void FC_TerminalInput::MoveBack(int offset)
{
    int p,q,i;
...
void FC_TerminalInput::SaveLine()
{
...
int FC_TerminalInput::LoadLine(int line)
{
    int len;
    if(line<m_FirstLine)
    {
