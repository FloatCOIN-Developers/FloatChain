...
#include "floatchain/floatchain.h"                                              // FCHN
...
/* FCHN START */
static boost::filesystem::path pathCachedFloatChain;
...
/* FCHN START */
    ...
    pathCachedFloatChain=fs::path(string(FC_gState->m_Params->DataDir()));
    fs::path &path =pathCachedFloatChain;
    return path;
...
/* FCHN START */
    ...
    if (!path.empty())
        return path;
    path=fs::path(string(FC_gState->m_Params->DataDir()));
    return pathCachedFloatChain;
//    pathCachedFloatChain=fs::path(string(FC_gState->m_Params->DataDir()));
//    fs::path &path =pathCachedFloatChain;
//    return path;
//    fs::path &path = fNetSpecific ? pathCachedNetSpecific : pathCached;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", string(FC_gState->m_Params->NetworkName()) + ".conf"));    // FCHN
...
{
    boost::filesystem::ifstream streaFConfig(GetConfigFile());
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "floatchain.pid"));
