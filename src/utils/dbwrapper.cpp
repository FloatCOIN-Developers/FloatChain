...
int cs_Database::Zero()
{
    ...
    m_ShMemKey=0;
    m_MaxClients=FC_DCT_DB_DEFAULT_MAX_CLIENTS;
    m_KeySize=FC_DCT_DB_DEFAULT_MAX_KEY_SIZE;
    m_ValueSize=FC_DCT_DB_DEFAULT_MAX_VALUE_SIZE;
    m_MaxRows=FC_DCT_DB_DEFAULT_MAX_ROWS;
    m_Semaphore=NULL;
    m_Signal=0;            
    m_LastError=0;
    ...
    m_Options=FC_OPT_DB_DATABASE_DEFAULT;
    m_Status=FC_STT_DB_DATABASE_CLOSED;  
    m_Log=NULL;
...
//    double start_time;
    ...
    {
        m_ReadBuffer=(char*)FC_New(FC_DCT_DB_READ_BUFFER_SIZE);
        if(m_ReadBuffer  == NULL)
        {
    ...
    switch(m_Options & FC_OPT_DB_DATABASE_TYPE_MASK)
    {
        case FC_OPT_DB_DATABASE_LEVELDB:    
//            return FC_ERR_NOT_SUPPORTED;

            if((m_Options & FC_OPT_DB_DATABASE_DELAYED_OPEN) == 0)
            {
                ...
                if(Options & FC_OPT_DB_DATABASE_CREATE_IF_MISSING)
                {
                    leveldb_options_set_create_if_missing((leveldb_options_t*)m_OpenOptions, 1);
                }
                
                if(Options & FC_OPT_DB_DATABASE_TRANSACTIONAL)
                {
...
//                m_Iterator=(void*)leveldb_create_iterator((leveldb_t*)m_DB,(leveldb_readoptions_t*)m_IterOptions);
        ...
        case FC_OPT_DB_DATABASE_REMOTE_SHMEM:    
            
            return FC_ERR_DBOPEN_ERROR;
/*            
            if(m_ShMemKey == 0)
            {
                ierr=FC_ERR_DB_SHMEM_KEY_NOT_SET;
                return ierr;
            }
            ...
            {
                ierr=FC_ERR_DB_SHMEM_CANNOT_OPEN;
                return ierr;
            }
            ...
            while((m_lpShMemSlot==NULL) && (cs_TimeNow()-start_time<FC_DCT_DB_DEFAULT_MAX_SHMEM_TIMEOUT))
            {
                if(m_lpShMem[FC_OFF_DB_SHMEM_HEAD_RESPONSE_PROCESS_ID]!=pid)
                {
                    m_lpShMem[FC_OFF_DB_SHMEM_HEAD_REQUEST_PROCESS_ID]=pid;
                }
                else
                {
                    m_lpShMemSlot=m_lpShMem+(m_lpShMem[FC_OFF_DB_SHMEM_HEAD_DATA_OFFSET]+
                            m_lpShMem[FC_OFF_DB_SHMEM_HEAD_RESPONSE_SLOT_ID]*m_lpShMem[FC_OFF_DB_SHMEM_HEAD_SLOT_SIZE])/sizeof(uint64_t);
                    m_lpShMem[FC_OFF_DB_SHMEM_HEAD_REQUEST_PROCESS_ID]=0;
                    m_lpShMem[FC_OFF_DB_SHMEM_HEAD_RESPONSE_PROCESS_ID]=0;
                }                
            }
            ...
            if(m_lpShMemSlot==NULL)
            {
                ...
                __US_ShMeFClose(m_hShMem);
                m_hShMem=NULL;
                ierr=FC_ERR_DB_SHMEM_CANNOT_CONNECT;
                return ierr;
            }
  */          
            break;
    ...
    m_Status=FC_STT_DB_DATABASE_OPENED;  
...
/*            
            ...
            {
                pid=__US_GetProcessId();
                if(m_lpShMemSlot[FC_OFF_DB_SHMEM_SLOT_PROCESS_ID] == pid)
                {
                    m_lpShMemSlot[FC_OFF_DB_SHMEM_SLOT_ROW_COUNT]=0;
                    m_lpShMemSlot[FC_OFF_DB_SHMEM_SLOT_RESULT]=FC_PRT_DB_SHMEM_SLOT_RESULT_UNDEFINED;
                    rptr=m_lpShMemSlot+m_lpShMem[FC_OFF_DB_SHMEM_HEAD_SLOT_DATA_OFFSET]/sizeof(uint64_t);
                    rptr[FC_OFF_DB_SHMEM_ROW_RESPONSE]=FC_PRT_DB_SHMEM_ROW_RESPONSE_UNDEFINED;
                    rptr[FC_OFF_DB_SHMEM_ROW_REQUEST]=FC_PRT_DB_SHMEM_ROW_REQUEST_CLOSE;
                    m_lpShMemSlot[FC_OFF_DB_SHMEM_SLOT_ROW_COUNT]=1;
                    start_time=cs_TimeNow();
                    while((m_lpShMemSlot[FC_OFF_DB_SHMEM_SLOT_PROCESS_ID] == pid) && (cs_TimeNow()-start_time<FC_DCT_DB_DEFAULT_MAX_SHMEM_TIMEOUT))
                    {
                        __US_Sleep(1);
                    }
                }
...
char *cs_Database::MoveNext(int *error)
{
    ...
    {
        NewSize=((keylen+vallen)/FC_DCT_DB_READ_BUFFER_SIZE + 1) * FC_DCT_DB_READ_BUFFER_SIZE;
        lpNewBuffer=(char*)FC_New(NewSize);
        if(lpNewBuffer  == NULL)
        {
            *error=FC_ERR_ALLOCATION;
            return NULL;
        }
    ...
    meFCpy(m_ReadBuffer,lpKey,keylen);
    meFCpy(m_ReadBuffer+keylen,lpValue,vallen);
...
{
    *error=FC_ERR_OPERATION_NOT_SUPPORTED;
    return NULL;
}
...
char *cs_Database::Read(char *key,int key_len,int *value_len,int Options,int *error)
{
    ...
    *value_len=0;
    *error=FC_ERR_NOERROR;
    
    if(key == NULL)
    {
        *error=FC_ERR_INTERNAL_ERROR;
        return NULL;
    }
    if(klen<0)klen=strlen(key);
...
//            *error=FC_ERR_NOT_SUPPORTED;
//            return NULL;

            if(Options & FC_OPT_DB_DATABASE_SEEK_ON_READ)
            {
//                *error=FC_ERR_OPERATION_NOT_SUPPORTED;
//                return NULL;                
                lpIterRead=NULL;
                ...
                if(leveldb_iter_valid((leveldb_iterator_t*)m_Iterator))
                {
                    ...
                    if(lpIterRead)
                    {
                        if( ((int)vallen == klen) && (meFCmp(lpIterRead,key,klen) == 0) )
                        {
    ...
    {
        NewSize=((*value_len)/FC_DCT_DB_READ_BUFFER_SIZE + 1) * FC_DCT_DB_READ_BUFFER_SIZE;
        lpNewBuffer=(char*)FC_New(NewSize);
        if(lpNewBuffer  == NULL)
        {
            *value_len=0;
    ...
    if(lpRead)
    {
        meFCpy(m_ReadBuffer,lpRead,*value_len);
    }
    if(lpIterRead)
    {
        meFCpy(m_ReadBuffer,lpIterRead,*value_len);
    }
...
//    cs_LogMessage(m_Log,FC_LOG_REPORT,"C-0083","Commit",msg);
