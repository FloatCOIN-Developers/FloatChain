...
FC_WalletTxs* pwalletTxsMain = NULL;
#endif
...
string FC_ParseIPPort(string strAddr,int *port)
{
    *port=0;
    string s_ip=strAddr;
...
std::string HelpMessage(HelpMessageMode mode)                                   // FCHN
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "floatchain.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "floatchain.pid") + "\n";
#endif
    strUsage += "  -reindex               " + _("Rebuild the blockchain and reindex transactions on startup.") + "\n";
#if !defined(WIN32)
    strUsage += "  -sysperms              " + _("Create new files with system default permissions, instead of umask 077 (only effective with disabled wallet functionality)") + "\n";
    strUsage += "  -shortoutput           " + _("Returns connection string if this node can start or default floatchain address otherwise") + "\n";
#endif
/* FCHN START */    
/* Default was 0 */    
...
/* FCHN END */    
    ...
    strUsage += "\n" + _("FloatChain runtime parameters") + "\n";    
    strUsage += "  -initprivkey=<privkey>                   " + _("Manually set the wallet default address and private key when running floatchaind for the first time.") + "\n";
    strUsage += "  -handshakelocal=<address>                " + _("Manually override the wallet address which is used for handshaking with other peers in a FloatChain blockchain.") + "\n";
    strUsage += "  -hideknownopdrops=<n>                    " + strprintf(_("Remove recognized FloatChain OP_DROP metadata from the responses to JSON_RPC calls (default: %u)"), 0) + "\n";
...
{
    return FormatParagraph(_("Copyright (c) Coin Sciences Ltd - www.floatchain.com")) + "\n" +
           "\n" +
           ...
           "\n" +
           FormatParagraph(_("Full terms are shown at: http://www.floatchain.com/terms-of-service/")) +
           "\n";
...
/* FCHN END */    
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. FloatChain Core is shutting down."));

    std::string strDataDir = GetDataDir().string();
    LogPrint("FChn","FChn: Data directory: %s\n",strDataDir.c_str());
#ifdef ENABLE_WALLET
...
// Anyway, we don't need this lock as we have permission db lock    
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. FloatChain Core is probably already running."), strDataDir));
#endif
/* FCHN END */
...
/* FCHN START */    
    ...
    {
        LogPrintf("FloatChain version %s (%s)\n", FC_gState->GetFullVersion(), CLIENT_DATE);
    }
...
/* FCHN START*/    
    uiInterface.InitMessage(_("Initializing floatchain..."));
    ...
    {    
        ...
        if (nLoadWalletRetForBuild != DB_LOAD_OK)                                   // FCHN-TODO wallet recovery
        {
        ...
        if(!pwalletMain->vchDefaultKey.IsValid())
        {
            ...
            {
                LogPrintf("FChn: Default key is specified using -initprivkey - not created\n");                
            }
            else
            {
                LogPrintf("FChn: Default key is not found - creating new... \n");
    ...
    //            RandAddSeedPerfmon();
        ...
        else
        {
            if(init_privkey.size())
            {
                LogPrintf("FChn: Wallet already has default key, -initprivkey is ignored\n");                
    ...
    seed_node=FC_gState->GetSeedNode();
    ...
    {        
        ...
        if((FC_gState->m_NetworkParams->m_Status == FC_PRM_STATUS_EMPTY) 
           || (FC_gState->m_NetworkParams->m_Status == FC_PRM_STATUS_MINIMAL))
        {
            if(FC_gState->m_NetworkParams->m_Status == FC_PRM_STATUS_MINIMAL)
            {
                InitializeFloatChainParams();                    
                if(init_privkey.size())
                {
                    if(seed_attempt == 1)
                    {
                        if(FC_gState->m_NetworkParams->GetParam("privatekeyversion",NULL) == NULL)
                        {
                            return InitError(_("The initprivkey runtime parameter can only be used when connecting to FloatChain 1.0 beta 2 or later"));                                                        
                        }
            ...
            LogPrintf("FChn: Parameter set is not complete - starting paramset discovery thread...\n");
            boost::thread_group seedThreadGroup;

            FC_gState->m_NetworkState=FC_NTS_WAITING_FOR_SEED;
            ...
            if(FC_QuerySeed(seedThreadGroup,seed_node))
            {
                if((FC_gState->m_NetworkState == FC_NTS_SEED_READY) || (FC_gState->m_NetworkState == FC_NTS_SEED_NO_PARAMS) )
                {
                    seed_error="Couldn't disconnect from the seed node, please restart floatchaind";
                }
                else
                {
                    if(seed_port.size() == 0)
                    ...
                    {
                        seed_error=strprintf("Couldn't connect to the seed node %s on port %s - please check floatchaind is running at that address and that your firewall settings allow incoming connections.",                
                ...
                (FC_gState->GetProtocolVersion() < (int)FC_gState->m_NetworkParams->GetInt64Param("protocolversion")) )
            {
                seed_error=strprintf("Couldn't connect to the seed node %s on port %s.\n"
                            "Blockchain was created by floatchaind with newer protocol version (%d)\n"                
                            "Please upgrade to the latest version of FloatChain or connect only to blockchains using protocol version %d or earlier.\n",                
                        seed_ip.c_str(),seed_port.c_str(),(int)FC_gState->m_NetworkParams->GetInt64Param("protocolversion"), FC_gState->GetProtocolVersion());
            }
            else
            {                                
                if( (FC_gState->m_NetworkParams->GetParam("protocolversion",&size) != NULL) &&
                ...
                {
                    seed_error=strprintf("The protocol version (%d) for blockchain %s has been deprecated and was last supported in FloatChain 1.0 beta 1\n",                
                            (int)FC_gState->m_NetworkParams->GetInt64Param("protocolversion"), FC_gState->m_Params->NetworkName());                    
                    return InitError(seed_error);            
                }
                else
                {                                
                    if(FC_gState->m_NetworkState == FC_NTS_SEED_NO_PARAMS)
                    {
                        char fileName[FC_DCT_DB_MAX_PATH];
                        FC_GetFullFileName(FC_gState->m_Params->NetworkName(),"params", ".dat",FC_FOM_RELATIVE_TO_DATADIR,fileName);
            ...
            LogPrintf("FChn: Exited from paramset discovery thread\n");        

            if(FC_gState->m_NetworkParams->m_Status == FC_PRM_STATUS_VALID)
            {
                SelectFloatChainParams(FC_gState->m_Params->NetworkName());
                delete FC_gState->m_Permissions;
                FC_gState->m_Permissions= new FC_Permissions;
                if(FC_gState->m_Permissions->Initialize(FC_gState->m_Params->NetworkName(),0))                                
                {
        ...
        {
            if(FC_gState->m_NetworkParams->m_Status == FC_PRM_STATUS_GENERATED)
            {
                const unsigned char *pubKey=pwalletMain->vchDefaultKey.begin();
                int pubKeySize=pwalletMain->vchDefaultKey.size();

                LogPrintf("FChn: Parameter set is new, THIS IS GENESIS NODE - looking for genesis block...\n");
                if(!GetBoolArg("-shortoutput", false))
                {    
                    sprintf(bufOutput,"Looking for genesis block...\n");
                    bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));                
                }
                FC_gState->m_NetworkParams->SetGlobals();                           // Needed to update IsProtocolFloatchain flag in case of bitcoin
                if(FC_gState->m_NetworkParams->Build(pubKey,pubKeySize))
                {
                    return InitError(_("Cannot build new blockchain"));
                }
                LogPrintf("FChn: Genesis block found\n");
                if(!GetBoolArg("-shortoutput", false))
                {    
    ...
    if(FC_gState->m_NetworkParams->m_Status == FC_PRM_STATUS_VALID)
    {
        LogPrintf("FChn: Parameter set is valid - initializing blockchain parameters...\n");
        FC_gState->m_NetworkParams->SetGlobals();
        InitializeFloatChainParams();        

        if(GetBoolArg("-reindex", false))
        {
            FC_RemoveDir(FC_gState->m_Params->NetworkName(),"entities.db");
            FC_RemoveFile(FC_gState->m_Params->NetworkName(),"entities",".dat",FC_FOM_RELATIVE_TO_DATADIR);
        }
        
        FC_gState->m_Assets= new FC_AssetDB;
        if(FC_gState->m_Assets->Initialize(FC_gState->m_Params->NetworkName(),0))                                
        {
            seed_error=strprintf("ERROR: Couldn't initialize asset database for blockchain %s. Please restart floatchaind with reindex=1.\n",FC_gState->m_Params->NetworkName());
        ...
        if(wallet_mode == 0)
        {
            FC_gState->m_WalletMode=FC_WMD_AUTO;
            wallet_mode_valid=true;
        }
        if(wallet_mode == FC_TDB_WALLET_VERSION)
        {
            FC_gState->m_WalletMode=FC_WMD_TXS | FC_WMD_ADDRESS_TXS; 
            wallet_mode_valid=true;
        }
        if(wallet_mode == 1)
        {
            FC_gState->m_WalletMode=FC_WMD_NONE;
            wallet_mode_valid=true;
            zap_wallet_txs=false;
        }
        if(wallet_mode == -1)
        {
            FC_gState->m_WalletMode=FC_WMD_TXS | FC_WMD_ADDRESS_TXS | FC_WMD_MAP_TXS;            
            wallet_mode_valid=true;
            zap_wallet_txs=false;
        }
        ...
        vector <FC_TxEntity> vSubscribedEntities;
        if(GetBoolArg("-reindex", false) || GetBoolArg("-rescan", false))
        {
            if(FC_gState->m_Features->Streams())
            {
                pwalletTxsMain=new FC_WalletTxs;
                if(pwalletTxsMain->Initialize(FC_gState->m_NetworkParams->Name(),FC_WMD_TXS | FC_WMD_ADDRESS_TXS) == FC_ERR_NOERROR)
                {
                    FC_Buffer *entity_list;
                    entity_list=pwalletTxsMain->GetEntityList();
                    for(int e=0;e<entity_list->GetCount();e++)
                    {
                        FC_TxEntityStat *stat;
                        stat=(FC_TxEntityStat *)entity_list->GetRow(e);
                        switch(stat->m_Entity.m_EntityType & FC_TET_TYPE_MASK)
                        {
            ...
            FC_RemoveDir(FC_gState->m_Params->NetworkName(),"wallet");            
            zap_wallet_txs=true;
        }
        ...
        else
        {
            if(!boost::filesystem::exists(pathWallet))
            {
                if((FC_gState->m_Permissions->m_Block >= 0) && !GetBoolArg("-rescan", false))
                {
                    if(FC_gState->m_WalletMode == FC_WMD_AUTO)
                    {
                        FC_gState->m_WalletMode=FC_WMD_NONE;
                    }
                    if(FC_gState->m_WalletMode != FC_WMD_NONE)
                    {
                        return InitError(strprintf("Wallet was created in version 1. To switch to version 2, with better performance and scalability, run: \nfloatchaind %s -walletdbversion=2 -rescan\n",FC_gState->m_NetworkParams->Name()));                                        
                    }                    
                }
                else
                {
                    ...
                    {
                        FC_gState->m_WalletMode = FC_WMD_TXS | FC_WMD_ADDRESS_TXS;
                        wallet_mode=FC_TDB_WALLET_VERSION;
                    }
                }
            }
            ...
            if(FC_gState->m_WalletMode != FC_WMD_NONE)
            {
                ...
                {
                    FC_gState->m_WalletMode |= FC_WMD_DEBUG;
                }
                string autosubscribe=GetArg("-autosubscribe","none");
                
                if(autosubscribe=="streams")
                {
                    FC_gState->m_WalletMode |= FC_WMD_AUTOSUBSCRIBE_STREAMS;
                }
                if(autosubscribe=="assets")
                {
                    FC_gState->m_WalletMode |= FC_WMD_AUTOSUBSCRIBE_ASSETS;
                }
                ...
                if(pwalletTxsMain->Initialize(FC_gState->m_NetworkParams->Name(),FC_gState->m_WalletMode))
                {
                    return InitError("Wallet tx database corrupted. Please restart floatchaind with -rescan\n");                        
                }

                if(FC_gState->m_WalletMode & FC_WMD_AUTO)
                {
                    FC_gState->m_WalletMode=pwalletTxsMain->m_Database->m_DBStat.m_InitMode;
                ...
                {
                    return InitError(strprintf("Wallet tx database was created with different wallet version (%d). Please restart floatchaind with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_WalletVersion));                        
                }        

                if((pwalletTxsMain->m_Database->m_DBStat.m_InitMode & FC_WMD_MODE_MASK) != (FC_gState->m_WalletMode & FC_WMD_MODE_MASK))
                {
                    return InitError(strprintf("Wallet tx database was created in different mode (%08X). Please restart floatchaind with reindex=1 \n",pwalletTxsMain->m_Database->m_DBStat.m_InitMode));                        
                }        
            }
        }
        LogPrint("FCblockperf","FChn-block-perf: Wallet initialization completed (%s)\n",(FC_gState->m_WalletMode & FC_WMD_TXS) ? pwalletTxsMain->Summary() : "");
        LogPrintf("Wallet mode: %08X\n",FC_gState->m_WalletMode);
        ...
        if(pwalletMain == NULL)                                                 // Opening wallet only after floatchain parameters were initizalized
        {
            pwalletMain = new CWallet(strWalletFile);
            DBErrors nLoadWalletRetForBuild = pwalletMain->LoadWallet(fFirstRunForBuild);
        ...
        if(pwalletMain)
        {
            ...
            if(new_wallet_txs)
            {
                ...
                entity.m_EntityType=FC_TET_WALLET_ALL | FC_TET_TIMERECEIVED;
                pwalletTxsMain->AddEntity(&entity,0);        
                entity.m_EntityType=FC_TET_WALLET_SPENDABLE | FC_TET_CHAINPOS;
                pwalletTxsMain->AddEntity(&entity,0);
                entity.m_EntityType=FC_TET_WALLET_SPENDABLE | FC_TET_TIMERECEIVED;
                pwalletTxsMain->AddEntity(&entity,0);

                for(int e=0;e<(int)vSubscribedEntities.size();e++)
                {
                    pwalletTxsMain->AddEntity(&(vSubscribedEntities[e]),FC_EFL_NOT_IN_SYNC);                    
                }                                
                ...
                BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
                {
                    ...
                    if(lpKeyID)
                    {
                        meFCpy(entstat.m_Entity.m_EntityID,lpKeyID,FC_TDB_ENTITY_ID_SIZE);
                        ...
                        if(!pwalletTxsMain->FindEntity(&entstat))
                        {
                            ...
                            entstat.m_Entity.m_EntityType=FC_TET_PUBKEY_ADDRESS | FC_TET_TIMERECEIVED;
                            pwalletTxsMain->AddEntity(&(entstat.m_Entity),0);
                        }
                    }
                    ...
                    if(lpScriptID)
                    {
                        meFCpy(entstat.m_Entity.m_EntityID,lpScriptID,FC_TDB_ENTITY_ID_SIZE);
                        entstat.m_Entity.m_EntityType=FC_TET_SCRIPT_ADDRESS | FC_TET_CHAINPOS;
                        if(!pwalletTxsMain->FindEntity(&entstat))
                        {
                            pwalletTxsMain->AddEntity(&(entstat.m_Entity),0);
                            entstat.m_Entity.m_EntityType=FC_TET_SCRIPT_ADDRESS | FC_TET_TIMERECEIVED;
                            pwalletTxsMain->AddEntity(&(entstat.m_Entity),0);
                        }
                    }
                }
            ...
            if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
            {
                ...
                {
                    LogPrint("FChn","FChn: Default connect  address: %s\n",CBitcoinAddress(pkey.GetID()).ToString().c_str());                
                }
                if(pwalletMain->GetKeyFromAddressBook(pkey,FC_PTP_SEND))
                {
                    LogPrint("FChn","FChn: Default send     address: %s\n",CBitcoinAddress(pkey.GetID()).ToString().c_str());                
                }
                if(pwalletMain->GetKeyFromAddressBook(pkey,FC_PTP_RECEIVE))
                {
                    LogPrint("FChn","FChn: Default receive  address: %s\n",CBitcoinAddress(pkey.GetID()).ToString().c_str());                
                }
                if(pwalletMain->GetKeyFromAddressBook(pkey,FC_PTP_CREATE))
                {
                    LogPrint("FChn","FChn: Default create   address: %s\n",CBitcoinAddress(pkey.GetID()).ToString().c_str());                
                }
                if(pwalletMain->GetKeyFromAddressBook(pkey,FC_PTP_ISSUE))
                {
                    LogPrint("FChn","FChn: Default issue    address: %s\n",CBitcoinAddress(pkey.GetID()).ToString().c_str());                
                }
                if(pwalletMain->GetKeyFromAddressBook(pkey,FC_PTP_MINE))
                {
                    LogPrint("FChn","FChn: Default mine     address: %s\n",CBitcoinAddress(pkey.GetID()).ToString().c_str());                
                }
                if(pwalletMain->GetKeyFromAddressBook(pkey,FC_PTP_ADMIN))
                {
                    LogPrint("FChn","FChn: Default admin    address: %s\n",CBitcoinAddress(pkey.GetID()).ToString().c_str());                
                }
                if(pwalletMain->GetKeyFromAddressBook(pkey,FC_PTP_ACTIVATE))
                {
                    LogPrint("FChn","FChn: Default activate address: %s\n",CBitcoinAddress(pkey.GetID()).ToString().c_str());                
                }
            }
            else
            {
                ...
                {
                    LogPrint("FChn","FChn: Default address: %s\n",CBitcoinAddress(pkey.GetID()).ToString().c_str());                
                }                
            }
            delete pwalletMain;
            pwalletMain=NULL;
        ...
        if(GetBoolArg("-reindex", false))
        {
            FC_gState->m_Assets->RollBack(-1);
            FC_gState->m_Permissions->RollBack(-1);
        }
    }
    else
    {
        if(FC_gState->m_NetworkParams->m_Status == FC_PRM_STATUS_MINIMAL)
        {
            ...
            if(seed_node)
            {
                ...
                fileHan=FC_OpenFile(FC_gState->m_NetworkParams->Name(),"seed",".dat","w",FC_FOM_RELATIVE_TO_DATADIR);
                if(fileHan)
                {
            ...
            if(pwalletMain)
            {
                ...
                if(pwalletMain->vchDefaultKey.IsValid())
                {
                    LogPrintf("FChn: Minimal blockchain parameter set is created, default address: %s\n",
                    ...
                    {    
                        ...
                        sprintf(bufOutput,"floatchain-cli %s grant %s connect\n",FC_gState->m_NetworkParams->Name(),
                             CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
                        sprintf(bufOutput,"floatchain-cli %s grant %s connect,send,receive\n\n",FC_gState->m_NetworkParams->Name(),
                             CBitcoinAddress(pwalletMain->vchDefaultKey.GetID()).ToString().c_str());
                        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
                    }
                    else
    ...
    FC_gState->m_NodePausedState=GetArg("-paused",0);
    LogPrintf("Node paused state is set to %08X\n",FC_gState->m_NodePausedState);
...
/* FCHN START */                
                ...
                if(fThisBound)
                {
                    if(FC_gState->m_IPv4Address == 0)
                    {
                        FC_SetIPv4ServerAddress(strBind.c_str());
                    }
                }
/* FCHN END */                
            }
...
/* FCHN START */    
    ...
    if(FC_gState->m_IPv4Address == 0)
    {
        found_ips=FC_FindIPv4ServerAddress(all_ips,max_ips);
    }
    if(!GetBoolArg("-shortoutput", false))
    {    
        sprintf(bufOutput,"Other nodes can connect to this node using:\n");
        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
        sprintf(bufOutput,"floatchaind %s:%d\n\n",FloatchainServerAddress().c_str(),GetListenPort());
        bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
        if(found_ips > 1)
        {
            ...
            {
                if(all_ips[i_ips] != FC_gState->m_IPv4Address)
                {
                    unsigned char *ptr;
                    ptr=(unsigned char *)(all_ips+i_ips);
                    sprintf(bufOutput,"floatchaind %s@%u.%u.%u.%u:%d\n",FC_gState->m_NetworkParams->Name(),ptr[3],ptr[2],ptr[1],ptr[0],GetListenPort());
                    bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
        ...
        {            
            ...
            {
                int port;
                string s_ip=FC_ParseIPPort(strAddr,&port);
                if(port>0)
                {
                    sprintf(bufOutput,"floatchaind %s@%s\n",FC_gState->m_NetworkParams->Name(),strAddr.c_str());
                    bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));
                    port=GetListenPort();
                }
                else
                {
                    sprintf(bufOutput,"floatchaind %s@%s:%d\n",FC_gState->m_NetworkParams->Name(),strAddr.c_str(),GetListenPort());
                    bytes_written=write(OutputPipe,bufOutput,strlen(bufOutput));                    
                }
            }
            sprintf(bufOutput,"\n");
    ...
    int version=FC_gState->m_NetworkParams->GetInt64Param("protocolversion");
    if(version != FC_gState->GetProtocolVersion())
    {
        if(!GetBoolArg("-shortoutput", false))
        {    
...
/* FCHN START */                
                bool fRet = uiInterface.ThreadSafeMessageBox(
                    strLoadError + ".\n\n" + _("Please restart floatchaind with reindex=1."),
...
/* FCHN END */                
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_TOO_NEW)                              // FCHN
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of FloatChain Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)                         // FCHN
            {
                strErrors << _("Wallet needed to be rewritten: restart FloatChain Core to complete") << "\n";
...
/* FCHN START */  
        ...
        {
            LOCK(cs_main);
            uint32_t paused=FC_gState->m_NodePausedState;
            FC_gState->m_NodePausedState=FC_NPS_MINING | FC_NPS_INCOMING;
            std::string strLockBlockError=SetLockedBlock(GetArg("-lockblock",""));
            FC_gState->m_NodePausedState=paused;
