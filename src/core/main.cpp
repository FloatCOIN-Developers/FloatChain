...
extern FC_WalletTxs* pwalletTxsMain;
...
/* FCHN START */
#define FC_TXSET_BLOCKS 50
set<uint256> setBlockTransactions[FC_TXSET_BLOCKS];
/* FCHN END */

/** Constant stuff for coinbase transactions we create: */
CScript COINBASE_FLAGS;

const string strMessageMagic = "FloatChain Signed Message:\n";
...
int FloatchainNode_ApplyUpgrades()
{
    FC_EntityDetails entity;
    FC_Buffer *permissions;
    permissions=NULL;
    map <uint64_t,int> map_sorted;

    int OldProtocolVersion=FC_gState->m_ProtocolVersionToUpgrade;
    int NewProtocolVersion=0;
    
    permissions=FC_gState->m_Permissions->GetUpgradeList(NULL,NULL);

    for(int i=0;i<permissions->GetCount();i++)
    {        
        FC_PermissionDetails *plsRow;
        plsRow=(FC_PermissionDetails *)(permissions->GetRow(i));
        if(plsRow->m_Type == FC_PTP_UPGRADE)
        {
...
//    for(int i=0;i<upgrades->GetCount();i++)
    {
        ...
        if(plsRow->m_Type == FC_PTP_UPGRADE)
        {
            ...
            {
                if(FC_gState->m_Assets->FindEntityByShortTxID(&entity,plsRow->m_Address))
                {
    ...
    FC_gState->m_Permissions->FreePermissionList(permissions);
    FC_gState->m_ProtocolVersionToUpgrade=NewProtocolVersion;
    
    if(FC_gState->m_ProtocolVersionToUpgrade != OldProtocolVersion)
    {
        LogPrintf("New protocol upgrade version: %d (was %d)\n",FC_gState->m_ProtocolVersionToUpgrade,OldProtocolVersion);
        if(FC_gState->m_ProtocolVersionToUpgrade > FC_gState->GetProtocolVersion())
        {
            LogPrintf("NODE SHOULD BE UPGRADED FROM %d TO %d\n",FC_gState->GetProtocolVersion(),FC_gState->m_ProtocolVersionToUpgrade);
        }
        else
        {
            if(FC_gState->m_ProtocolVersionToUpgrade > FC_gState->m_NetworkParams->ProtocolVersion())
            {
                LogPrintf("NODE IS UPGRADED FROM %d TO %d\n",FC_gState->m_NetworkParams->ProtocolVersion(),FC_gState->m_ProtocolVersionToUpgrade);
                FC_gState->m_NetworkParams->m_ProtocolVersion=FC_gState->m_ProtocolVersionToUpgrade;// UPGRADE CODE HERE
            }        
        }
    }
...
void FloatchainNode_UpdateBlockByHeightList(CBlockIndex *pindex)
{
    if(pindex->nHeight < 0)
    {
        return;
...
bool FloatchainNode_IsBlockChainSynced(CNode *pnode)
{
...
/* FCHN START */    
    ...
    {
        max_op_returns=FCP_MAX_STD_OP_RETURN_COUNT;
    }
/* FCHN END */    
...
/* FCHN END */    
    ...
    string reason;
    if (Params().RequireStandard() && !IsStandardTx(tx, reason, check_for_dust))//FCHN
        return state.DoS(0,
...
/* FCHN END */
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++)
    {
        ...
        {
            if(fDebug)LogPrint("FChn","Conflicting with in-memory %s\n",tx.vin[i].ToString().c_str());
...

    {
        ...
        BOOST_FOREACH(const CTxIn txin, tx.vin) {
            if (!view.HaveCoins(txin.prevout.hash)) {
                if(fDebug)LogPrint("FChn","Missing tx (%s)\n",txin.prevout.hash.ToString().c_str());
                if (pfMissingInputs)
            ...
            if (!tx.IsCoinBase()) {
                for (unsigned int i = 0; i < tx.vin.size(); i++) {
                    ...
                    if (!coins || !coins->IsAvailable(prevout.n)) {
                        if(fDebug)LogPrint("FChn","Missing coin (%s,%d)\n",prevout.hash.ToString().c_str(),prevout.n);
                        return state.Invalid(error("AcceptToMemoryPool : inputs already spent"),
                                 REJECT_DUPLICATE, "bad-txns-inputs-spent");
                    }
                }
...
/* FCHN START */
        ...
        permissions_from=FC_gState->m_Permissions->m_MempoolPermissions->GetCount();
        
        if(FC_gState->m_Features->Streams())
        {
            if(!AcceptFloatChainTransaction(tx,view,-1,true,reason, &replay))
            {
                return state.DoS(0,
                                 error("AcceptToMemoryPool: : AcceptFloatChainTransaction failed %s : %s", hash.ToString(),reason),
                                 REJECT_NONSTANDARD, reason);
            }
        }
        else
        ...
        permissions_to=FC_gState->m_Permissions->m_MempoolPermissions->GetCount();
        entry.SetReplayNodeParams(( (replay & FC_PPL_REPLAY) != 0) ? true : false,permissions_from,permissions_to);
/* FCHN END */
...
/* FCHN END */    
    if(fAddToWallet)
    {
        if(((FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS) == 0) || (FC_gState->m_WalletMode & FC_WMD_MAP_TXS))
        {
...
//    CAmount nSubsidy = 50 * COIN;
    CAmount nSubsidy = FCP_INITIAL_BLOCK_REWARD;// * COIN;
    if(nHeight == 1)
    {
        if(FCP_FIRST_BLOCK_REWARD >= 0)
        {
            nSubsidy = FCP_FIRST_BLOCK_REWARD;// * COIN;
        }
    }
/* FCHN END */    
 ...
 */ 
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 86400 / FCP_TARGET_BLOCK_TIME ||
            pindexBestHeader->GetBlockTime() < GetTime() - 24 * 60 * 60);
/* FCHN END */    
    if (!state)
        lockIBDState = true;
...
//    if (pindexBestForkTip && chainActive.Height() - pindexBestForkTip->nHeight >= 72)
    if (pindexBestForkTip && chainActive.Height() - pindexBestForkTip->nHeight >= 43200 / FCP_TARGET_BLOCK_TIME)
...
//            chainActive.Height() - pindexNewForkTip->nHeight < 72)
            chainActive.Height() - pindexNewForkTip->nHeight < 43200 / FCP_TARGET_BLOCK_TIME)
/* FCHN END */    
    {
...
//    FC_DumpSize("sigScript",(unsigned char*)(&scriptSig.begin()[0]),scriptSig.end()-scriptSig.begin(),scriptSig.end()-scriptSig.begin());
//    FC_DumpSize("scriptPubKey",(unsigned char*)(&scriptPubKey.begin()[0]),scriptPubKey.end()-scriptPubKey.begin(),scriptPubKey.end()-scriptPubKey.begin());
 */ 
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if(fScriptChecks)
            {
                ...
                if(ExtractDestinationScriptValid(coins->vout[prevout.n].scriptPubKey, addressRet))
                {
                    ...
                    if(lpKeyID != NULL)
                    {
                        if(!FC_gState->m_Permissions->CanSend(NULL,(unsigned char*)(lpKeyID)))
                        {
                    ...
                    else
                    {
                        ...
                        if(lpScriptID)
                        {
                            if(FC_gState->m_Permissions->CanSend(NULL,(unsigned char*)(lpScriptID)))
                            {
                                send_permission_flags=SCRIPT_VERIFY_SKIP_SEND_PERMISSION_CHECK;
                                async_count++;
                            }                
...
/* FCHN END */        
    ...
    if(!fJustCheck)
    {
        if(pindex->pprev)
        {
            if(fDebug)LogPrint("FChn","FChn: Connecting block %s (height %d) ...\n",block.GetHash().ToString().c_str(),pindex->pprev->nHeight+1);
        }
        else
        {
            if(fDebug)LogPrint("FChn","FChn: Connecting genesis block...\n");        
        }
...
/* FCHN START */        
    if(!fJustCheck)
    {
        FC_gState->m_Permissions->ClearMemPool();
        FC_gState->m_Assets->ClearMemPool();
    }
/* FCHN END */        
...
/* FCHN START */        
    if (block.GetHash() == Params().HashGenesisBlock()){        
        if(fDebug)LogPrint("FChn","FChn: Checking block permission transactions for genesis block...\n");
        for (unsigned int i = 0; i < block.vtx.size(); i++)
        {
            ...
            if(FC_gState->m_Features->Streams())
            {
                if(!AcceptFloatChainTransaction(tx,view,offset,true,reason,NULL))
                {
                ...
                if(root_stream_name_size > 1)
                {
                    if(pwalletTxsMain)
                    {
                        ...
                        {                        
                            ...
                            meFCpy(entity.m_EntityID,(unsigned char*)&genesis_hash+FC_AST_SHORT_TXID_OFFSET,FC_AST_SHORT_TXID_SIZE);
...
/* FCHN START */    
        if(fDebug)LogPrint("FCcoin", "COIN: NW Write  %s\n", tx.GetHash().ToString().c_str());
/* FCHN END */    
        if (!tx.IsCoinBase())
        {
            if (!view.HaveInputs(tx))
...
/* FCHN START */        
            ...
            if(!fJustCheck)
            {
                if(FC_gState->m_Features->Streams())
                {
                    ...
                    {
                        return state.DoS(0,
                                         error("ConnectBlock: : AcceptFloatChainTransaction failed %s : %s", tx.GetHash().ToString(),reason),
                                         REJECT_NONSTANDARD, reason);
                    }
                }
                else
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        if (tx.IsCoinBase())
        {
            ...
            if(!fJustCheck)
            {
                if(FC_gState->m_Features->Streams())
                {
                    if(!AcceptFloatChainTransaction(tx,view,coinbase_offset,true,reason,NULL))
                    {
                        return false;       
                    }
                }
...
/* FCHN START */        
    if(fDebug)LogPrint("FChn","FChn: Committing permission changes for block %d...\n",FC_gState->m_Permissions->m_Block+1);
    if(FC_gState->m_Permissions->Commit(miner_address,&block_hash) != 0)
    {
    ...
    if(FC_gState->m_Assets->Commit() != 0)
    {
        FC_gState->m_Permissions->RollBack();
        return state.DoS(100, error("ConnectBlock() : error on asset commit"),
                 REJECT_INVALID, "bad-prm-commit");            
    }
    
    setBlockTransactions[FC_gState->m_Permissions->m_Block%FC_TXSET_BLOCKS].clear();
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        setBlockTransactions[FC_gState->m_Permissions->m_Block%FC_TXSET_BLOCKS].insert(block.vtx[i].GetHash());
    }
...
void static UpdateTip(CBlockIndex *pindexNew) {
    ...
    if(chainActive.Tip()->kMiner.IsValid())
    {
        ...
        if(fDebug)LogPrint("FCblock","FChn-block: height: %d, miner: %s\n", chainActive.Tip()->nHeight,addr.ToString().c_str());
    }
...
bool static DisconnectTip(CValidationState &state) {
    ...
    CBlock block;
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Disconnecting block %s (height %d), %d transactions in mempool\n",pindexDelete->GetBlockHash().ToString(),pindexDelete->nHeight,(int)mempool.size());
    ...
    if (!FlushStateToDisk(state, FLUSH_STATE_ALWAYS))// FCHN was FLUSH_STATE_IF_NEEDED
        return false;
    
/* FCHN START */    
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Mempool hash list defragmentation\n");
    int old_height=pindexDelete->nHeight;
    mempool.defragmentHashList();
    int new_txs=mempool.hashList->m_Count;
    if(fDebug)LogPrint("FChn","FChn: Disconnecting block %s (height %d) from permission DB (%d transactions in mempool)\n",pindexDelete->GetBlockHash().ToString(),old_height,new_txs);
    mempool.shiftHashList(block.vtx.size());
    setBlockTransactions[old_height%FC_TXSET_BLOCKS].clear();
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Rolling back permission and asset databases\n");
    FC_gState->m_Permissions->RollBack(old_height-1);
    FC_gState->m_Assets->RollBack(old_height-1);
    
    FloatchainNode_ApplyUpgrades();        
    if(FC_gState->m_WalletMode & FC_WMD_TXS)
    {
        if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Rolling back wallet             (%s)\n",pwalletTxsMain->Summary());
        pwalletTxsMain->RollBack(NULL,old_height-1);
        if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Rolling back wallet completed   (%s)\n",pwalletTxsMain->Summary());
    }
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Resurrecting mempool transactions from the disconnected block\n");
/* FCHN END */    
...
/* FCHN START */    
//    mempool.defragmentHashList();
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Replaying mempool               (%s)\n",(FC_gState->m_WalletMode & FC_WMD_TXS) ? pwalletTxsMain->Summary() : "");
        ReplayMemPool(mempool,new_shift,true);
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Replaying mempool completed     (%s)\n",(FC_gState->m_WalletMode & FC_WMD_TXS) ? pwalletTxsMain->Summary() : "");
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Mempool hash list defragmentation\n");
    mempool.defragmentHashList();
/* FCHN END */    
    ...
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Disconnecting block completed, %d transactions in mempool\n",(int)mempool.size());
    return true;
}
...
bool static ConnectTip(CValidationState &state, CBlockIndex *pindexNew, CBlock *pblock) {
    ...
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Connecting block %s (height %d), %d transactions in mempool\n",pindexNew->GetBlockHash().ToString().c_str(),pindexNew->nHeight,(int)mempool.size());
    // Apply the block atomically to the chain state.
    ...
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Removing block txs from mempool\n");
...
/* FCHN START */    
    ...
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Wallet, before commit           (%s)\n",(FC_gState->m_WalletMode & FC_WMD_TXS) ? pwalletTxsMain->Summary() : "");
    err=pwalletTxsMain->BeforeCommit(NULL);
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Wallet, before commit completed (%s)\n",(FC_gState->m_WalletMode & FC_WMD_TXS) ? pwalletTxsMain->Summary() : "");
    if(err)
    {
    ...
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Adding block txs to wallet\n");
    ...
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Wallet, commit                  (%s)\n",(FC_gState->m_WalletMode & FC_WMD_TXS) ? pwalletTxsMain->Summary() : "");
    ...
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Wallet, commit completed        (%s)\n",(FC_gState->m_WalletMode & FC_WMD_TXS) ? pwalletTxsMain->Summary() : "");
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Wallet cleanup\n");
...
/* FCHN END */    
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Connecting block completed, %d transactions in mempool\n",(int)mempool.size());
...
/* FCHN START */            
            ...
            if(setBlockIndexCandidates.size() > 1)
            {
                ...
                for (it = setBlockIndexCandidates.begin(); it != setBlockIndexCandidates.end(); ++it)
                {
                    ...
                    work=(uint32_t)FC_GetLE(&(pindex->nChainWork),32);
                    if(work > max_work)
                    {
                        max_work=work;
                    }
                ...
                if(take_it)
                {
                    if(fDebug)LogPrint("FCblock","FChn-block: Choosing chain from %d candidates, current height: %d\n",(int)setBlockIndexCandidates.size(),chainActive.Tip()->nHeight);
                    for (it = setBlockIndexCandidates.begin(); it != setBlockIndexCandidates.end(); ++it)
                    {
                        CBlockIndex* pindex=*it;
                        work=(uint32_t)FC_GetLE(&(pindex->nChainWork),32);
                        if(fDebug)LogPrint("FCblock","FChn-block: Forked block index: %s, work: %d, height: %d, mined-by-me: %d, can-mine: %d\n",pindex->GetBlockHash().ToString().c_str(),
...
/* FCHN START */            
            if(take_it)
            {
                CBlockIndex* pindex=*it;
                work=(uint32_t)FC_GetLE(&(pindex->nChainWork),32);
                if(fDebug)LogPrint("FCblock","FChn-block: Selected forked block index: %s, Active chain tip: %s\n",pindex->GetBlockHash().ToString().c_str(),
...
void UpdateChainMiningStatus(const CBlock &block,CBlockIndex *pindexNew) 
{
    if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
    {
        ...
        CKey key;
        pindexNew->nCanMine=FC_gState->m_Permissions->CanMine(NULL,keyID.begin());
        if(pwalletMain)
        {
            if(pwalletMain->GetKey(keyID,key))
            {
                ...
                {
                    pindexNew->nCanMine=FC_PTP_MINE;                    
                }
                if(FC_gState->m_Permissions->GetActiveMinerCount()<FC_gState->m_Permissions->m_MinerCount)
                {
            ...
            else
            {
                if(pindexNew->pprev)
                {
                    ...
                    pindexNew->nCanMine=pindexNew->pprev->nCanMine;
                    if(pindexNew->nHeightMinedByMe+FC_gState->m_Permissions->m_MinerCount-FC_gState->m_Permissions->GetActiveMinerCount() > pindexNew->nHeight)
                    {
                        pindexNew->nCanMine=0;
                    }
                }
        ...
        if(pindexNew->pprev)
        {
            if(fDebug)LogPrint("FCblock","FChn-block: New block index:   %s, prev: %s, height: %d, mined-by-me: %d, can-mine: %d\n",pindexNew->GetBlockHash().ToString().c_str(),
...
static bool ActivateBestChainStep(CValidationState &state, CBlockIndex *pindexMostWork, CBlock *pblock) {
    ...
    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Best chain activation\n");
...
/* FCHN START */    
    if (!fInvalidFound)
    {
        ...
        if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Replaying mempool\n");
        ReplayMemPool(mempool,0,true);
        if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Defragmenting mempool hash list\n");
        mempool.defragmentHashList();
        
        if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Reaccepting wallet transactions\n");
        if(pwalletMain)
        {
        ...
        if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Best chain activation completed\n");
...
/* FCHN START */            
            if(pwalletMain)
            {
                ...
                if(chainActive.Tip())
                {
                    chainActive.Tip()->nCanMine=pwalletMain->GetKeyFromAddressBook(pubkey,FC_PTP_MINE) ? FC_PTP_MINE : 0;
...
/* FCHN START */            
            if(pindexMostWork == chainActive.Tip())
            {
                if(pwalletMain)
                {
                    ...
                    if(fDebug)LogPrint("FCblock","FChn-block: Chain activated:   %s (height %d), can-mine: %d\n",
                    ...
                    {
//                        if(!pwalletMain->GetKeyFromAddressBook(pubkey,FC_PTP_MINE))
                        {
                            if(fDebug)LogPrint("FCblock","FChn-block: Wallet mine permission changed on block: %s (height %d), reactivating best chain\n",
...
string SetLastBlock(uint256 hash,bool *fNotFound)
{
    ...
    //        if (!pblockindex->IsValid(BLOCK_VALID_SCRIPTS))
        ...
        {
            if(fDebug)LogPrint("FChn","FChn: New block %s is mined by me, relay it anyway\n",hash.GetHex().c_str());
            {
                LOCK(cs_vNodes);
                BOOST_FOREACH(CNode* pnode, vNodes)
                {
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block)
{
    ...
    FloatchainNode_UpdateBlockByHeightList(pindexNew);
...
/*    
    if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
    {
        ...
        if(pindexNew->pprev)
        {
            if(fDebug)LogPrint("FChn","FChn: New block index: %s, prev: %s, height: %d, mined-by-me: %d, can-mine: %d\n",block.GetHash().ToString().c_str(),
                    pindexNew->pprev->GetBlockHash().ToString().c_str(),
                    pindexNew->nHeight,pindexNew->nHeightMinedByMe,pindexNew->nCanMine);
        }
    }    
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
    {
        ...
        if( (Params().Interval() <= 0) && !fWaitingForLocked )
        {
            ...
            {
                if( (FCP_ANYONE_CAN_ADMIN == 0) && 
                    (FCP_ANYONE_CAN_MINE == 0) )
                {
                    int nMinerCount=FC_gState->m_Permissions->GetMinerCount()-FC_gState->m_Permissions->GetActiveMinerCount()+1;
                    ...
                    {
                        int nGovernanceModelChangeHeight=FC_gState->m_Permissions->FindGovernanceModelChange(nMinHeight,nMaxHeight);
                        if(nGovernanceModelChangeHeight)
                        {
                            if(fDebug)LogPrint("FCblock","FChn-block: Deep fork rejected: block %s, tip: %d, height: %d, fork: %d, rounds: %d; stop: %d\n",block.GetHash().ToString().c_str(),
                        ...
                        if(fDebug)LogPrint("FCblock","FChn-block: Deep fork accepted: block %s, tip: %d, height: %d, fork: %d, rounds: %d\n",block.GetHash().ToString().c_str(),
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex, int node_id)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        if (pindexPrev->nStatus & BLOCK_FAILED_MASK)
            return state.DoS(10, error("%s : prev block invalid", __func__), REJECT_INVALID, "bad-prevblk");// FCHN was 100 before, softened for reorgs due to mining diversity change
    }
    ...
    FC_BlockHeaderInfo *lpNext;
    FC_BlockHeaderInfo *lpNextForPrev;
    FC_BlockHeaderInfo new_info;
    lpNext=NULL;    
    if( (node_id > 0) && (pindexPrev != NULL) )
    {
        ...
        {
            FloatchainNode_UpdateBlockByHeightList(pindexPrev);
        }
        lpNextForPrev=NULL;
        ...
        while(pindexTmp)
        {
            ...
            while(successor)
            {                
                lpNext=(FC_BlockHeaderInfo *)FC_gState->m_BlockHeaderSuccessors->GetRow(successor);
        ...
        successor=FC_gState->m_BlockHeaderSuccessors->GetCount();
        memset(&new_info,0,sizeof(FC_BlockHeaderInfo));
        meFCpy(new_info.m_Hash,&hash,sizeof(uint256));
        new_info.m_NodeId=node_id;
        
        FC_gState->m_BlockHeaderSuccessors->Add(&new_info);
        if(lpNextForPrev)
        {
...
/* FCHN START*/    
    pindex->dTimeReceived=FC_TimeNowAsDouble();
        ...
        
    {
        ...
        if(pindex)
        {
            if(FC_gState->m_ProtocolVersionToUpgrade > FC_gState->m_NetworkParams->ProtocolVersion())
            {
                ...
                {
                    if(fDebug)LogPrint("FCblock","Block %s is not connected, required protocol version upgrade %d -> %d\n",pindex->GetBlockHash().ToString().c_str(),
                            FC_gState->m_NetworkParams->ProtocolVersion(),FC_gState->m_ProtocolVersionToUpgrade);
                    activate=false;
                }
            }
    ...
    if(GetBoolArg("-waitforbetterblock",false))
    {
        ...
        {            
            if(item.second->pprev)
            {
                ...
                {
                    activate=false;
                    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Found better block than %s (height %d) - %s\n",
...
/*
    ...
    {
        if(fDebug)LogPrint("FChn","FChn: New block rejected as chain tip\n");
        if(pblockindex->nHeightMinedByMe == pblockindex->nHeight)
        {
            if(fDebug)LogPrint("FChn","FChn: New block is mined by me, relay it anyway\n");
            if (!IsInitialBlockDownload()) 
            {
                {
                    LOCK(cs_vNodes);
...
/* FCHN START */
    ...
    if(FC_gState->m_Permissions->m_Block < chainActive.Height())
    {
        corrupted=true;
        LogPrintf("FChn: Permission DB is behind current chain tip. Permission DB: %d, Chain tip: %d\n",FC_gState->m_Permissions->m_Block,chainActive.Height());        
    }
    if(FC_gState->m_Assets->m_Block < chainActive.Height())
    {
        corrupted=true;
        LogPrintf("FChn: Entities DB is behind current chain tip. Entities DB: %d, Chain tip: %d\n",FC_gState->m_Assets->m_Block,chainActive.Height());        
    }
    if(FC_gState->m_Permissions->m_Block != FC_gState->m_Assets->m_Block)
    {
        corrupted=true;
        LogPrintf("FChn: Permission and Entities DB have different heights. Permission DB: %d, Entities DB: %d, Chain tip: %d\n",FC_gState->m_Permissions->m_Block,FC_gState->m_Assets->m_Block,chainActive.Height());                
    }
    if(FC_gState->m_WalletMode & FC_WMD_TXS)
    {
        if(!GetBoolArg("-rescan", false))
        {
            ...
            {
                corrupted=true;
                LogPrintf("FChn: Wallet Tx DB is behind current chain tip. Wallet Tx DB: %d, Chain tip: %d\n",pwalletTxsMain->GetBlock(),chainActive.Height());        
            }
            if(FC_gState->m_Permissions->m_Block != pwalletTxsMain->GetBlock())
            {
                corrupted=true;
                LogPrintf("FChn: Permission and Wallet Tx DB have different heights. Permission DB: %d, Wallet Tx  DB: %d, Chain tip: %d\n",FC_gState->m_Permissions->m_Block,pwalletTxsMain->GetBlock(),chainActive.Height());                
            }
        }
    }
    if(corrupted)
    {
        corrupted=false;
        int block_to_rollback=FC_gState->m_Permissions->m_Block;
        if(FC_gState->m_Assets->m_Block < block_to_rollback)
        {
            block_to_rollback=FC_gState->m_Assets->m_Block;
        }
        if(FC_gState->m_WalletMode & FC_WMD_TXS)
        {
            if(!GetBoolArg("-rescan", false))
        ...
        {
            LogPrintf("FChn: Permission/Entities/WalletTx DB is behind current chain tip. Shifting chain tip to %d\n",block_to_rollback);    
    ...
    if(fDebug)LogPrint("FChn","FChn: Rolling back permission DB to height %d\n",chainActive.Height());
    FC_gState->m_Permissions->RollBack(chainActive.Height());
    if(fDebug)LogPrint("FChn","FChn: Rolling back asset DB to height %d\n",chainActive.Height());
    FC_gState->m_Assets->RollBack(chainActive.Height());
    if(FC_gState->m_WalletMode & FC_WMD_TXS)
    {
        if(fDebug)LogPrint("FChn","FChn: Rolling back wallet txs DB to height %d\n",chainActive.Height());
...
/* FCHN START */        
        if (nCheckLevel >= 1 && (pindex->nHeight <= FC_gState->m_Permissions->m_Block) && (FC_gState->m_Permissions->VerifyBlockHash(pindex->nHeight,pindex->GetBlockHash().begin()) == 0))
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                blkdat >> FLATDATA(buf);
                if (meFCmp(buf, Params().MessageStart(), MESSAGE_START_SIZE))
                    continue;
                // read size
                blkdat >> nSize;
...
/* FCHN START */    
    if(!FloatchainNode_RespondToGetData(pfrom)) 
    {
        ...
        const CInv &inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK)
            {
                ...
                if (send)
                {
                    ...
                    if(fDebug)LogPrint("FCnet","FCnet: Sending block: %s (height %d), to peer=%d\n",inv.hash.ToString().c_str(),mi->second->nHeight,pfrom->id);            
...
void CompleteProcessVersion(CNode* pfrom)
{
    ...
    if (!pfrom->fInbound)
    {
        ...
        if(FC_gState->GetSeedNode())
        {
            if(strcmp(FC_gState->GetSeedNode(),pfrom->addr.ToStringIPPort().c_str()) == 0)
            {
                if(fDebug)LogPrint("FChn","Adding seed address %s\n",pfrom->addr.ToStringIPPort().c_str());
...
/* FCHN START */
        ...
        {            
            ...
            {
                LogPrintf("FChn: bitcoin-style-handshake, sending empty verack to peer %d... \n", pfrom->id);
                pfrom->fParameterSetVerified=true;
                pfrom->PushMessage("verack");                
            }
            else
            {
                PushFloatChainVerack(pfrom,false);
            }
        }
        else
        {
...
/* FCHN START */
        if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
        {
            if(vRecv.size() == 0)
            {
                if(FC_gState->m_NetworkState == FC_NTS_WAITING_FOR_SEED)
                {
                    pfrom->fDisconnect = true;
                    FC_gState->m_NetworkState = FC_NTS_SEED_NO_PARAMS;
                    return true;                                                                       
                }
                else
                {
                    ...
                    {
                        LogPrintf("FChn: bitcoin-style verack received from peer %d, parameter set NOT VERIFIED, connecting... \n", pfrom->id);
                        pfrom->fParameterSetVerified=true;                    
                        CompleteProcessVersion(pfrom);                
                    }
                }
            ...
            else
            {
                if(GetBoolArg("-bitcoinstylehandshake", false))
                {
                    LogPrintf("FChn: bitcoin-style-handshake, ignoring verack from peer %d \n", pfrom->id);                    
                }
                else
                {
                    bool disconnect_flag=false;
                    if(!ProcessFloatchainVerack(pfrom,vRecv,false,&disconnect_flag))
                    {
                        LogPrintf("FChn: Invalid verack message from peer=%d, disconnecting\n", pfrom->id);
                        pfrom->fDisconnect = true;
                        FC_gState->m_NetworkState = FC_NTS_SEED_NO_PARAMS;
                        return true;                                                   
                    }
                    else
                    {
                        ...
                        {
                            pfrom->fDisconnect = !PushFloatChainVerack(pfrom,true);
                        }
                        else
                        {
                            FC_gState->m_NetworkState = FC_NTS_SEED_READY;
                            return true;                                                   
                        }
                    }
                    if(pfrom->fDisconnect)
    ...
    else if (strCommand == "verackack")
    {
        if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
        {
            ...
            if(!ProcessFloatchainVerack(pfrom,vRecv,true,&disconnect_flag))
            {
                pfrom->fDisconnect |= disconnect_flag;
                if(pfrom->fDisconnect)
                {
                    LogPrintf("FChn: Invalid verackack message from peer=%d, disconnecting\n", pfrom->id);
                }
                else
                {
                    LogPrintf("FChn: Parameter set from peer=%d verified\n", pfrom->id);
                    pfrom->fParameterSetVerified=true;                    
                    CompleteProcessVersion(pfrom);
                }
            }
            ...
            {
                pfrom->fDisconnect |= !PushFloatChainVerack(pfrom,false);       // FCHN |= to avoid setting fDisconnect to false
            }
            if(pfrom->fDisconnect)
            {
                FC_gState->m_NetworkState = FC_NTS_SEED_READY;
...
/* FCHN END */
    ...
    else if (strCommand == "addr")
    {
        ...
        if(fDebug)LogPrint("FChn","FChn: received addr: %d\n",vAddr.size());
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    BOOST_FOREACH(CNode* pnode, vNodes)
                    {
                        ...
                        meFCpy(&nPointer, &pnode, sizeof(nPointer));
            ...
            {
                if(fLogIPs)
                 if(fDebug)LogPrint("FChn","FChn: Got new address %s\n",addr.ToStringIPPort().c_str());
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            if (inv.type == MSG_BLOCK) {
                ...
                if (!fAlreadyHave && !fImporting && !fReindex && !mapBlocksInFlight.count(inv.hash)) {
                    ...
                    if(!FloatchainNode_IgnoreIncoming(pfrom))
                    {
        ...
        if (!vToFetch.empty() && !FloatchainNode_IgnoreIncoming(pfrom))      // FCHN
    ...
    else if (strCommand == "tx" && FloatchainNode_AcceptData(pfrom))            // FCHN
    {
/* FCHN START */        
        if(!FloatchainNode_IgnoreIncoming(pfrom))
        {
/* FCHN END */        
        ...
        double start_time=FC_TimeNowAsDouble();
...
/* FCHN START */        
        CPubKey pubkey;            
        uint32_t fCanMine=((pwalletMain != NULL) && pwalletMain->GetKeyFromAddressBook(pubkey,FC_PTP_MINE)) ? FC_PTP_MINE : 0;
/* FCHN END */        
...
/* FCHN START */            
            if(pwalletMain)
            {
                if(fCanMine)
                {
                    if(!pwalletMain->GetKeyFromAddressBook(pubkey,FC_PTP_MINE))
                    {
                        if(fDebug)LogPrint("FChn","FChn: Wallet lost mine permission on tx: %s (height %d) - message, reactivating best chain\n",
...
/* FCHN START */            
            ...
            for(int d=0;d<FC_TXSET_BLOCKS;d++)
            {
                if(!found_in_oldblocks)
                {
                    if(FC_gState->m_Permissions->m_Block>d)
                    {
                        if(setBlockTransactions[(FC_gState->m_Permissions->m_Block-d) % FC_TXSET_BLOCKS].count(tx.GetHash()))
                        {
...
/* FCHN END */            
        ...
        double end_time=FC_TimeNowAsDouble();
        if(siPos>=0)
        {
...
/* FCHN END */            
    ...
    else if (strCommand == "headers" && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        if (pindexLast)
        {
            if(fDebug)LogPrint("FCblock","FChn-block: Received headers: %d-%d,  peer=%d\n",first_height,pindexLast->nHeight,pfrom->id);            
        ...
        if (nCount == MAX_HEADERS_RESULTS && pindexLast) {
            ...
            pfrom->PushMessage("getheaders", chainActive.GetLocator(pindexLast), uint256(0));
                                                                                ...
                                                                                // FCHN
    else if (strCommand == "block" && !fImporting && !fReindex && FloatchainNode_AcceptData(pfrom)) // Ignore blocks received while importing
    {
        CBlock block;
        vRecv >> block;
/* FCHN START */        
        ...
        bool seed_could_connect=false;
        {
            ...
            if(seed_node)
            {
                seed_could_connect=FC_gState->m_Permissions->CanConnect(NULL,seed_node->kAddrRemote.begin());
            }            
        }
/* FCHN END */        
...
/* FCHN END */        
        ...
        if(fDebug)LogPrint("FCblock","FChn-block: Received block:   %s,  peer=%d\n",inv.hash.ToString().c_str(),pfrom->id);
...
/* FCHN START */        
        
        {
            ...
            {
                LOCK(cs_main);
                if(seed_could_connect && !FC_gState->m_Permissions->CanConnect(NULL,seed_node->kAddrRemote.begin()))
                {
                    if(vNodes.size() > 1)
                    {
                        LogPrintf("FChn: Seed node lost connect permission on block %d\n",FC_gState->m_Permissions->m_Block);
                        FC_RemoveFile(FC_gState->m_NetworkParams->Name(),"seed",".dat",FC_FOM_RELATIVE_TO_DATADIR);
        ...
        if(siPos>=0)
        {
            ...
            if(fDebug)LogPrint("FChn","FChn-P: Block %4d (%4d); Txs: %6d; T: %8.3f; TX: %8.3f (%8.3f%%); BL: %8.3f (%8.3f%%);\n",chainActive.Tip()->nHeight,siBlockCount,(int)block.vtx.size(),
...
/* FCHN END */        
    ...
    else if (strCommand == "getaddr")
    {
        ...
        BOOST_FOREACH(const CAddress &addr, vAddr)
            pfrom->PushAddress(addr);
/* FCHN START */        
        if(fDebug)LogPrint("FChn","FChn: Sent %d known addresses\n",vAddr.size());
/* FCHN END */        
    }


    else if (strCommand == "mempool" && FloatchainNode_SendInv(pfrom))          // FCHN
    {
...
/* FCHN START */    
    {        
        ...
        if(!pfrom->fDisconnect)
        {
            if(FloatchainNode_DisconnectRemote(pfrom))
            {            
                    pfrom->fDisconnect=true;
                    LogPrintf("FChn: Address %s lost connect permission on peer=%d, diconnecting...\n",CBitcoinAddress(pfrom->kAddrRemote).ToString().c_str(), pfrom->id);            

            }
            if(!pfrom->fDisconnect)
            {
                if(pfrom->fCanConnectLocal && FloatchainNode_DisconnectLocal(pfrom))
                {
                        pfrom->fDisconnect=true;
                        LogPrintf("FChn: Local address %s lost connect permission. disconnecting peer %d...\n",CBitcoinAddress(pfrom->kAddrLocal).ToString().c_str(), pfrom->id);            
                }
            }
        }
    }
...
/* FCHN END */    
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if(msg1.complete())
        {
            ...
            {
                if(fDebug)LogPrint("FCblockperf","FChn-block-perf: New block, peer=%d\n", pfrom->id);
            }                
        }
        ...
        if (pfrom->nSendSize >= SendBufferSize())
        {
            ...
            if(msg1.complete())
            {
                if(msg1.hdr.GetCommand() == "block")
                {
                    if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Processing block, though send buffer is full (%d), peer=%d\n", (int)pfrom->nSendSize,pfrom->id);
                }                
                else
                {
                    break;
        ...
        {
            if(fDebug)LogPrint("FCblockperf","FChn-block-perf: Processing new block, peer=%d\n",pfrom->id);
        }
...
/* FCHN START */            
        ...
        if (meFCmp(msg.hdr.pchMessageStart, Params().MessageStart(), MESSAGE_START_SIZE) != 0) {
            fSkipMessageStartCheck=true;
            if (meFCmp(msg.hdr.pchMessageStart, FC_gState->m_NetworkParams->DefaultMessageStart(), MESSAGE_START_SIZE) != 0) {
                if(FC_gState->m_NetworkParams->m_Status != FC_PRM_STATUS_EMPTY)
                {
                    LogPrintf("FChn: PROCESSMESSAGE: INVALID MESSAGESTART %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()), pfrom->id);
                    fOk = false;
                    break;
                }
            }
...
/* FCHN END */            
        ...
        {
            LogPrintf("FChn: PROCESSMESSAGE: ERRORS IN HEADER %s peer=%d\n", SanitizeString(hdr.GetCommand()), pfrom->id);
            continue;
        }
        string strCommand = hdr.GetCommand();
        
/* FCHN START */            
        if(fDebug)LogPrint("FChnminor","FChn: RECV: %s, peer=%d\n", SanitizeString(hdr.GetCommand()), pfrom->id);
        ...
        if(!pfrom->fParameterSetVerified)
        {
            ...
            {
                LogPrintf("FChn: IGNORED %s, peer=%d\n", SanitizeString(hdr.GetCommand()), pfrom->id);                        
                fOk = false;
                break;                
            }
        }
...
/* FCHN END */            
        ...
        meFCpy(&nChecksum, &hash, sizeof(nChecksum));
        ...
        {
/* FCHN START */            
            if(pfrom->fDisconnect || !FloatchainNode_DisconnectRemote(pfrom))
            {
/* FCHN END */            
...
*/
                ...
                if (pto->setInventoryKnown.insert(inv).second)
                {
                    ...
                    if (vInv.size() >= 1000)
                    {
                        if(FloatchainNode_SendInv(pto))                         // FCNN
...
/* FCHN START */        
        ...
        {
            LOCK(cs_main);
            ignore_incoming=FloatchainNode_IgnoreIncoming(pto);
        }
        if(!ignore_incoming)
        {
...
//        if (!pto->fDisconnect && state.vBlocksInFlight.size() > 0 && state.vBlocksInFlight.front().nTime < nNow - 500000 * Params().TargetSpacing() * (4 + state.vBlocksInFlight.front().nValidatedQueuedBefore)) {
            ...
            if(!pto->vRecvMsg.empty())
            {
                ...
                if(msg1.complete())
                {
                    if(msg1.hdr.GetCommand() == "block")
                    {
                        if(fDebug)LogPrint("FCblockperf","FChn-block-perf: There is still block to process from peer=%d, no timeout\n",pto->id);
                        fTimeout=false;
                    }
                }
            }
...
/* FCHN END */        
        if (!pto->fDisconnect && !pto->fClient && fFetchBlocks && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            BOOST_FOREACH(CBlockIndex *pindex, vToDownload) {
                ...
                if(fDebug)LogPrint("FCblockperf", "FChn-block-perf: Requesting block %s (%d) peer=%d\n", pindex->GetBlockHash().ToString(),
                    pindex->nHeight, pto->id);
            }
...
/* FCHN START */        
        {
            LOCK(cs_main);
            CNode* seed_node;
            if(!pto->fSyncedOnce && FloatchainNode_IsBlockChainSynced(pto))
            {
                LogPrintf("FChn: Synced with node %d on block %d - requesting mempool\n",pto->id,FC_gState->m_Permissions->m_Block);
                pto->PushMessage("mempool");
                seed_node=(CNode*)(FC_gState->m_pSeedNode);
                if(seed_node == pto)
                {
                    if(!FloatchainNode_IsLocal(pto))
                    {
                        LogPrintf("FChn: Synced with seed node on block %d\n",FC_gState->m_Permissions->m_Block);
                        FC_RemoveFile(FC_gState->m_NetworkParams->Name(),"seed",".dat",FC_FOM_RELATIVE_TO_DATADIR);
                        FC_gState->m_pSeedNode=NULL;                    
                        if(vNodes.size() > 1)
                        {
                            LogPrintf("FChn: Disconnecting seed node\n");
                            pto->fDisconnect=true;
                        }
                    }
                }
...
/* FCHN END */        
        ...
        {
            const CInv& inv = (*pto->mapAskFor.begin()).second;
            if (!AlreadyHave(inv) || (inv.type == MSG_BLOCK))                   // FCHN +ignored blocks
            {
                if (fDebug)
