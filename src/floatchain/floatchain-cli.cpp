...
/* FCHN START */    
    ...
    strUsage += "  -saveclilog=<n>          " + _("If <n>=0 floatchain-cli history is not saved, default 1") + "\n";
/*    
...
//    ParseParameters(argc, argv);
      if (FC_gState->m_Params->HasOption("-?") || 
        ...
        FC_gState->m_Params->m_NumArguments<minargs)
      {
        fprintf(stdout,"\nFloatChain Core RPC client %s\n\n",FC_gState->GetFullVersion());
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  floatchain-cli <blockchain-name> [options] <command> [params]  " + _("Send command to FloatChain Core") + "\n" +
                  "  floatchain-cli <blockchain-name> [options] help                " + _("List commands") + "\n" +
                  "  floatchain-cli <blockchain-name> [options] help <command>      " + _("Get help for a command") + "\n";

            strUsage += "\n" + HelpMessageCli();                                // FCHN-TODO Edit help message
        }
...
*/
    int RPCPort=FC_DEFAULT_RPC_PORT;        
    int read_err;
    err=FC_ERR_NOERROR;
    
    boost::filesystem::path path=boost::filesystem::path(string(FC_gState->m_Params->DataDir(0,0)));
    if(boost::filesystem::is_directory(path))
    {
        path=boost::filesystem::path(string(FC_gState->m_Params->DataDir(1,0)));
    
        if (!boost::filesystem::is_directory(path)) 
        {
            err=FC_gState->m_Params->ReadConfig(NULL);
        }
        else
        {
            read_err=FC_gState->m_NetworkParams->Read(FC_gState->m_Params->NetworkName());
            if(read_err)
            {
                err=FC_gState->m_Params->ReadConfig(NULL);     
                ...
                {
                    if(read_err != FC_ERR_FILE_READ_ERROR)
                    {
            ...
            else
            {
                ...
                RPCPort=FC_gState->m_NetworkParams->GetInt64Param("defaultrpcport");
            }
        }
    }    
    ...
    RPCPort=FC_gState->m_Params->GetOption("-rpcport",RPCPort);
    
    SelectFloatChainBaseParams(FC_gState->m_Params->NetworkName(),RPCPort);
    
    // Check for -testnet or -regtest parameter (BaseParams() calls are only valid after this clause)
/*    
    if (!SelectBaseParamsFroFCommandLine()) {
        ...
        throw runtime_error(strprintf(
            _("No credentials found for chain \"%s\"\n\n"
              ...
              "If the file does not exist, create it with owner-readable-only file permissions."),
                FC_gState->m_Params->NetworkName(),FC_gState->m_Params->DataDir(1,0)));
...
int main(int argc, char* argv[])
{
    ...
    {
        path_cli_log = string(FC_gState->m_Params->DataDir(0,0));
    }
    path_cli_log /= string(".cli_history");
    boost::filesystem::create_directories(path_cli_log);
    path_cli_log /= string(FC_gState->m_Params->NetworkName() + string(".log"));
    
 #ifndef WIN32   
    if(FC_gState->m_Params->m_NumArguments == 1)                                // Interactive mode
    {
        fprintf(stdout,"\nFloatChain Core RPC client %s\n\n",FC_gState->GetFullVersion());
        if (mapArgs["-rpcuser"] == "" && mapArgs["-rpcpassword"] == "")
        {
        ...
        FC_TerminalInput *term=new FC_TerminalInput;
        term->SetPrompt(FC_gState->m_Params->NetworkName());
        char *command;
        char *commandEnd;
        
        char *dest;
        dest=NULL;
        dest=(char*)FC_New(FC_DCT_TERM_BUFFER_SIZE+16384);
        char *argv_p[1024];
        ...
        while(!exitnow && ((command=term->GetLine()) != NULL))
        {
            ...
            else
            {
                ...
                offset=0;
                argc_p=0;
                strcpy(dest+offset,"floatchain-cli");
                argv_p[argc_p]=dest+offset;
                argc_p++;
                offset+=strlen(dest+offset)+1;
                strcpy(dest+offset,FC_gState->m_Params->NetworkName());
                argv_p[argc_p]=dest+offset;
                argc_p++;
                offset+=strlen(dest+offset)+1;
                shift=FC_StringToArg(command,dest+offset);
                while(shift>0)
                {                    
                    argv_p[argc_p]=dest+offset;
                    argc_p++;
                ...
                else
                {
                    if(argc_p>2)
                    {
                        try {
                            ...
                            {
                                FC_SaveCliCommandToLog(path_cli_log.string().c_str(), argc_p, argv_p);
                            }
    ...
    try {
        FC_SaveCliCommandToLog(path_cli_log.string().c_str(), argc, argv);
