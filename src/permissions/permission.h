// Copyright (c) 2014-2017 Coin Sciences Ltd
// FloatChain code distributed under the GPLv3 license, see COPYING file.

#ifndef FLOATCHAIN_PERMISSION_H
#define	FLOATCHAIN_PERMISSION_H

#include "utils/declare.h"
#include "utils/dbwrapper.h"

#define FC_PTP_CONNECT          0x00000001
#define FC_PTP_SEND             0x00000002
#define FC_PTP_RECEIVE          0x00000004
#define FC_PTP_WRITE            0x00000008
#define FC_PTP_ISSUE            0x00000010
#define FC_PTP_CREATE           0x00000020
#define FC_PTP_MINE             0x00000100
#define FC_PTP_ADMIN            0x00001000
#define FC_PTP_ACTIVATE         0x00002000
#define FC_PTP_UPGRADE          0x00010000
#define FC_PTP_BLOCK_MINER      0x01000000
#define FC_PTP_BLOCK_INDEX      0x02000000
#define FC_PTP_ALL              0x00FFFFFF
#define FC_PTP_GLOBAL_ALL       0x00003137

#define FC_PTP_CACHED_SCRIPT_REQUIRED      0x02000000                           // Special temporary value for coin selection


#define FC_PFL_NONE             0x00000000
#define FC_PFL_IS_SCRIPTHASH    0x00000001
#define FC_PFL_ENTITY_GENESIS   0x00000002
#define FC_PFL_HAVE_PENDING     0x01000000

#define FC_PFB_NONE               0x00000000
#define FC_PFB_MINED_BEFORE       0x00000001                                    // Idential with FoundInDB value of normal row
#define FC_PFB_GOVERNANCE_CHANGE  0x00000002

#define FC_PLS_SIZE_ENTITY            32
#define FC_PLS_SIZE_ADDRESS           20
#define FC_PLS_SIZE_HASH              32
#define FC_PLS_SIZE_UPGRADE           16
#define FC_PLS_SIZE_OFFSETS_PER_ROW    6

#define FC_PPL_REPLAY             0x00000001    
#define FC_PPL_ADMINMINERGRANT    0x00000002    

#define FC_PSE_UPGRADE                0x01                                      
#define FC_PSE_ADMINMINERLIST         0x02                                      



typedef struct FC_MempoolPermissionRow
{    
    unsigned char m_Entity[FC_PLS_SIZE_ENTITY];                                 // Entity genesis transaction TxID, all 0s for master permissions 
    unsigned char m_Address[FC_PLS_SIZE_ADDRESS];                               // Address 
    uint32_t m_Type;                                                            // Permission type FC_PTP_ constants
} FC_MempoolPermissionRow;

/** Database record structure */

typedef struct FC_PermissionDBRow
{    
    unsigned char m_Entity[FC_PLS_SIZE_ENTITY];                                 // Entity genesis transaction TxID, all 0s for master permissions 
    unsigned char m_Address[FC_PLS_SIZE_ADDRESS];                               // Address 
    uint32_t m_Type;                                                            // Permission type FC_PTP_ constants
    uint32_t m_BlockFrom;                                                       // Permission block-from 
    uint32_t m_BlockTo;                                                         // Permission block-to
    uint64_t m_LedgerRow;                                                       // Row in the ledger corresponding to the last change for this key
    uint32_t m_Flags;                                                           // Flags FC_PFL_ constants
    uint32_t m_Reserved1;                                                       // Reserved to align to 80 bytes
    void Zero();
    int InBlockRange(uint32_t block);
} FC_PermissionDBRow;

/** Database block miner record structure */

typedef struct FC_BlockMinerDBRow
{    
    unsigned char m_BlockHash[FC_PLS_SIZE_ENTITY];                              // BlockHash
    unsigned char m_Null[FC_PLS_SIZE_ADDRESS];                                  // Should be Null
    uint32_t m_Type;                                                            // Permission type FC_PTP_ constants, should be FC_PTP_BLOCK_MINER
    unsigned char m_Address[FC_PLS_SIZE_ADDRESS];                               // Miner Address
    uint32_t m_AdminMinerCount;                                                 // Admin and miner counts, if 0 - should be recalculated
    void Zero();
} FC_BlockMinerDBRow;

/** Database admin/miner grants record structure */

typedef struct FC_AdminMinerGrantDBRow
{    
    unsigned char m_BlockHash[FC_PLS_SIZE_ENTITY];                              // BlockHash
    uint64_t m_Reserved1;                                                       // Reserved, should be 0
    uint64_t m_Reserved2;                                                       // Reserved, should be 0
    uint32_t m_RecordID;                                                        // ID for the record containing up to 6 tx offsets
    uint32_t m_Type;                                                            // Permission type FC_PTP_ constants, should be FC_PTP_BLOCK_MINER
    uint32_t m_Offsets[FC_PLS_SIZE_OFFSETS_PER_ROW];                            // Offsets of transactions with admin/miner grant
    void Zero();
} FC_AdminMinerGrantDBRow;


/** Database */

typedef struct FC_PermissionDB
{   
    char m_FileName[FC_DCT_DB_MAX_PATH];                                        // Full file name
    FC_Database *m_DB;                                                          // Database object
    uint32_t m_KeyOffset;                                                       // Offset of the key in FC_PermissionDBRow structure, 32 for protocol<=10003, 0 otherwise
    uint32_t m_KeySize;                                                         // Size of the database key, 24 for protocol<=10003, 56 otherwise
    uint32_t m_ValueOffset;                                                     // Offset of the value in FC_PermissionDBRow structure, 56 
    uint32_t m_ValueSize;                                                       // Size of the database value, 24
    uint32_t m_TotalSize;                                                       // Totals size of the database row
    FC_PermissionDB()
    {
        Zero();
    }
    
    ~FC_PermissionDB()
    {
        Close();
    }
    void Zero();
    int Open();
    int Close();
    void SetName(const char *name);
} FC_PermissionDB;

/** Ledger and mempool record structure */

typedef struct FC_PermissionLedgerRow
{    
    unsigned char m_Entity[FC_PLS_SIZE_ENTITY];                                 // Entity genesis transaction TxID, all 0s for master permissions 
    unsigned char m_Address[FC_PLS_SIZE_ADDRESS];                               // Address 
    uint32_t m_Type;                                                            // Permission type FC_PTP_ constants
    uint64_t m_PrevRow;                                                         // Previous row in the ledger corresponding to this key
    uint32_t m_BlockFrom;                                                       // Permission block-from after processing this row
    uint32_t m_BlockTo;                                                         // Permission block-to after processing this row
    uint32_t m_Consensus;                                                       // If consensus is reached returns number of admins, 0 otherwise
    uint32_t m_Flags;                                                           // Flags FC_PFL_ constants
    unsigned char m_Admin[FC_PLS_SIZE_ADDRESS];                                 // Admin address 
    uint32_t m_GrantFrom;                                                       // Permission block-from specified in this row
    uint32_t m_GrantTo;                                                         // Permission block-to specified in this row
    uint32_t m_Timestamp;                                                       // Timestamp of this row
//    uint32_t m_FoundInDB;                                                     // Row is found in database
    int32_t m_Offset;                                                           // Tx offset in the block, -1 if in mempool
    uint32_t m_BlockReceived;                                                   // Block this transaction was confirmed
    uint64_t m_ThisRow;                                                         // Row in the ledger    
    
    void Zero();
} FC_PermissionLedgerRow;

/** Block details row stored in permission ledger */

typedef struct FC_BlockLedgerRow
{    
    unsigned char m_Entity[FC_PLS_SIZE_ENTITY];                                 // Placeholder, always zeroes
    unsigned char m_Address[FC_PLS_SIZE_ADDRESS];                               // Miner Address or sprintf(...,"Block %08X row",<block height>)
    uint32_t m_Type;                                                            // FC_PTP_BLOCK_MINER or FC_PTP_BLOCK_INDEX
    uint64_t m_PrevRow;                                                         // Previous row in the ledger corresponding to this key
    uint32_t m_BlockFrom;                                                       // BlockHeight
    uint32_t m_BlockTo;                                                         // BlockHeight
    uint32_t m_AdminCount;                                                      // AdminCount
    uint32_t m_MinerCount;                                                      // MinerCount
    unsigned char m_CommitHash[FC_PLS_SIZE_HASH];                               // BlockHash
    uint32_t m_BlockFlags;                                                      // Flags FC_PBF_ constants
    uint32_t m_BlockReceived;                                                   // BlockHeight
    uint64_t m_ThisRow;                                                         // Row in the ledger    
    
    void Zero();
} FC_BlockLedgerRow;


/** Ledger */

typedef struct FC_PermissionLedger
{   
    char m_FileName[FC_DCT_DB_MAX_PATH];                                        // Full file name
    int m_FileHan;                                                              // File handle
    uint32_t m_KeyOffset;                                                       // Offset of the key in FC_PermissionLedgerRow structure, 32 for protocol<=10003, 0 otherwise
    uint32_t m_KeySize;                                                         // Size of the ledger key, 24 for protocol<=10003, 56 otherwise
    uint32_t m_ValueOffset;                                                     // Offset of the value in FC_PermissionLedgerRow structure, 56 
    uint32_t m_ValueSize;                                                       // Size of the ledger value 72
    uint32_t m_TotalSize;                                                       // Totals size of the ledger row
   
    FC_PermissionLedger()
    {
        Zero();
    }
    
    ~FC_PermissionLedger()
    {
        Close();
    }
    
    void Zero();
    int Open();
    int Close();
    void Flush();
    void SetName(const char *name);
    int GetRow(uint64_t RowID,FC_PermissionLedgerRow *row);
    uint64_t GetSize();
    int WriteRow(FC_PermissionLedgerRow *row);
    int SetRow(uint64_t RowID,FC_PermissionLedgerRow *row);
    
} FC_PermissionLedger;

/** Permission details structure */

typedef struct FC_PermissionDetails
{    
    unsigned char m_Entity[FC_PLS_SIZE_ENTITY];                                 // Entity genesis transaction TxID, all 0s for master permissions 
    unsigned char m_Address[FC_PLS_SIZE_ADDRESS];                               // Address 
    uint32_t m_Type;                                                            // Permission type FC_PTP_ constants
    uint32_t m_BlockFrom;                                                       // Permission block-from after processing this row
    uint32_t m_BlockTo;                                                         // Permission block-to after processing this row
    uint32_t m_Flags;                                                           // Flags FC_PFL_ constants
    int32_t m_RequiredAdmins;                                                   // Number of admins required for consensus                                                  
    unsigned char m_LastAdmin[FC_PLS_SIZE_ADDRESS];                             // Last admin address 
    uint32_t m_Reserved1;                                                       // Reserved for alignment
    uint64_t m_LastRow;                                                         // Last row in the ledger        
    void Zero();
} FC_PermissionDetails;




typedef struct FC_Permissions
{    
    FC_PermissionDB *m_Database;
    FC_PermissionLedger *m_Ledger;
    FC_Buffer   *m_MemPool;
    FC_Buffer   *m_TmpPool;
    char m_Name[FC_PRM_NETWORK_NAME_MAX_SIZE+1]; 
    char m_LogFileName[FC_DCT_DB_MAX_PATH+1]; 
    int m_Block;
    uint64_t m_Row;
    int m_AdminCount;
    int m_MinerCount;
//    int m_DBRowCount;

    uint64_t m_CheckPointRow;
    int m_CheckPointAdminCount;
    int m_CheckPointMinerCount;
    uint64_t m_CheckPointMemPoolSize;
    
    int m_CopiedBlock;
    int m_ForkBlock;
    uint64_t m_CopiedRow;
    int m_CopiedAdminCount;
    int m_CopiedMinerCount;
    int m_ClearedAdminCount;
    int m_ClearedMinerCount;
    int m_ClearedMinerCountForMinerVerification;
    int m_TmpSavedAdminCount;
    int m_TmpSavedMinerCount;
    
    FC_Buffer   *m_CopiedMemPool;

    int m_CheckForMempoolFlag;
    FC_Buffer               *m_MempoolPermissions;
    FC_Buffer               *m_MempoolPermissionsToReplay;    
    
    
    void *m_Semaphore;
    uint64_t m_LockedBy;

    FC_Permissions()
    {
        Zero();
    }
    
    ~FC_Permissions()
    {
        Destroy();
    }
    
// External functions    
    int Initialize(const char *name,int mode);

    int SetPermission(const void* lpEntity,const void* lpAddress,uint32_t type,const void* lpAdmin,uint32_t from,uint32_t to,uint32_t timestamp,
                                                                                                   uint32_t flags,int update_mempool,int offset);
    int SetApproval(const void* lpUpgrade,uint32_t approval,const void* lpAdmin,uint32_t from,uint32_t timestamp,uint32_t flags,int update_mempool,int offset);
    int Commit(const void* lpMiner,const void* lpHash);
    int RollBack(int block);
    int RollBack();
    
    int ClearMemPool();
    int CopyMemPool();
    int RestoreMemPool();
    
    int SetCheckPoint();
    int RollBackToCheckPoint();
    
    uint32_t GetAllPermissions(const void* lpEntity,const void* lpAddress,uint32_t type);
    uint32_t GetPermissionType(const char *str,int entity_type);
    uint32_t GetPossiblePermissionTypes(uint32_t entity_type);
    
    int GetAdminCount();
    int GetMinerCount();
    int GetActiveMinerCount();
    
    int GetBlockMiner(uint32_t block,unsigned char* lpMiner);
    uint32_t FindGovernanceModelChange(uint32_t from,uint32_t to);

    int IsApproved(const void* lpUpgrade);
    
    int CanConnect(const void* lpEntity,const void* lpAddress);
    int CanSend(const void* lpEntity,const void* lpAddress);
    int CanReceive(const void* lpEntity,const void* lpAddress);
    int CanWrite(const void* lpEntity,const void* lpAddress);
    int CanCreate(const void* lpEntity,const void* lpAddress);
    int CanIssue(const void* lpEntity,const void* lpAddress);
    int CanMine(const void* lpEntity,const void* lpAddress);
    int CanAdmin(const void* lpEntity,const void* lpAddress);    
    int CanActivate(const void* lpEntity,const void* lpAddress);    
    
    int CanMineBlock(const void* lpAddress,uint32_t block);
    
    int IsActivateEnough(uint32_t type);
    

    FC_Buffer *GetPermissionList(const void* lpEntity,const void* lpAddress,uint32_t type,FC_Buffer *old_buffer);
    FC_Buffer *GetUpgradeList(const void* lpUpgrade,FC_Buffer *old_buffer);
    FC_Buffer *GetPermissionDetails(FC_PermissionDetails *plsRow);
    void FreePermissionList(FC_Buffer *permissions);
    
    void MempoolPermissionsCopy();
    int MempoolPermissionsCheck(int from, int to);

    int RollBackBeforeMinerVerification(uint32_t block);
    int RestoreAfterMinerVerification();
    void SaveTmpCounts();    
    int StoreBlockInfo(const void* lpMiner,const void* lpHash);    
    int IncrementBlock(uint32_t admin_miner_count);    
    int GetBlockMiner(const void* lpHash,unsigned char* lpMiner,uint32_t *lpAdminMinerCount);
    int GetBlockAdminMinerGrants(const void* lpHash,int record,int32_t *offsets);
    int CanMineBlockOnFork(const void* lpAddress,uint32_t block,uint32_t last_after_fork);
    int IsBarredByDiversity(uint32_t block,uint32_t last,int miner_count);
    
    
    
// Internal functions    
    int Zero();    
    int Destroy();

    void Lock(int write_mode);
    void UnLock();
    
    int SetPermissionInternal(const void* lpEntity,const void* lpAddress,uint32_t type,const void* lpAdmin,uint32_t from,uint32_t to,uint32_t timestamp,
                                                                                                           uint32_t flags,int update_mempool,int offset);
    int CommitInternal(const void* lpMiner,const void* lpHash);
    int StoreBlockInfoInternal(const void* lpMiner,const void* lpHash,int update_counts);    
    int RollBackInternal(int block);
    uint32_t CalculateBlockFlags();
    int FindLastAllowedMinerRow(FC_PermissionLedgerRow *row,uint32_t block,int prev_result);
    
    
    int UpdateCounts();
    int AdminConsensus(const void* lpEntity,uint32_t type);
    
    int ClearMemPoolInternal();

    
    
    int RequiredForConsensus(const void* lpEntity,const void* lpAddress,uint32_t type,uint32_t from,uint32_t to);
    int VerifyConsensus(FC_PermissionLedgerRow *newRow,FC_PermissionLedgerRow *lastRow,int *remaining);
    int VerifyBlockHash(int32_t block,const void* lpHash);
    int FillPermissionDetails(FC_PermissionDetails *plsRow,FC_Buffer *plsDetailsBuffer);    
    
    uint32_t GetPermission(const void* lpAddress,uint32_t type);
    uint32_t GetPermission(const void* lpEntity,const void* lpAddress,uint32_t type);
    uint32_t GetPermission(const void* lpAddress,uint32_t type,FC_PermissionLedgerRow *row);
    uint32_t GetPermission(const void* lpEntity,const void* lpAddress,uint32_t type,FC_PermissionLedgerRow *row,int checkmempool);
    int IsSetupPeriod();
    

    void LogString(const char *message);
    void Dump();
    
    
} FC_Permissions;



#endif	/* FLOATCHAIN_PERMISSION_H */

