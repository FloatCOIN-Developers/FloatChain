...
/* FCHN START */
    int32_t checksum=(int32_t)FC_GetLE(&hash,4);
    checksum ^= (int32_t)FC_gState->m_NetworkParams->GetInt64Param("addresschecksumvalue");
    vch.insert(vch.end(), (unsigned char*)&checksum, (unsigned char*)&checksum + 4);
//    vch.insert(vch.end(), (unsigned char*)&hash, (unsigned char*)&hash + 4);
/* FCHN END */    
//    FC_DumpSize("TS",&vch[0],vch.size(),vch.size());
...
/* FCHN START */
    ...
    if (meFCmp((unsigned char*)&checksum, &vchRet.end()[-4], 4) != 0) {
//    if (meFCmp(&hash, &vchRet.end()[-4], 4) != 0) {
        vchRet.clear();
        return false;
    }
/* FCHN END */    
...
/*    
    ...
    if (!vchData.empty())
        meFCpy(&vchData[0], &vchTemp[nVersionBytes], vchData.size());
*/
    ...
    for(int i=0;i<(int)nVersionBytes;i++)
    {
        ...
        meFCpy(&vchData[i*shift],&vchTemp[i*(shift+1)+1],size);
    }
...
{
    return SetString(str.c_str(),Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS).size());// FCHN, 1 (default) in original code
}
 ...
 */
    ...
    for(int i=0;i<nVersionBytes;i++)
    {
        ...
        meFCpy(&vch[i*(shift+1)+1],&vchData[i*shift],size);
    }
//        FC_DumpSize("TS",&vchVersion[0],vchVersion.size(),vchVersion.size());
...
//    strcpy(test,EncodeBase58(vch).c_str());
    ...
    for(int i=0;i<(int)nVersionBytes;i++)
    {
        ...
        meFCpy(data+i*shift,&vch[i*(shift+1)+1],size);
    }
    
    CKeyID kBurn;
    meFCpy(&kBurn,data,nDataBytes);
...
{
    return CBase58Data::SetString(pszSecret,Params().Base58Prefix(CChainParams::SECRET_KEY).size()) && IsValid(); // FCHN 1 (default) in original code
}
