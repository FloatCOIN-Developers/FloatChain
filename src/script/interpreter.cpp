...
/* FCHN START */            
            if(fLongElement)
            {
                if(FC_gState->m_Features->FixedIn10007() == 0)
                {
                    if(opcode != OP_DROP)
                    {
            ...
            if(FC_gState->m_Features->Streams())                                // It is now part of the IsStandard check
            {
                fLongElement=false;                                
            }
            else
            {
                ...
                {
                    if(FC_gState->m_Features->VerifySizeOfOpDropElements() == 0)
                    {
                        fLongElement=true;                    
                    }
                    else
...
/* FCHN START */
    if(CheckSendPermission)
    {
        if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
        {
            ...
            if(FC_gState->m_Permissions->CanSend(NULL,pubkey_hash))
            {
...
bool VerifyScript(const CScript& scriptSig, const CScript& scriptPubKey, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror)
{
    ...
    if (CastToBool(stack.back()) == false)
        return set_error(serror, SCRIPT_ERR_CHECKSIGVERIFY);                    // FCHN
//        return set_error(serror, SCRIPT_ERR_EVAL_FALSE);
/* FCHN START */
