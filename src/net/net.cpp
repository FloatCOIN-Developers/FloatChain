...
/* FCHN START */        
        if(pszDest)
        {
            ...
            {
                if(strcmp(pszDest,FC_gState->GetSeedNode()) == 0)
                {
                    FC_gState->m_pSeedNode=pnode;
                    if(fDebug)LogPrint("FChn","FChn: Connected to seed node %s\n",pszDest);
                }
            }
        }
...
void CNode::PushVersion()
{
    ...
    nVersionNonceSent=nLocalHostNonce;                                              // FCHN
    if (fLogIPs)
    {
...
/* FCHN START */
    ...
    {
        subver=FormatSubVersion("FloatChain", FC_gState->GetProtocolVersion(), std::vector<string>());
    }
...
bool CNode::ReceiveMsgBytes(const char *pch, unsigned int nBytes)
{
    while (nBytes > 0) {
        ...
        if (msg.complete())
        {
            ...
            if(fDebug)LogPrint("FCnet","FCnet: complete message: %s, peer=%d\n", msg.hdr.GetCommand(),id);
        }
    }
...
int CNetMessage::readHeader(const char *pch, unsigned int nBytes)
{
    ...
    meFCpy(&hdrbuf[nHdrPos], pch, nCopy);
    nHdrPos += nCopy;
    
//    FC_Dump("HEAD",pch,nCopy);
    ...
    if(fDebug)LogPrint("FCnet","FCnet: received header: %s\n", hdr.GetCommand());
...
int CNetMessage::readData(const char *pch, unsigned int nBytes)
{
    ...
    meFCpy(&vRecv[nDataPos], pch, nCopy);
    
//    FC_Dump("RECV",pch,nCopy);
...
//        FC_Dump("SEND",(const void*)(&data[pnode->nSendOffset]),data.size()- pnode->nSendOffset);
...
/* FCHN END */                    
        ...
        if(vNodes.size() != nPrevNodeCount) {
            ...
            uiInterface.NotifyNuFConnectionsChanged(nPrevNodeCount);
        }
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "FloatChain " + FormatFullVersion();
...
/* FCHN START */    
    ...
    if(FC_gState->GetSeedNode())                                                // FCHN-TODO. Connection to seed node, find later how to disconnect
    {
        CAddress addr;
        LogPrintf("Seed node is set, trying to connect to %s...\n",FC_gState->GetSeedNode());
        OpenNetworkConnection(addr, NULL, FC_gState->GetSeedNode());
    }
...
/* Avoid communication with the node before connection is properly established with verackack and the network paramset is valid*/            
        ...
        if((FC_gState->m_NetworkParams->m_Status == FC_PRM_STATUS_VALID) && pnode->fParameterSetVerified)
        {
/* FCHN END */
            // Send messages
            {
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. FloatChain Core is probably already running."), addrBind.ToString());
        else
...
bool CAddrDB::Read(CAddrMan& addr)
{
    ...
    try {
        ...
        if (meFCmp(pchMsgTmp, Params().MessageStart(), sizeof(pchMsgTmp)))
...
void CNode::BeginMessage(const char* pszCommand) EXCLUSIVE_LOCK_FUNCTION(cs_vSend)
{
    ...
    if(fDebug)LogPrint("FChnminor","FChn: SEND: %s\n",SanitizeString(pszCommand));
}
...
void CNode::EndMessage() UNLOCK_FUNCTION(cs_vSend)
{
    ...
    meFCpy((char*)&ssSend[CMessageHeader::MESSAGE_SIZE_OFFSET], &nSize, sizeof(nSize));
    ...
    meFCpy((char*)&ssSend[CMessageHeader::CHECKSUM_OFFSET], &nChecksum, sizeof(nChecksum));
...
int FC_QuerySeed(boost::thread_group& threadGroup,const char *seedAddr)
{
    int err;
//    AddOneShot(seedAddr);
    StartNode(threadGroup);

    unsigned int StartTime=FC_TimeNowAsUInt();
    
    if(fDebug)LogPrint("FChn","FChn: Sending query to seed node %s\n",seedAddr);
        
    err=FC_ERR_NOERROR;
    
    while( (FC_gState->m_NetworkState != FC_NTS_SEED_READY) && (FC_gState->m_NetworkState != FC_NTS_SEED_NO_PARAMS))                       // FCHN-TODO add timeout
    {        
        __US_Sleep(1);
        if(FC_TimeNowAsUInt() - StartTime > 10)
        {
            LogPrintf("FChn: Could not establish connection with seed node - timeout\n");
            FC_gState->m_NetworkState=FC_NTS_SEED_READY;
            err=FC_ERR_CONNECTION_ERROR;
        }
    }
    
    if(err)
    {
        FC_gState->m_NetworkState=FC_NTS_NOT_READY;
    }
    
    if(fDebug)LogPrint("FChn","FChn: Query completed, waiting for seed node to disconnect\n");
    
    StartTime=FC_TimeNowAsUInt();
        ...
        {
            {
                ...
                if(vNodes.size() == 0)
                {
                    if(fDebug)LogPrint("FChn","FChn: Successfully disconnected from seed node\n");                    
            ...
            if(fCheckDisconnect)
            {
                ...
                if(FC_TimeNowAsUInt() - StartTime > 10)
                {
                    if(fDebug)LogPrint("FChn","FChn: Could not disconnect from seed node - timeout\n");                    
                    fCheckDisconnect=false;
                    err=FC_ERR_CONNECTION_ERROR;
                }
