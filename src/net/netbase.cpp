...
void CNetAddr::SetRaw(Network network, const uint8_t *ip_in)
{
    ...
    {
        case NET_IPV4:
            meFCpy(ip, pchIPv4, 12);
            meFCpy(ip+12, ip_in, 4);
            break;
        case NET_IPV6:
            meFCpy(ip, ip_in, 16);
            break;
...
bool CNetAddr::SetSpecial(const std::string &strName)
{
    if (strName.size()>6 && strName.substr(strName.size() - 6, 6) == ".onion") {
        ...
        if (vchAddr.size() != 16-sizeof(pchOnionCat))
            return false;
        meFCpy(ip, pchOnionCat, sizeof(pchOnionCat));
...
{
    return (meFCmp(ip, pchIPv4, sizeof(pchIPv4)) == 0);
}
...
bool CNetAddr::IsRFC6052() const
{
    ...
    return (meFCmp(ip, pchRFC6052, sizeof(pchRFC6052)) == 0);
}
...
bool CNetAddr::IsRFC4862() const
{
    ...
    return (meFCmp(ip, pchRFC4862, sizeof(pchRFC4862)) == 0);
}
...
bool CNetAddr::IsRFC6145() const
{
    ...
    return (meFCmp(ip, pchRFC6145, sizeof(pchRFC6145)) == 0);
}
...
{
    return (meFCmp(ip, pchOnionCat, sizeof(pchOnionCat)) == 0);
}
...
bool CNetAddr::IsValid() const
{
    ...
    if (meFCmp(ip, pchIPv4+3, sizeof(pchIPv4)-3) == 0)
        return false;

    // unspecified IPv6 address (::/128)
    unsigned char ipNone[16] = {};
    if (meFCmp(ip, ipNone, 16) == 0)
        return false;
...
bool CNetAddr::GetInAddr(struct in_addr* pipv4Addr) const
{
    if (!IsIPv4())
        return false;
    meFCpy(pipv4Addr, ip+12, 4);
    return true;
}

bool CNetAddr::GetIn6Addr(struct in6_addr* pipv6Addr) const
{
    meFCpy(pipv6Addr, ip, 16);
    return true;
}
...
{
    return a.valid == b.valid && a.network == b.network && !meFCmp(a.netmask, b.netmask, 16);
}
