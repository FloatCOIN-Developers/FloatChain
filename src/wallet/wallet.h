...
/* FCHN START */
struct FC_WalletTxs;
#define FC_TFL_INVALID                  0x00000001
#define FC_TFL_IS_COINBASE              0x00000002
#define FC_TFL_ALL_INPUTS_ARE_FINAL     0x00000004
#define FC_TFL_FROM_ME                  0x00010000
#define FC_TFL_ALL_INPUTS_FROM_ME       0x00020000
#define FC_TFL_IS_CHANGE                0x00040000
#define FC_TFL_IS_SPENDABLE             0x00080000
#define FC_TFL_IS_MINE_FOR_THIS_SEND    0x10000000
#define FC_TFL_IMPOSSIBLE               0x80000000

#define FC_CSF_ALLOW_SPENDABLE_P2SH     0x00000001
#define FC_CSF_ALLOW_NOT_SPENDABLE_P2SH 0x00000002
#define FC_CSF_ALLOW_NOT_SPENDABLE      0x00000004
#define FC_CSF_SIGN                     0x00000008
#define FC_CSF_ALLOWED_COINS_ARE_MINE   0x00000010
...
public:
    ...
    int GroupCount();
    int GetGroup(FC_Buffer *assets,int addIfNeeded);
...
//    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed=true, const CCoinControl *coinControl = NULL) const;
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed=true, const CCoinControl *coinControl = NULL, bool fOnlyUnlocked=true, 
                        bool fOnlyCoinsNoTxs=false, uint160 addr=0, uint32_t flags=FC_CSF_ALLOW_SPENDABLE_P2SH) const;
/* FCHN END */    
    ...
    bool CreateTransaction(CScript scriptPubKey, const CAmount& nValue,
                           CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl *coinControl = NULL);
/* FCHN START */    
    int SelectFloatChainCombineCoinsMinConf(int nConfMine, int nConfTheirs, std::vector<COutput> vCoins, FC_Buffer *in_map, FC_Buffer *in_amounts,
                                int in_selected_row,int in_asset_row,
                                std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet,int max_inputs) const;    
    bool SelectFloatChainCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, std::vector<COutput> vCoins, FC_Buffer *in_map, FC_Buffer *in_amounts,
    ...
    bool SelectFloatChainCoins(const CAmount& nTargetValue, std::vector<COutput> &vCoins, FC_Buffer *in_map, FC_Buffer *in_amounts, 
                                int in_selected_row,int in_asset_row,int in_preferred_row,
                                std::set<std::pair<uint256,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl) const;    
    bool CreateFloatChainTransaction(const std::vector<std::pair<CScript, CAmount> >& vecSend,
                           CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl *coinControl = NULL,
...
/* FCHN START */        
        ...
        return false;
//        return (GetDebit(filter) >= 0);                                         // FCHN > in original code
/* FCHN END */        
    }
...
public:
    ...
    COutput(const CWalletTx *txIn, int iIn, int nDepthIn, bool fSpendableIn, const FC_Coin& coinIn)
    {
