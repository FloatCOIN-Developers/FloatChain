...
void CAssetGroupTree::Dump()
{
    ...
    if(fDebug)LogPrint("FChn","FChn: Asset Grouping. Group Size: %d. Group Count: %d\n",nAssetsPerGroup,lpAssetGroups->GetCount()-1);
...
int CAssetGroupTree::Resize(int newAssets)
{
    ...
    if(fDebug)LogPrint("FChn","Asset grouping resize %d -> %d\n",nAssetsPerGroup,n);
    
    FC_Buffer *new_asset_groups_buffer;
    
    new_asset_groups_buffer=new FC_Buffer;
    
    err=new_asset_groups_buffer->Initialize(sizeof(CAssetGroup),sizeof(CAssetGroup)+n*sizeof(int),FC_BUF_MODE_DEFAULT);
    if(err)
    {
        return err;
    }
    ...
    for(i=0;i<lpAssetGroups->GetCount();i++)                                    
    {
        ...
        meFCpy(new_asset_groups_buffer->GetRow(i),lpAssetGroups->GetRow(i),sizeof(CAssetGroup)+nAssetsPerGroup*sizeof(int));
    }
...
int CAssetGroupTree::Initialize(int assetsPerGroup,int maxAssetsPerGroup,int optimalGroupCount,int mode)
{
    ...
    lpAssets=new FC_Buffer;
    err=lpAssets->Initialize(FC_AST_ASSET_QUANTITY_OFFSET,FC_AST_ASSET_QUANTITY_OFFSET+sizeof(int),FC_BUF_MODE_MAP);
    ...
    lpAssetGroups=new FC_Buffer;
    err=lpAssetGroups->Initialize(sizeof(CAssetGroup),sizeof(CAssetGroup)+assetsPerGroup*sizeof(int),FC_BUF_MODE_DEFAULT);
    ...
    lpTmpGroupBuffer=(int*)FC_New(maxAssetsPerGroup*sizeof(int));
...
int CAssetGroupTree::GetGroup(FC_Buffer* assets, int addIfNeeded)
{
...
//        if(FC_GetLE(assetRef+FC_AST_ASSET_REF_TYPE_OFFSET,FC_AST_ASSET_REF_TYPE_SIZE) != FC_AST_ASSET_REF_TYPE_SPECIAL)// Special rows are ignored - there is no real assets with asset-ref block=0
        if( (FC_GetABRefType(assetRef) != FC_AST_ASSET_REF_TYPE_SPECIAL) &&
            (FC_GetABRefType(assetRef) != FC_AST_ASSET_REF_TYPE_GENESIS) )                    
//        if(FC_GetLE(assetRef,4) > 0)                             
        {
                                                                                ...
                                                                                // Group is already in the right place in the tree, but size is not updated yet 
    {
        ...
        {
            *(int*)(lpAssets->GetRow(i)+FC_AST_ASSET_QUANTITY_OFFSET)=thisGroup->nThisGroup;
        }
...
int CAssetGroupTree::GetGroup(unsigned char *assetRef,int addIfNeeded)
{
    ...
    if(row >= 0)                                                                // Found
    {
        return *(int*)(lpAssets->GetRow(row)+FC_AST_ASSET_QUANTITY_OFFSET);
    }
    ...
    *(int*)(lpAssets->GetRow(asset_id)+FC_AST_ASSET_QUANTITY_OFFSET)=thisGroup->nThisGroup;        
    aptr=(int*)(lpAssetGroups->GetRow(thisGroup->nThisGroup)+sizeof(CAssetGroup));
...
int64_t FC_GetABCoinQuantity(void *ptr,int coin_id)
{
    return (int64_t)FC_GetLE((unsigned char*)ptr+FC_AST_ASSET_QUANTITY_OFFSET+coin_id*FC_AST_ASSET_QUANTITY_SIZE,FC_AST_ASSET_QUANTITY_SIZE);        
}

void FC_SetABCoinQuantity(void *ptr,int coin_id,int64_t quantity)
{
    FC_PutLE((unsigned char*)ptr+FC_AST_ASSET_QUANTITY_OFFSET+coin_id*FC_AST_ASSET_QUANTITY_SIZE,&quantity,FC_AST_ASSET_QUANTITY_SIZE);        
}
...
void DebugPrintAssetTxOut(uint256 hash,int index,unsigned char* assetrefbin,int64_t quantity)
{
    ...
    if(FC_gState->m_Features->ShortTxIDInTx())
    {
        if(debug_print)
        {
            ...
            if(FC_GetABRefType(assetrefbin) == FC_AST_ASSET_REF_TYPE_SPECIAL)
            {
                printf("Special:        %08x%08x",(uint32_t)FC_GetLE(assetrefbin,4),(uint32_t)FC_GetLE(assetrefbin+4,4));
            }
            else
            {
                for(int i=FC_AST_SHORT_TXID_OFFSET+FC_AST_SHORT_TXID_SIZE-1;i>=FC_AST_SHORT_TXID_OFFSET;i--)
                {
...
void AvalableCoinsForAddress(CWallet *lpWallet,vector<COutput>& vCoins, const CCoinControl* coinControl,const set<CTxDestination>* addresses,uint32_t flags)
{
    ...
    this_time=FC_TimeNowAsDouble();    
    if(fDebug)LogPrint("FCperf","FCperf: AvailableCoins: Time: %8.6f \n", this_time-last_time);
    last_time=this_time;
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)                            // We already processed from-address
    {
        if(addresses->size() == 1)
        {
            return;
    ...
    this_time=FC_TimeNowAsDouble();    
    if(fDebug)LogPrint("FCperf","FCperf: Address Filtering: Time: %8.6f \n", this_time-last_time);
    last_time=this_time;
}
...
bool InsertCoinIntoMatrix(int coin_id,                                          // IN  coin id in the vCoins array
                          ...
                          FC_Buffer *tmp_amounts,                               // IN  Asset values of the UTXO
                          FC_Buffer *out_amounts,                               // IN  Only assets found in this buffer will be added, if NULL - all assets
                          FC_Buffer *in_amounts,                                // OUT Buffer to fill, rows -assets, columns - coins
                          FC_Buffer *in_map,                                    // OUT txid/vout->coin id map (used in coin selection)
                          ...
                          int64_t pure_native)                                  // IN  Pure native currency flag
{
    ...
    meFCpy(buf_map,&hash,32);                                                   // Updating txid/vout->coin id map
    FC_PutLE(buf_map+32,&out_i,4);
    FC_PutLE(buf_map+36,&coin_id,4);
    in_map->Add(buf_map,buf_map+36);

    for(int i=0;i<tmp_amounts->GetCount();i++)                                  // Inserting asset amounts into the matrix
    {
        quantity=FC_GetABQuantity(tmp_amounts->GetRow(i));
        if( (out_amounts == NULL) || (out_amounts->Seek(tmp_amounts->GetRow(i)) >= 0) )// Only assets found in out_amounts if specified
        {
            ...
            if(row < 0)                                                         // New asset
            {
                ...
                meFCpy(in_row,tmp_amounts->GetRow(i),FC_AST_ASSET_QUANTITY_OFFSET);
                FC_SetABCoinQuantity(in_row,coin_id,quantity);
            ...
            {
                FC_SetABCoinQuantity(in_amounts->GetRow(row),coin_id,quantity);
            }
        }
    }    

    quantity=0;                                                                 // Setting "selected" flag to 0
    FC_SetABCoinQuantity(in_amounts->GetRow(in_special_row[0]),coin_id,quantity);
    quantity=1;                                                                 // Setting "parsed" flag to 1
    FC_SetABCoinQuantity(in_amounts->GetRow(in_special_row[1]),coin_id,quantity);                   
            
    FC_SetABCoinQuantity(in_amounts->GetRow(in_special_row[4]),coin_id,pure_native);                   
...
bool FindRelevantCoins(CWallet *lpWallet,                                       // IN  Wallet (to access grouping object)
                       ...
                       FC_Buffer *out_amounts,                                  // IN  assets amounts to be found
                       ...
                       FC_Buffer *in_amounts,                                   // OUT Buffer to fill, rows - assets, columns - coins
                       FC_Buffer *in_map,                                       // OUT txid/vout->coin id map (used in coin selection)
                       FC_Buffer *tmp_amounts,                                  // TMP temporary asset-quantity buffer
                       unsigned char *in_row,int in_size,                       // TMP temporary buffer row and its size
                       FC_Script *lpScript,                                     // TMP temporary floatchain script object
                       int *in_special_row,                                     // IN  Coordinates of special rows
                       ...
                       std::string& strFailReason)                              // OUT error message
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        out_i=out.i;
        tmp_amounts->Clear();
        if(ParseFloatchainTxOutToBuffer(hash,txout,tmp_amounts,lpScript,&allowed,&required,mapSpecialEntity,strError))
        {
...
/*            
            ...
            {
//                if( (FC_GetLE(assetRef+FC_AST_ASSET_REF_TYPE_OFFSET,FC_AST_ASSET_REF_TYPE_SIZE) != FC_AST_ASSET_REF_TYPE_SPECIAL)
                      || (FC_GetLE(tmp_amounts->GetRow(i)+4,4) != FC_PTP_SEND) )
                {
 ...
 */ 
            ...
            {
                if(debug_print)DebugPrintAssetTxOut(hash,out_i,tmp_amounts->GetRow(i),FC_GetABQuantity(tmp_amounts->GetRow(i)));
            }
            
            is_relevant=true;
            if(required & FC_PTP_ISSUE)                                         // This txout contains unconfirmed genesis, cannot be spent
            {
                is_relevant=false;
            }
            if(is_relevant)
            {
                if(allowed & FC_PTP_SEND)
                {
                        ...
                        std::string& strFailReason)                             // OUT error message
{
    ...
    {       
        string strError;
        int allowed=FC_PTP_SEND;
        int required=FC_PTP_SEND;
        int out_i;
        ...
        if( (count<max_inputs) && (out.nDepth >= min_conf) )
        {
            ...
            out_i=out.i;
            tmp_amounts->Clear();
            if(ParseFloatchainTxOutToBuffer(hash,txout,tmp_amounts,lpScript,&allowed,&required,strError))
            {
                if( (required & FC_PTP_ISSUE) == 0 )                            // Ignore txouts containing unconfirmed geneses
                {
    ...
    meFCpy(in_amounts->GetRow(in_special_row[0]),in_amounts->GetRow(in_special_row[1]),in_size);
...
bool CalculateChangeAmounts(CWallet *lpWallet,                                  // IN  Wallet (to access grouping object)
                            ...
                            FC_Buffer *out_amounts,                             // IN  assets amounts to be found                            
                            int expected_required,                              // IN  expected permissions
                            FC_Buffer *in_amounts,                              // IN  selected asset amounts
                            FC_Buffer *change_amounts,                          // OUT change amounts, asset-quantity buffer with additional field - group
                            FC_Buffer *tmp_amounts,                             // TMP temporary asset-quantity buffer
                            FC_Script *lpScript,                                // TMP temporary floatchain script object
                            int *in_special_row,                                // IN  Coordinates of special rows
...
{
    int coin_id=0;
    unsigned char buf[FC_AST_ASSET_FULLREF_BUF_SIZE+sizeof(int)];
    int64_t quantity;
    string strError;
    int allowed=expected_required;
    int required=expected_required;
    ...
    {
                                                                                // Coin is taken
        if(FC_GetABCoinQuantity(in_amounts->GetRow(in_special_row[1]),coin_id))
        {
                                                                                // Coin is selected
            if(FC_GetABCoinQuantity(in_amounts->GetRow(in_special_row[0]),coin_id))
            {
                CTxOut txout;
                uint256 hash=out.GetHashAndTxOut(txout);
                out_i=out.i;
                ...
                if(ParseFloatchainTxOutToBuffer(hash,txout,tmp_amounts,lpScript,&allowed,&required,mapSpecialEntity,strError))
                {
                    for(int i=0;i<tmp_amounts->GetCount();i++)
                    {
                        if(debug_print)DebugPrintAssetTxOut(hash,out.i,tmp_amounts->GetRow(i),FC_GetABQuantity(tmp_amounts->GetRow(i)));
                        if(fDebug)LogAssetTxOut("Input : ",hash,out_i,tmp_amounts->GetRow(i),FC_GetABQuantity(tmp_amounts->GetRow(i)));
                    }
                    for(int i=0;i<tmp_amounts->GetCount();i++)                  // Add all assets onto change buffer, not only those from out_amounts
                    {
                        quantity=FC_GetABQuantity(tmp_amounts->GetRow(i));
                        if( (FC_GetABRefType(tmp_amounts->GetRow(i)) != FC_AST_ASSET_REF_TYPE_SPECIAL) &&
                            (FC_GetABRefType(tmp_amounts->GetRow(i)) != FC_AST_ASSET_REF_TYPE_GENESIS) )                    
                        ...
                        if(row < 0)                                             // New asset
                        {
                            memset(buf,0,FC_AST_ASSET_FULLREF_BUF_SIZE+sizeof(int));
                            meFCpy(buf,tmp_amounts->GetRow(i),FC_AST_ASSET_QUANTITY_OFFSET);
                            FC_SetABQuantity(buf,quantity);
                            FC_PutLE(buf+FC_AST_ASSET_FULLREF_BUF_SIZE,&group_id,sizeof(int));
                        ...
                        {
                            quantity+=FC_GetABQuantity(change_amounts->GetRow(row));
                            FC_SetABQuantity(change_amounts->GetRow(row),quantity);
                        }
                    }    
    ...
    {
        if(FC_GetABRefType(out_amounts->GetRow(i)) != FC_AST_ASSET_REF_TYPE_GENESIS)
        {
            quantity=-FC_GetABQuantity(out_amounts->GetRow(i));
            if(quantity)
            {
                ...
                {
                    memset(buf,0,FC_AST_ASSET_FULLREF_BUF_SIZE+sizeof(int));
                    meFCpy(buf,out_amounts->GetRow(i),FC_AST_ASSET_QUANTITY_OFFSET);
                    FC_SetABQuantity(buf,quantity);
                    err=change_amounts->Add(buf);
                    if(err)
                    {
    ...
    {
        if(debug_print)DebugPrintAssetTxOut(0,0,change_amounts->GetRow(i),FC_GetABQuantity(change_amounts->GetRow(i)));
    }
...
bool FindChangeAddress(CWallet *lpWallet,CTxDestination& address,const set<CTxDestination>* addresses, CReserveKey& reservekey, const CCoinControl* coinControl,int expected_required,map<uint32_t, uint256>* mapSpecialEntity)
{
    ...
    else
    {
        ...
        if(required & FC_PTP_SEND)
        {
            required-=FC_PTP_SEND;
        }
        required |= FC_PTP_RECEIVE;
        ...
        CPubKey vchPubKey;
        if(!lpWallet->GetKeyFromAddressBook(vchPubKey,FC_PTP_SEND | required,addresses,mapSpecialEntity))
        {
            if(!lpWallet->GetKeyFromAddressBook(vchPubKey,FC_PTP_SEND | FC_PTP_RECEIVE,addresses))
            {
                if(!lpWallet->GetKeyFromAddressBook(vchPubKey,required,addresses))
                {
                    if(!lpWallet->GetKeyFromAddressBook(vchPubKey,FC_PTP_RECEIVE,addresses))
                    {
...
bool SelectCoinsToUse(const vector<COutPoint>* lpCoinsToUse,                            // IN  List of coins to be used in this transaction
                      FC_Buffer *in_map,                                        // IN  txid/vout->coin id map (used in coin selection)
                      FC_Buffer *in_amounts,                                    // IN/OUT Input amounts buffer, output - selected row
                      int *in_special_row,                                      // IN  Coordinates of special rows
                      std::string& strFailReason)                               // OUT error message
{
    int64_t quantity;
    ...
    BOOST_FOREACH(const COutPoint& coin, *lpCoinsToUse)
    {
        ...
        coin_id=FC_GetLE(in_map->GetRow(row)+36,4);
        quantity=1;
        FC_SetABCoinQuantity(in_amounts->GetRow(in_special_row[0]),coin_id,quantity);                                                        
    }            
                      ...
                      const CCoinControl* coinControl)
{
    ...
    if(in_asset_row>=0)
    {
        for(coin_id=0;coin_id<(int)vCoins.size();coin_id++)
        {
            ...
            if(FC_GetABCoinQuantity(in_amounts->GetRow(in_special_row[1]),coin_id))
            {
                ...
                quantity=FC_GetABCoinQuantity(in_amounts->GetRow(in_asset_row),coin_id);
                if(quantity)
                {
    ...
    if(( nTargetValue > 0) || (!found_selected) )
    {
        ...
        if(!lpWallet->SelectFloatChainCoins(nTargetValue,vCoins,in_map,in_amounts,in_special_row[0],in_asset_row,in_prefered_row,setAssetCoins, nAssetValueIn, coinControl))
        {
            return false;
        }                        
...
CAmount BuildAssetTransaction(CWallet *lpWallet,                                // IN  Wallet object
                              ...
                              FC_Buffer *in_amounts,                            // IN  Input amount matrix
                              FC_Buffer *change_amounts,                        // IN  Change amount matrix
                              ...
                              std::string& strFailReason)                       // OUT error message
{
    ...
    {
        quantity=FC_GetABQuantity(change_amounts->GetRow(i));
        ...
        if(quantity>0)
        {
            group_id=FC_GetLE(change_amounts->GetRow(i)+FC_AST_ASSET_FULLREF_BUF_SIZE,sizeof(int));
            if(FC_GetABRefType(change_amounts->GetRow(i)) != FC_AST_ASSET_REF_TYPE_SPECIAL)
            //if(FC_GetLE(change_amounts->GetRow(i),4) != 0)
            {
            ...
            if( (FC_GetABRefType(change_amounts->GetRow(i)) == FC_AST_ASSET_REF_TYPE_SPECIAL) &&
                 (FC_GetLE(change_amounts->GetRow(i)+4,4) == FC_PTP_SEND) )
            {
                nTotalInValue=quantity;
            }
        }
    ...
    if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
    {
        ...
        {        
            ...
            if(aptr)
            {
                if((change_aptr == NULL) ||  meFCmp(change_aptr,aptr,20))
                {                    
                    if(FC_gState->m_Permissions->CanReceive(NULL,aptr))
                    {
                        extra_change_count++;
                    ...
                    {
                        if((FC_gState->m_Features->AnyoneCanReceiveEmpty() != 0) && (min_output == 0))                          
                        {
                            extra_change_count++;                            
                        }                        
                    }
    ...
    {
        if(fDebug)LogPrint("FCatxo","FCatxo: Missing amount: %ld. Fee: %ld, Change: %ld, Inputs: %ld\n",missing_amount,nFeeRet,change_count*min_output,nTotalInValue);
    ...
    while(missing_amount == 0)
    {
        ...
        int assets_per_opdrop=(MAX_SCRIPT_ELEMENT_SIZE-4)/(FC_gState->m_NetworkParams->m_AssetRefSize+FC_AST_ASSET_QUANTITY_SIZE);
        
        if(FC_gState->m_Features->VerifySizeOfOpDropElements())
        {
            assets_per_opdrop=(MAX_SCRIPT_ELEMENT_SIZE-4)/(FC_gState->m_NetworkParams->m_AssetRefSize+FC_AST_ASSET_QUANTITY_SIZE);
        }
            ...
            for(int i=0;i<change_amounts->GetCount();i++)
            {
                ...
                if( (tmp_amounts->GetCount()>=assets_per_opdrop) || (i == change_amounts->GetCount()-1) )
                {
                    if(tmp_amounts->GetCount())
                    {
                        ...
                        for(int i=0;i<tmp_amounts->GetCount();i++)
                        {
                            if(debug_print)DebugPrintAssetTxOut(0,0,tmp_amounts->GetRow(i),FC_GetABQuantity(tmp_amounts->GetRow(i)));
                            if(fDebug)LogAssetTxOut("Change: ",0,0,tmp_amounts->GetRow(i),FC_GetABQuantity(tmp_amounts->GetRow(i)));
                        }
                        lpScript->Clear();
                        lpScript->SetAssetQuantities(tmp_amounts,FC_SCR_ASSET_SCRIPT_TYPE_TRANSFER);
        ...
        if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
        {
            ...
            {        
                ...
                if(aptr)
                {
                    if((change_aptr == NULL) ||  meFCmp(change_aptr,aptr,20))
                    {
                        if( (FC_gState->m_Permissions->CanReceive(NULL,aptr) != 0) || 
                            ((FC_gState->m_Features->AnyoneCanReceiveEmpty() != 0) && (default_change_output == 0)) )
                        {
        ...
        {            
            if(FC_GetABCoinQuantity(in_amounts->GetRow(in_special_row[1]),coin_id))
            {
                if(FC_GetABCoinQuantity(in_amounts->GetRow(in_special_row[0]),coin_id))
                {
                    ...
                    if(required & FC_PTP_CACHED_SCRIPT_REQUIRED)
                    {
                        if(!fScriptCached)
                        {
                            if(FC_GetABCoinQuantity(in_amounts->GetRow(in_special_row[6]),coin_id))
                            {
                                int cs_offset;
                                CScript script3;        
                                size_t elem_size;
...
{   
    ...
    {
        if (nValue < 0)                                                         // Floatchain allows protocol zero-value outputs
        {
...
//    if (vecSend.empty() || nValue < 0)
    ...
    FC_Buffer *out_amounts;                                         
    
    out_amounts=new FC_Buffer;
    FC_InitABufferMap(out_amounts);
        
    FC_Buffer *change_amounts;
    
    change_amounts=new FC_Buffer;
    change_amounts->Initialize(FC_AST_ASSET_QUANTITY_OFFSET,FC_AST_ASSET_FULLREF_BUF_SIZE+sizeof(int),FC_BUF_MODE_MAP);
    
    FC_Buffer *tmp_amounts;
    
    tmp_amounts=new FC_Buffer;
    FC_InitABufferMap(tmp_amounts);
    
    FC_Script *lpScript;
    lpScript=new FC_Script;    

    FC_Buffer *in_amounts;
    in_amounts=NULL;
    ...
    if(vecSend.size())
    {
        ...
        {
            CTxOut txout(s.second, s.first);
            int this_required=FC_PTP_ALL;
            if(!ParseFloatchainTxOutToBuffer(hash,txout,out_amounts,lpScript,NULL,&this_required,&mapSpecialEntity,strFailReason))
            {
                goto exitlbl;
            }
    ...
    else
    {
        ...
        if(!ParseFloatchainTxOutToBuffer(hash,txout,out_amounts,lpScript,NULL,&required,strFailReason))
        {
            goto exitlbl;
        }        
    }
    ...
    if(fDebug)LogPrint("FCatxo","FCatxo: ====== New transaction, required %d\n",required);
    for(int i=0;i<out_amounts->GetCount();i++)
    {
        if(debug_print)DebugPrintAssetTxOut(0,0,out_amounts->GetRow(i),FC_GetABQuantity(out_amounts->GetRow(i)));
        if(fDebug)LogAssetTxOut("Output: ",0,0,out_amounts->GetRow(i),FC_GetABQuantity(out_amounts->GetRow(i)));
    }
    ...
    if(fDebug)LogPrint("FCperf","Coin Selection, available coins time: %8.6f\n",this_time-last_time);
    last_time=this_time;
    ...
    in_size=FC_AST_ASSET_QUANTITY_OFFSET+vCoins.size()*FC_AST_ASSET_QUANTITY_SIZE;
    in_amounts=new FC_Buffer;
    in_amounts->Initialize(FC_AST_ASSET_QUANTITY_OFFSET,in_size,FC_BUF_MODE_MAP);

    this_time=FC_TimeNowAsDouble();
    if(csperf_debug_print)if(vecSend.size())printf("Alloc                   : %8.6f (%d)\n",this_time-last_time,in_size);
    last_time=this_time;
                                                                                // Map coin -> index in in_amounts
                                                                                // We need this as in SelectFloatChainCoinsMinConf coins will be shuffled
                                                                                // Key: 32-byte txid, 4-byte output id. Value - index in in_amounts
    in_map=new FC_Buffer;
    in_map->Initialize(36,40,FC_BUF_MODE_MAP);
    
    in_row=(unsigned char*)FC_New(in_size);
    ...
    memset(in_row,0,FC_AST_ASSET_QUANTITY_OFFSET);
    type=0;
    FC_PutLE(in_row+4,&type,4);
    FC_SetABRefType(in_row,FC_AST_ASSET_REF_TYPE_SPECIAL);
    ...
    
    {
        LOCK2(cs_main, lpWallet->cs_wallet);
        {
            ...
            if(vecSend.size())                                                  // Normal transaction
            {
                ...
                if(fDebug)LogPrint("FCperf","FCperf: CS: Input Parsing: Time: %8.6f \n", this_time-last_time);
                last_time=this_time;
    
                for(int asset=0;asset<out_amounts->GetCount();asset++)
                {
                    if(FC_GetABRefType(out_amounts->GetRow(asset)) != FC_AST_ASSET_REF_TYPE_GENESIS)
                    {
                        int in_asset_row;
                        nTotalOutValue=FC_GetABQuantity(out_amounts->GetRow(asset));
                        ...
                        if((nTotalOutValue > 0) ||                              // Can send 0 amount of specific asset if there are no outputs carrying this asset
                             ((FC_GetABRefType(out_amounts->GetRow(asset)) == FC_AST_ASSET_REF_TYPE_SPECIAL) &&
                             (FC_GetLE(out_amounts->GetRow(asset)+4,4) == FC_PTP_SEND)))// But not for native currency, otherwise we can get "no inputs"                                  
                        {
...
/*
                                                if(required & FC_PTP_WRITE)     // publish always comes with addresses set, SEND fails before write
                                                {
                                                    strFailReason = _("No unspent output with write permission");                                                                                                 
                                                }
                                                else
 ...
 */ 
                ...
                if(fDebug)LogPrint("FCperf","FCperf: CS: Coin Selection: Time: %8.6f \n", this_time-last_time);
                last_time=this_time;                
            }
            else
            {
            ...
            if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
            {
                min_output=FCP_MINIMUM_PER_OUTPUT;
            }            
                                                                                            // Storing selection before pure-native 
            meFCpy(in_amounts->GetRow(in_special_row[2]),in_amounts->GetRow(in_special_row[0]),in_size);
            ...
            if(fDebug)LogPrint("FCperf","FCperf: Transaction Building: Time: %8.6f \n", this_time-last_time);
            last_time=this_time;
            ...
            while(missing_amount > 0)                                           // We still need more native currency
            {
                ...
                meFCpy(in_amounts->GetRow(in_special_row[0]),in_amounts->GetRow(in_special_row[2]),in_size);
...
exitlbl:
    ...
    {
        if(fDebug)LogPrint("FCatxo","FCatxo: ====== Error: %s\n",strFailReason.c_str());
        if(!skip_error_message)
        {
            LogPrintf("FChn: Coin selection: %s\n",strFailReason.c_str());
        }
        return false;
    }
    ...
    if(fDebug)LogPrint("FCatxo","FCatxo: ====== Created: %s\n",wtxNew.GetHash().ToString().c_str());
    return true;
}

bool CWallet::CreateFloatChainTransaction(const vector<pair<CScript, CAmount> >& vecSend,
                                CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl,
                                const set<CTxDestination>* addresses,int min_conf,int min_inputs,int max_inputs,const vector<COutPoint>* lpCoinsToUse, int *eErrorCode)
{
    ...
    {
        return CreateAssetGroupingTransaction(this, vecSend,wtxNew,reservekey,nFeeRet,strFailReason,coinControl,addresses,min_conf,min_inputs,max_inputs,lpCoinsToUse,FC_CSF_ALLOW_SPENDABLE_P2SH | FC_CSF_SIGN, eErrorCode);        
    } 
    return true;
}
...
bool CWallet::UpdateUnspentList(const CWalletTx& wtx, bool update_inputs)
{
    ...
    int unspent_count=0;
//    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)                          // Not supported, not used in this case
...
//    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)                          // Not supported, not used in this case
    ...
    int max_assets_per_group=assets_per_opdrop*FCP_STD_OP_DROP_COUNT;
    ...
    if( (tmp_amounts != NULL) && (lpScript != NULL) )                       // Count assets in unspent coins
    {
        ...
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            ParseFloatchainTxOutToBuffer(hash,txout,tmp_amounts,lpScript,NULL,NULL,strError);
        }
        ...
        if(fDebug)LogPrint("FChn","FChn: Found %d assets in %d groups\n",asset_count,lpAssetGroups->GroupCount()-1);
        lpAssetGroups->Dump();
    }        
    ...
    if(fDebug)LogPrint("FChn","FChn: Unspent list initialized: Total: %d, Unspent: %d\n",mapWallet.size(),mapUnspent.size());
...
//    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)                          // Not supported, not used
...
exitlbl:    
    ...
    this_time=FC_TimeNowAsDouble();
    if(fDebug)LogPrint("FChn","FChn: Wallet coins: Total: %d, Unspent: %d, Kept: %d, Purged: %d, Skipped: %d, Time: %8.6f\n",total,mapUnspent.size(),should_keep.size(),count,skipped,this_time-last_time);
}
...
bool CWallet::CreateAndCommitOptimizeTransaction(CWalletTx& wtx,std::string& strFailReason,const std::set<CTxDestination>* addresses,int min_conf,int min_inputs,int max_inputs)
{
    ...
    {    
        if(fDebug)LogPrint("FChn","Committing wallet optimization tx. Inputs: %ld, Outputs: %ld\n",wtx.vin.size(),wtx.vout.size());
        
        if (CommitTransaction(wtx, reservekey, strFailReason))
        {
            if(fDebug)LogPrint("FChn","Committing wallet optimization tx completed\n");
            return true;
        }
    }    
...
{
    if(FC_TimeNowAsUInt()<nNextUnspentOptimization)
    {
        return false;
    }

    if(fDebug)LogPrint("FChn","FChn: Wallet optimization\n");
...
//    PurgeSpentCoins(8,1000);
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(out.fSpendable)
        {
            ...
            if(ExtractDestinationScriptValid(script1, addressRet))
            {
                ...
                if(lpKeyID)
                {
                    if (out.nDepth >= (((out.coin.m_Flags & FC_TFL_FROM_ME)) > 0 ? min_conf : ((min_conf == 0) ? 1 : min_conf)))
                    {
                    ...
                    if(lpKeyID)
                    {
                        ...
                        if(result)
                        {
                            LogPrintf("Combine transaction for address %s (%d inputs,%d outputs): %s; Time: %8.3fs\n",
                                    bitcoin_address.ToString().c_str(), wtx.vin.size(),wtx.vout.size(),wtx.GetHash().GetHex().c_str(),FC_TimeNowAsDouble()-start_time);
                        }
                    }
                }
            }
    ...
    nNextUnspentOptimization=FC_TimeNowAsUInt()+next_delay;
    return result;
}
