...
{
    LOCK(cs_wallet);
//    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)                          // Not supported as returns pointer
...
//    RandAddSeedPerfmon();                                                     // FCHN removed in 146c0a7 when merging sec256k1
    CKey secret;
...
string CWallet::SetDefaultKeyIfInvalid(std::string init_privkey)
{
    ...
    if(!vchSecret.SetString(init_privkey))
    {
        ...
        {
            return "Private key version is not set, please chose seed node running at least FloatChain 1.0 beta 2";            
        }
...
{
    set<uint256> result;
//    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)                            // Not supported as Spent array is no longer maintained
...
{
//    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)                          // Not supported as Spends array not maintained 
...
/* FCHN START */            
            ...
            {
                FC_TxDefRow txdef;
                if(pwalletTxsMain->FindWalletTx(wtx.GetHash(),&txdef) == FC_ERR_NOERROR)
                {
...
void CWallet::SyncTransaction(const CTransaction& tx, const CBlock* pblock)
{
    ...
    {
        LogPrint("FChn","FChn: Wallet optimization after block\n");
        fEmpty=true;
    }
...
FC_TxImport *StartImport(CWallet *lpWallet,bool fOnlyUnsynced, int block, int *err)
{
    vector <CBitcoinAddress> vAddressesToImport;
    vector <FC_TxEntity> vStreamsToImport;
    FC_TxEntity entity;
    FC_TxImport *imp;
    FC_Buffer *m_ChainEntities;
    ...
    *err=FC_ERR_NOERROR;
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, CAddressBookData)& item, lpWallet->mapAddressBook)
        {
            ...
            {
                entStat.Zero();
                meFCpy(&entStat,&entity,sizeof(FC_TxEntity));
                if(pwalletTxsMain->FindEntity(&entStat))
                {
                    if(!fOnlyUnsynced || ((entStat.m_Flags & FC_EFL_NOT_IN_SYNC) != 0) )
                    {
    ...
    if(m_ChainEntities)
    {
        ...
        {
            lpent=(FC_TxEntityStat*)m_ChainEntities->GetRow(i);
            switch(lpent->m_Entity.m_EntityType & FC_TET_TYPE_MASK)
            {
                ...
                case FC_TET_ASSET:
                    if(lpent->m_Entity.m_EntityType & FC_TET_CHAINPOS)
                    {
    ...
    FC_Buffer *lpEntities;
    lpEntities=new FC_Buffer();
    lpEntities->Initialize(sizeof(FC_TxEntity),sizeof(FC_TxEntity),FC_BUF_MODE_DEFAULT);
    if(vAddressesToImport.size())
    {
        ...
        for(unsigned int i=0;i<vAddressesToImport.size();i++)
        {
            ...
            if(lpKeyID)
            {
                ...
                entity.m_EntityType=FC_TET_PUBKEY_ADDRESS | FC_TET_CHAINPOS;
                lpEntities->Add(&entity,NULL);
                entity.m_EntityType=FC_TET_PUBKEY_ADDRESS | FC_TET_TIMERECEIVED;
                lpEntities->Add(&entity,NULL);
            }
            if(lpScriptID)
            {
                ...
                entity.m_EntityType=FC_TET_SCRIPT_ADDRESS | FC_TET_CHAINPOS;
                lpEntities->Add(&entity,NULL);
                entity.m_EntityType=FC_TET_SCRIPT_ADDRESS | FC_TET_TIMERECEIVED;
                lpEntities->Add(&entity,NULL);
            }        
        }
    }
    ...
    if(vStreamsToImport.size())
    {
        ...
        {     
            bool take_it=false;
            if( ( (vStreamsToImport[i].m_EntityType & FC_TET_TYPE_MASK) == FC_TET_ASSET ) &&
                ( FC_gState->m_Features->ShortTxIDInTx() == 0) )    
            {
                if(FC_gState->m_Assets->FindEntityByRef(&stream_entity,vStreamsToImport[i].m_EntityID))
                {
                    take_it=true;
                }   
            }
            ...
            {
                if(FC_gState->m_Assets->FindEntityByShortTxID(&stream_entity,vStreamsToImport[i].m_EntityID))
                {
                    take_it=true;
                }                
            }
//            if(FC_gState->m_Assets->FindEntityByShortTxID(&stream_entity,vStreamsToImport[i].m_EntityID))
            ...
            {
                entity.Zero();            
                meFCpy(entity.m_EntityID,vStreamsToImport[i].m_EntityID,FC_TDB_ENTITY_ID_SIZE);
                entity.m_EntityType=vStreamsToImport[i].m_EntityType;
                lpEntities->Add(&entity,NULL);
                entity.m_EntityType=(vStreamsToImport[i].m_EntityType - FC_TET_CHAINPOS) | FC_TET_TIMERECEIVED;
                lpEntities->Add(&entity,NULL);
                ptr=(unsigned char *)stream_entity.GetRef();
                if(stream_entity.IsUnconfirmedGenesis() == 0)
                {
                    b=(int)FC_GetLE(ptr,4)-1;
...
void CWallet::ReacceptWalletTransactions()
{
    ...
    {        
        pwalletTxsMain->Lock();
        LogPrint("FChn","ReacceptWalletTransactions: %ld txs in unconfirmed pool \n", pwalletTxsMain->m_UnconfirmedSends.size());
                    ...
                    if (!mempool.exists(wtxid))
                    {
                        ...
                        if(nDepth < 0)
                        {
                            ...
                            if(!wtx.AcceptToMemoryPool(false))
                            {
                                ...
                                pwalletTxsMain->SaveTxFlag((unsigned char*)&wtxid,FC_TFL_INVALID,1);
                            }
                        }
                        else
                        {
...
//    nNextResend = GetTime() + GetRand(30 * 60);
    nNextResend = GetTime() + GetRand(3 * FCP_TARGET_BLOCK_TIME);
/* FCHN END */    
    if (!fForce && fFirst)
        return;

    // Only do it if there's been a new block since last time
    if(GetTime() < nNextResend + 3 * FCP_TARGET_BLOCK_TIME )
    {
...
//            BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, pwalletTxsMain->m_UnconfirmedSends)
            BOOST_FOREACH(const uint256& wtxid, pwalletTxsMain->m_UnconfirmedSendsHashes) 
            {
                ...
                {                
                    ...
                    if (nTimeBestReceived - (int64_t)wtx.nTimeReceived > FCP_TARGET_BLOCK_TIME)
                    {
...
{
//    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)                          // Not supported, used only in QT
    CAmount nTotal = 0;
    {
        LOCK2(cs_main, cs_wallet);
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
...
//    std::map<COutPoint, FC_Coin> m_UTXOs[FC_TDB_MAX_IMPORTS];    
        ...
        if(fOnlyCoinsNoTxs)
        {
            for (map<COutPoint, FC_Coin>::const_iterator it = pwalletTxsMain->m_UTXOs[0].begin(); it != pwalletTxsMain->m_UTXOs[0].end(); ++it)
            {
                const FC_Coin& coin = it->second;
                if((addr == 0) || (addr == coin.m_EntityID))
                {
                    ...
                    {
                        if(coin.m_Flags & FC_TFL_IS_SPENDABLE)
                        {
                        ...
                        if( (coin.m_EntityType & FC_TET_TYPE_MASK) == FC_TET_SCRIPT_ADDRESS )
                        {
                            is_p2sh=true;
                        }
                    ...
                    if(flags & FC_CSF_ALLOW_NOT_SPENDABLE)
                    {
                        mine=ISMINE_SPENDABLE;                                
                    }
                    else
                    {
                        if(is_p2sh)
                        {
                            if((flags & FC_CSF_ALLOW_SPENDABLE_P2SH) == 0)
                            {
                                mine=ISMINE_NO;
                            }
                            else
                            {
                                if(flags & FC_CSF_ALLOW_NOT_SPENDABLE_P2SH)
                                {
                                    mine=ISMINE_SPENDABLE;
                                }
                            }
                    ...
                    if(flags & FC_CSF_ALLOWED_COINS_ARE_MINE)
                    {
                        if((mine & ISMINE_SPENDABLE) != ISMINE_NO)
                        {
                            coin_fixed=coin;
                            coin_fixed.m_Flags |= FC_TFL_IS_MINE_FOR_THIS_SEND;
                            use_fixed_coin=true;
                        }
                    }
...
//                    LogPrintf("DEBUG: %s\n",coin.ToString().c_str());
        ...
        else
        {
            ...
            {
                const FC_Coin& coin = vCoinsToTake[i];
                isminetype mine = IsMine(coin.m_TXOut);
                int nDepth=coin.GetDepthInMainChain();
                uint256 txid=coin.m_OutPoint.hash;
                uint32_t vout=coin.m_OutPoint.n;
...
/* FCHN END */        
        LOCK2(cs_main, cs_wallet);
/* FCHN START */        
        LogPrint("FChn","FChn: Wallet coins: Total: %d, Unspent: %d\n",mapWallet.size(),mapUnspent.size());
//        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
//        {
...
/* FCHN START */                    
                    ...
                    type_ored=0;
                    no_dust_check=FC_gState->m_TmpScript->IsOpReturnScript();
                    ...
                    if((type_ored == 0))// || (type_ored & FC_PTP_RECEIVE))
                    {
                        if(no_dust_check == 0)
                        {
...
//                        vchPubKey=vchDefaultKey;
                        ...
                        if(!GetKeyFromAddressBook(vchPubKey,FC_PTP_RECEIVE))
                        {
                            LogPrintf("FChn: Internal error: Cannot find address for change having receive permission\n");
                                ...
                                const set<CTxDestination>* addresses,int min_conf,int min_inputs,int max_inputs,const vector<COutPoint>* lpCoinsToUse, int *eErrorCode)
{
    ...
    return CreateFloatChainTransaction(vecSend, wtxNew, reservekey, nFeeRet, strFailReason, coinControl, addresses, min_conf, min_inputs, max_inputs, lpCoinsToUse, eErrorCode);
}
...
int CWallet::SelectFloatChainCombineCoinsMinConf(int nConfMine, int nConfTheirs, vector<COutput> vCoins, FC_Buffer *in_map, FC_Buffer *in_amounts,
                                ...
                                set<pair<const CWalletTx*,unsigned int> >& setCoinsRet,int max_inputs) const
{
    ...
    BOOST_FOREACH(const COutput &output, vCoins)
    {
        ...
        if(take_it)
        {
            if(FC_GetABCoinQuantity(in_amounts->GetRow(in_selected_row),coin_id))
            {
                take_it=false;                
            }            
        }
...
bool CWallet::SelectFloatChainCoinsMinConf(const CAmount& nTargetValue, int nConfMine, int nConfTheirs, vector<COutput> vCoins, FC_Buffer *in_map, FC_Buffer *in_amounts,
...
{
//    printf("SelectFloatChainCoinsMinConf\n");
    setCoinsRet.clear();
    nValueRet = 0;
...
//                printf("Not spendable\n");
    ...
    //                printf("Not enough confitmations\n");
            ...
            {
                if(output.coin.m_Flags & FC_TFL_IS_MINE_FOR_THIS_SEND)
                {
        ...
        //                printf("Not spendable\n");
                    ...
                    if (output.nDepth < (((output.coin.m_Flags & FC_TFL_FROM_ME) > 0) ? nConfMine : nConfTheirs))
                    {
        //                printf("Not enough confitmations\n");
                        take_it=false;
                    }                
...
//                printf("Already selected\n");
        ...
        {
            if(FC_GetABCoinQuantity(in_amounts->GetRow(in_preferred_row),coin_id) == 0)
            {
                take_it=false;                
            }            
        }
...
bool CWallet::SelectFloatChainCoins(const CAmount& nTargetValue, vector<COutput> &vCoins, FC_Buffer *in_map, FC_Buffer *in_amounts, 
                                int in_selected_row,int in_asset_row,int in_preferred_row,
    ...
    if (coinControl && coinControl->HasSelected())
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            if(out.fSpendable)
            {
                ...
                meFCpy(buf_map,&hash,32);                                       // Retrieving coin index in the matrix
                FC_PutLE(buf_map+32,&out_i,4);
                int row=in_map->Seek(buf_map);
...
//                    if(FC_GetLE(in_amounts->GetRow(in_selected_row)+coin_id*FC_AST_ASSET_REF_SIZE,FC_AST_ASSET_QUANTITY_SIZE) == 0)// BUG??? 
                    if(FC_GetABCoinQuantity(in_amounts->GetRow(in_selected_row),coin_id) == 0)
                    {
                                                                                // Value is take  from the matrix
                        nValueRet += FC_GetABCoinQuantity(in_amounts->GetRow(in_asset_row),coin_id);
    ...
    {
        if((SelectFloatChainCoinsMinConf(nTargetValue, 1, 6, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet) ||
            SelectFloatChainCoinsMinConf(nTargetValue, 1, 1, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet) ||
            (bSpendZeroConfChange && SelectFloatChainCoinsMinConf(nTargetValue, 0, 1, vCoins, in_map, in_amounts, in_selected_row, in_asset_row, preferred_row, setCoinsRet, nValueRet))))
        {
            return true;
        }
...
/* FCHN END */            
        ...
        if (!wtxNew.AcceptToMemoryPoolReturnReason(false,true,reject_reason))   // FCHN
        {
            // This must not fail. The transaction has already been signed and recorded.
            LogPrintf("CommitTransaction() : Error: Transaction not valid: %s\n",reject_reason.c_str());  // FCHN
            return false;
        }
        else
        {
...
bool CWallet::GetKeyFromAddressBook(CPubKey& result,uint32_t type,const set<CTxDestination>* addresses,map<uint32_t, uint256>* mapSpecialEntity)
{
    ...
    if((addresses == NULL) && (mapSpecialEntity == NULL))
    {
        ...
        perm=FC_gState->m_Permissions->GetAllPermissions(NULL,(unsigned char*)(&keyID),type);
        if(perm == type)
        {
            result=vchDefaultKey;
            return true;        
                ...
                if(perm == type)
                {
                    ...
                    if(mapSpecialEntity)
                    {
                        if(type & FC_PTP_ISSUE)
                        {
                            ...
                            take_it=false;
                            if(FC_gState->m_Permissions->CanIssue(lpEntity,(unsigned char*)(&keyID)))
                            {
                                take_it=true;
                            }                                                 
                        }
                        if(type & FC_PTP_WRITE)
                        {
                            unsigned char *lpEntity=NULL;
                            ...
                            take_it=false;
                            if(FC_gState->m_Permissions->CanWrite(lpEntity,(unsigned char*)(&keyID)))
                            {
                                take_it=true;
                            }                                                 
                        }
...
/* FCHN START */    
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        ...
        {
            meFCpy(entity.m_EntityID,&(it->first),FC_TDB_ENTITY_ID_SIZE);
            entity.m_EntityType=FC_TET_PUBKEY_ADDRESS | FC_TET_CHAINPOS;
...
/* FCHN END */    
        ...
        for (std::map<CKeyID, CBlockIndex*>::const_iterator it = mapKeyFirstBlock.begin(); it != mapKeyFirstBlock.end(); it++)
            mapKeyBirth[it->first] = it->second->GetBlockTime() - 12 * Params().TargetSpacing(); // FCHN block times can be 2h off
    }
...
{
    if((FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS) == 0)
    {
    ...
    {
        FC_TxDefRow txdef;
        if(pwalletTxsMain->FindWalletTx(GetHash(),&txdef) == FC_ERR_NOERROR)
        {
            if((txdef.m_Flags & FC_TFL_INVALID) == 0)
            {
                if(txdef.m_Block >= 0)
                {
