...
string JSONRPCRequestForLog(const string& strMethod, const Array& params, const Value& id)
{
    ...
    map<string, int>::iterator it = mapLogParaFCounts.find(strMethod);
    if (it != mapLogParaFCounts.end())
    {
...
/*    
    ...
    request.push_back(Pair("chain_name", string(FC_gState->m_Params->NetworkName())));
 */ 
...
/* FCHN START */    
    ...
    {
        if (dAmount < 0.0 || dAmount > (double)MAX_MONEY/(double)COIN)                                  // FCHN - was <=
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid amount");        
    }
    
//    if (dAmount < 0.0 || dAmount > 21000000.0)                                  // FCHN - was <=
//        throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid amount");
/* FCHN END */    
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "FloatChain server stopping";
}
...
uint32_t GetPausedServices(const char *str)
{
    ...
    while(ptr<=ptrEnd)
    {
        ...
        if( (c == ',') || (c ==0x00))
        {
            ...
            {
                type=0;
                if(meFCmp(start,"incoming",  ptr-start) == 0)type = FC_NPS_INCOMING;
                if(meFCmp(start,"mining",    ptr-start) == 0)type = FC_NPS_MINING;
...
Value pausecmd(const Array& params, bool fHelp)
{
    ...
    FC_gState->m_NodePausedState |= type;
...
Value resumecmd(const Array& params, bool fHelp)
{
    ...
    FC_gState->m_NodePausedState &= (FC_NPS_ALL ^ type);
...
/* FCHN START */    
    ...
    { "wallet",             "grantfrom",              &grantfroFCmd,           false,     false,      true }, 
    { "wallet",             "approvefrom",            &approvefrom,            false,     false,      true }, 
    { "wallet",             "revokefrom",             &revokefroFCmd,          false,     false,      true },
    { "wallet",             "issuefrom",              &issuefroFCmd,           false,     false,      true },
    { "wallet",             "issuemorefrom",          &issuemorefroFCmd,       false,     false,      true },
    ...
    { "wallet",             "createfrom",             &createfroFCmd,            false,     false,      true },
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use floatchaind, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
              "It is recommended you use the following random password:\n"
              "rpcuser=floatchainrpc\n"
              "rpcpassword=%s\n"
              "(you do not need to remember this password)\n"
              "The username and password MUST NOT be the same.\n"
              "If the file does not exist, create it with owner-readable-only file permissions.\n"
              "It is also recommended to set alertnotify so you are notified of problems;\n"
              "for example: alertnotify=echo %%s | mail -s \"FloatChain Alert\" admin@foo.com\n"),
...
/* FCHN START */    
    ...
    if (valChainName.type() != null_type)
    {
        ...
        if (strcmp(valChainName.get_str().c_str(),FC_gState->m_Params->NetworkName()))
...
static Object JSONRPCExecOne(const Value& req)
{
    ...
    JSONRequest jreq;
/* FCHN START */    
    uint32_t wallet_mode=FC_gState->m_WalletMode;
/* FCHN END */    
    try {
        jreq.parse(req);
    ...
    {
/* FCHN START */    
        FC_gState->m_WalletMode=wallet_mode;
        if(fDebug)LogPrint("FCapi","FCapi: API request failure A\n");        
/* FCHN END */    
...
/* FCHN START */    
        FC_gState->m_WalletMode=wallet_mode;
        if(fDebug)LogPrint("FCapi","FCapi: API request failure B\n");        
/* FCHN END */    
...
/* FCHN START */    
    uint32_t wallet_mode=FC_gState->m_WalletMode;
    if(fDebug)LogPrint("FCapi","FCapi: API request from %s\n",conn->peer_address_to_string().c_str());
/* FCHN END */    
    try
    {
        // Parse request
...
/* FCHN START */    
        FC_gState->m_WalletMode=wallet_mode;
        if(fDebug)LogPrint("FCapi","FCapi: API request failure: %s, code: %d\n",jreq.strMethod.c_str(),find_value(objError, "code").get_int());
        
//        if(fDebug)LogPrint("FCapi","FCapi: API request failure C\n");        
/* FCHN END */    
...
/* FCHN START */    
        FC_gState->m_WalletMode=wallet_mode;
        if(fDebug)LogPrint("FCapi","FCapi: API request failure D\n");        
/* FCHN END */    
...
json_spirit::Value CRPCTable::execute(const std::string &strMethod, const json_spirit::Array &params) const
{
    ...
    if(FC_gState->m_ProtocolVersionToUpgrade > FC_gState->m_NetworkParams->ProtocolVersion())
    {
        ...
        {
            throw JSONRPCError(RPC_UPGRADE_REQUIRED, strprintf("BlockChain was upgraded to protocol version %d, please upgrade FloatChain",FC_gState->m_ProtocolVersionToUpgrade));
        }
    }
    ...
    try
    {
        ...
        if(fDebug)
        {
            ...
            LogPrint("FCapi","FCapi: API request: %s\n",strRequest.c_str());
        }
...
/* FCHN START */        
        if(fDebug)LogPrint("FCapi","FCapi: API request successful: %s\n",strMethod.c_str());
/* FCHN END */        
        return result;
    }
    catch (std::exception& e)
    {
        if(fDebug)LogPrint("FCapi","FCapi: API request failure: %s\n",strMethod.c_str());
        if(strcmp(e.what(),"Help message not found\n") == 0)
        {
            throw JSONRPCError(RPC_MISC_ERROR, FC_RPCHelpString(strMethod).c_str());
        }
...
std::string HelpExampleCli(string methodname, string args){
    return "> floatchain-cli " + std::string(FC_gState->m_NetworkParams->Name()) + " " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(string methodname, string args){
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + std::string(FC_gState->m_NetworkParams->Name()) + " " + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:"+
            strprintf("%d",(int)FC_gState->m_NetworkParams->GetInt64Param("defaultrpcport")) + "\n";// FCHN was hard-coded 8332 before
}

const CRPCTable tableRPC;
std::map<std::string, std::string> mapHelpStrings;
std::map<std::string, int> mapLogParaFCounts;
