...
Value getnewaddress(const Array& params, bool fHelp)
{
    ...
    if (params.size() > 0)
    {
        ...
        {
            throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need accounts, run floatchaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
        }            
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        FC_TxEntity entity;
        entity.Zero();
        meFCpy(entity.m_EntityID,&keyID,FC_TDB_ENTITY_ID_SIZE);
...
{
    if (fHelp || params.size() < 1 || params.size() > 2)                        // FCHN
        throw runtime_error("Help message not found\n");
...
void SendMoney(const CTxDestination &address, CAmount nValue, CWalletTx& wtxNew,FC_Script *dropscript,FC_Script *opreturnscript) // FCHN
{
    // Check amount
/* FCHN START */    
//    if (nValue <= 0)
...
/* FCHN START */    
    ...
    if(dropscript)
    {
        if(fDebug)LogPrint("FChnminor","FChn: Sending script with %d OP_DROP element(s)",dropscript->GetNumElements());
        if(dropscript->GetNumElements() > FCP_STD_OP_DROP_COUNT )
...
Value listaddresses(const Array& params, bool fHelp)
{
    ...
    for(int i=0;i<entity_count;i++)
    {
        ...
        if((lpEntity->m_Entity.m_EntityType == (FC_TET_PUBKEY_ADDRESS | FC_TET_CHAINPOS)) || 
           (lpEntity->m_Entity.m_EntityType == (FC_TET_SCRIPT_ADDRESS | FC_TET_CHAINPOS)))
        {
    ...
    FC_AdjustStartAndCount(&count,&start,address_count);
...
{
    if (fHelp || params.size() < 2 || params.size() > 4)                        // FCHN
        throw runtime_error("Help message not found\n");
    ...
    elem = FC_gState->m_TmpScript->GetData(FC_gState->m_TmpScript->GetNumElements()-1,&elem_size);
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Not supported with scalable wallet - if you need accounts, run floatchaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }            
...
Value getbalance(const Array& params, bool fHelp)
{
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need getbalance, run floatchaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
    string strAccount = AccountFromValue(params[0]);
...
Value movecmd(const Array& params, bool fHelp)
{
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need move, run floatchaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
...
{
    if (fHelp || params.size() < 3 || params.size() > 6)                        // FCHN 
        throw runtime_error("Help message not found\n");

    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need sendfrom, run floatchaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need sendmany, run floatchaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }
...
{
    if (fHelp || params.size() < 2 || params.size() > 3)                        // FCHN
    {
        throw runtime_error("Help message not found\n");
    }
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        FC_TxEntity entity;
        entity.Zero();
        meFCpy(entity.m_EntityID,&innerID,FC_TDB_ENTITY_ID_SIZE);
...
void ListTransactions(const CWalletTx& wtx, const string& strAccount, int nMinDepth, bool fLong, Array& ret, const isminefilter& filter, FC_TxDefRow *txdef)
{
    CAmount nFee;
    string strSentAccount;
    list<COutputEntry> listReceived;
...
Value listtransactions(const Array& params, bool fHelp)
{
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Not supported with scalable wallet - if you need listtransactions, run floatchaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
/*
        ...
        FC_TxEntity wallet_by_time;
        FC_TxEntityRow *lpEntTx;
        wallet_by_time.Zero();
        wallet_by_time.m_EntityType=FC_TET_TIMERECEIVED;
        if(filter & ISMINE_WATCH_ONLY)
        {
            wallet_by_time.m_EntityType |= FC_TET_WALLET_ALL;
        }
        else
        {
            wallet_by_time.m_EntityType |= FC_TET_WALLET_SPENDABLE;            
        }
...
Value listaccounts(const Array& params, bool fHelp)
{
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "Accounts are not supported with scalable wallet - if you need listaccounts, run floatchaind -walletdbversion=1 -rescan, but the wallet will perform worse");        
    }   
...
Value listsinceblock(const Array& params, bool fHelp)
{
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        ...
        wallet_entity.Zero();
        wallet_entity.m_EntityType=FC_TET_WALLET_SPENDABLE | FC_TET_CHAINPOS;
        if(filter & ISMINE_WATCH_ONLY)
        {
        ...
        if(tx_count)
        {
            ...
            while(up_tx-down_tx>chunk_size)
            {
                ...
                {
                    entrow=(FC_TxEntityRow *)(lpEntRowBuffer->GetRow(0));
            ...
            while(this_tx<=tx_count)
            {
                ...
                if( (err == FC_ERR_NOERROR) && (lpEntRowBuffer->GetCount() > 0) )
                {
                    ...
                    {
                        entrow=(FC_TxEntityRow*)lpEntRowBuffer->GetRow(i);
...
Value backupwallet(const Array& params, bool fHelp)
{
    ...
    char bufOutput[FC_DCT_DB_MAX_PATH+1];
    char *full_path=__US_FullPath(params[0].get_str().c_str(),bufOutput,FC_DCT_DB_MAX_PATH+1);
    
    if(full_path)
    {
        char bufWallet[FC_DCT_DB_MAX_PATH+1];
        FC_GetFullFileName(FC_gState->m_NetworkParams->Name(),"wallet",".dat",FC_FOM_RELATIVE_TO_DATADIR,bufWallet);
...
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))              // FCHN
        throw runtime_error(
...
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))             // FCHN
        throw runtime_error("Help message not found\n");
...
{
    if (fHelp || params.size() < 1 || params.size() > 1)                        // FCHN
        throw runtime_error("Help message not found\n");
