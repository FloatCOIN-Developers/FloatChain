...
void WalletTxToJSON(const CWalletTx& wtx, Object& entry,bool skipWalletConflicts, int vout)
{
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        ...
        if(wtx.txDef.m_Flags & FC_TFL_INVALID)
        {
        ...
        {
            if((item.first.size() <13) || (meFCmp(item.first.c_str(),"fullsize",8) != 0))
            {
...
//void SendMoneyToSeveralAddresses(const std::vector<CTxDestination> addresses, CAmount nValue, CWalletTx& wtxNew,FC_Script *dropscript,FC_Script *opreturnscript,const std::vector<CTxDestination>& fromaddresses)
...
void FindAddressesWithPublishPermission(vector<CTxDestination>& fromaddresses,FC_EntityDetails *stream_entity)
{
    if(fromaddresses.size() == 1)
    {
        ...
        if(aptr)
        {
            if((stream_entity->AnyoneCanWrite() == 0) && (FC_gState->m_Permissions->CanWrite(stream_entity->GetTxID(),aptr) == 0))
            {
    ...
    else
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
        {
            ...
            {
                if((stream_entity->AnyoneCanWrite() != 0) || (FC_gState->m_Permissions->CanWrite(stream_entity->GetTxID(),(unsigned char*)(&keyID)) != 0))
                {
                    if(FC_gState->m_Permissions->CanSend(NULL,(unsigned char*)(&keyID)))
                    {
...
bool CBitcoinAddressFromTxEntity(CBitcoinAddress &address,FC_TxEntity *lpEntity)
{
    ...
    if((lpEntity->m_EntityType & FC_TET_TYPE_MASK) == FC_TET_PUBKEY_ADDRESS)
    {            
        unsigned char *ptr=lpEntity->m_EntityID;
        dest=CKeyID(*(uint160*)ptr);
        is_address=true; 
    }
    if((lpEntity->m_EntityType & FC_TET_TYPE_MASK) == FC_TET_SCRIPT_ADDRESS)
    {
        unsigned char *ptr=lpEntity->m_EntityID;
...
Object StreamItemEntry(const CWalletTx& wtx,const unsigned char *stream_id, bool verbose)
{
    ...
    for (int j = 0; j < (int)wtx.vout.size(); ++j)
    {
        if(stream_output < 0)
        {
            ...
            {
                if(FC_gState->m_TmpScript->GetNumElements()) // 2 OP_DROPs + OP_RETURN - item key
                {
                    ...
                    {
                        if(meFCmp(short_txid,stream_id,FC_AST_SHORT_TXID_SIZE) == 0)
                        {
                            stream_output=j;
...
//    int iad=-1;
    ...
    if(fIsFound)
    {
        ...
        int required=0;
        if(ParseFloatchainTxOutToBuffer(hash,txout,amounts,lpScript,NULL,&required,strFailReason))
        {
