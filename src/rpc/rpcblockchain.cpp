...
/* FCHN START */
bool ParseFloatchainTxOutToBuffer(uint256 hash,const CTxOut& txout,FC_Buffer *amounts,FC_Script *lpScript,int *allowed,int *required,string& strFailReason);
vector<int> ParseBlockSetIdentifier(Value blockset_identifier);
bool CreateAssetBalanceList(const CTxOut& out,FC_Buffer *amounts,FC_Script *lpScript);
...
/* FCHN START */    
    ...
    {
        if(FC_gState->m_Permissions->GetBlockMiner(blockindex->nHeight,(unsigned char*)&keyID) == FC_ERR_NOERROR)
        {
...
Value getblockhash(const Array& params, bool fHelp)
{
    ...
    int64_t nHeight = params[0].get_int64();                                    // FCHN - was int
...
Value clearmempool(const Array& params, bool fHelp)
{
    ...
    uint32_t required_paused_state=FC_NPS_INCOMING | FC_NPS_MINING;
    if((FC_gState->m_NodePausedState & required_paused_state) != required_paused_state)
    {
...
Value gettxout(const Array& params, bool fHelp)
{
    ...
    std::string strHash = params[0].get_str();
    uint256 hash(strHash);
    int64_t n = params[1].get_int64();                                          // FCHN was int
    bool fMempool = true;
    if (params.size() > 2)
