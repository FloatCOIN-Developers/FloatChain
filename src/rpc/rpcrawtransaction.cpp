...
/* FCHN START */    
// TODO too many duplicate code with ListWalletTransactions and may be AccepFloatChainTransaction
...
//            int e=FC_gState->m_TmpScript->GetNumElements()-1;
            int e=lpScript->GetNumElements()-1;
            {
                if(FC_gState->m_Features->OpDropDetailsScripts())
                {
                    ...
                    if((err == 0) && (new_entity_type == FC_ENT_TYPE_ASSET))
                    {
                        if(asset_update == 0)
                        {
            ...
            else
            {
                ...
                {
                    cs_offset=0;
                    while( (cs_err=lpScript->GetCachedScript(cs_offset,&cs_new_offset,&cs_vin,&cs_script,&cs_size)) == FC_ERR_NOERROR )
                    {
                        if(cs_offset)
                        {
                            Object cs_obj;
        ...
        if(CreateAssetBalanceList(txout,asset_amounts,lpScript,&required))
        {
            ...
            for(int a=0;a<asset_amounts->GetCount();a++)
            {
                ...
                if(is_valid_asset)
                {
                    asset_entry=AssetEntry(txid,FC_GetABQuantity(ptr),0x05);
                    if(is_genesis)
                    {
                        asset_entry.push_back(Pair("type", "issuefirst"));       
                        issuerawqty+=FC_GetABQuantity(ptr);
                        is_issuefirst=true;
                    }
                    else
                    {
...
/* FCHN END */    
    ...
    if(is_issuefirst || is_issuemore)
    {
        ...
        if(detals_script_found)
        {
            ...
            {
                next_offset=FC_GetParamFromDetailsScript(details_script,details_script_size,offset,&param_value_start,&param_value_size);
                if(param_value_start > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            case FC_ENT_SPRM_FOLLOW_ONS:
                                if( (param_value_size > 0) && (param_value_size <= 4) )
                                {
                                    if(FC_GetLE(details_script+param_value_start,param_value_size))
                                    {
                                        is_open=true;
                                    }
                                }
            ...
            if(is_issuemore)
            {
                ...
                if(FC_gState->m_Assets->FindEntityByShortTxID(&genesis_entity,short_txid))
                {
...
/* FCHN START */        
    ...
    
    {
        ...
        pwalletMain->nNextUnspentOptimization=FC_TimeNowAsUInt()+GetArg("-autocombinesuspend", 15);        
    }
...
Value appendrawchange(const Array& params, bool fHelp)
{
    ...
    FC_InitABufferMap(asset_amounts);
    FC_Buffer *amounts;
    amounts=new FC_Buffer;
    FC_InitABufferMap(amounts);
    ...
    {
        if(ParseFloatchainTxOutToBuffer(0,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
    ...
    {
        uint64_t quantity=FC_GetABQuantity(asset_amounts->GetRow(i));
        quantity=-quantity;
        FC_SetABQuantity(asset_amounts->GetRow(i),quantity);        
    }
    ...
    {
        if(ParseFloatchainTxOutToBuffer(tx.vin[i].prevout.hash,txout,asset_amounts,lpScript,&allowed,&required,strError))
        {
            nAmount+=txout.nValue;
        }
        i++;
    ...
    for(int i=0;i<asset_amounts->GetCount();i++)
    {
        if(FC_GetABRefType(asset_amounts->GetRow(i)) == FC_AST_ASSET_REF_TYPE_GENESIS)
//        if((int)FC_GetLE(asset_amounts->GetRow(i)+4,4)<0)
        {
            throw JSONRPCError(RPC_UNCONFIRMED_ENTITY, "Unconfirmed issue transaction in input");        
        }
        if(FC_GetABRefType(asset_amounts->GetRow(i)) != FC_AST_ASSET_REF_TYPE_SPECIAL)
//        if((int)FC_GetLE(asset_amounts->GetRow(i),4)>0)          
        {
            int64_t quantity=FC_GetABQuantity(asset_amounts->GetRow(i));
            if(quantity < 0)
            {
    ...
    {
        int assets_per_opdrop=(FCP_STD_OP_DROP_SIZE-4)/(FC_gState->m_NetworkParams->m_AssetRefSize+FC_AST_ASSET_QUANTITY_SIZE);
    ...
    if(amounts->GetCount())
    {
        lpScript->SetAssetQuantities(amounts,FC_SCR_ASSET_SCRIPT_TYPE_TRANSFER);
    ...
    for(int i=0;i<(int)script_for_cache.size();i++)
    {
        if( (cache_array[i] < 0) && fRequired)
        {
            ...
            if(ExtractDestinationScriptValid(script_for_cache[i], addressRet))
            {
                ...
                if(aptr)
                {
                    if(FC_gState->m_Permissions->CanAdmin(NULL,aptr))
                    {
                        cache_array[i]=1;
                    }                                                                     
                }
...
Value appendrawtransaction(const Array& params, bool fHelp)
{
    ...
    if(params[0].get_str().size())
    {
        ...
        {                                                           
            ...
            uint32_t disallowed=FC_CheckSigScriptForMutableTx((unsigned char*)(&pc2[0]),(int)(script2.end()-pc2));        
    ...
    if( required & (FC_PTP_ADMIN | FC_PTP_MINE) )
    {
        if(FC_gState->m_Features->CachedInputScript())
        {
    ...
    {
        FindFollowOnsInScript(rawTx.vout[i].scriptPubKey,FC_gState->m_TmpAssetsOut,FC_gState->m_TmpScript);
    }
    entity.Zero();
 ...
 */  
        if(!FC_gState->m_Assets->FindEntityByFullRef(&entity,FC_gState->m_TmpAssetsOut->GetRow(0)))
        {
...
Value appendrawmetadata(const json_spirit::Array& params, bool fHelp)
{
    ...
    {
        FindFollowOnsInScript(tx.vout[i].scriptPubKey,FC_gState->m_TmpAssetsOut,FC_gState->m_TmpScript);
    }
    entity.Zero();
    if(FC_gState->m_TmpAssetsOut->GetCount())
    {
...
Value disablerawtransaction(const Array& params, bool fHelp)
{
    ...
    {
        throw JSONRPCError(RPC_NOT_SUPPORTED, "disablerawtransaction supported only for protocol=floatchain");
    }    
    ...
    for(int i=0;i<(int)tx.vin.size();i++)
    {
        ...
        if(FindPreparedTxOut(preparedTxOut,tx.vin[i].prevout,strError))
        {
            if( (pwalletMain->IsMine(preparedTxOut) & ISMINE_SPENDABLE) != ISMINE_NO )
            {
                ...
                if(FC_gState->m_Permissions->CanSend(NULL,(unsigned char*)(lpKeyID)) == 0)
                {
                    take_it=false;
                }

                if(FC_gState->m_Permissions->CanReceive(NULL,(unsigned char*)(lpKeyID)) == 0)
                {
                    take_it=false;
                }
