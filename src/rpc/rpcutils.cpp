...
bool AssetRefDecode(unsigned char *bin, const char* string, const size_t stringLen)
{
    ...
    meFCpy(buffer, string, stringLen);
    ...
    FC_PutLE(bin+0,&blockNum,4);
    FC_PutLE(bin+4,&txOffset,8);
...
uint256 FC_GenesisCoinbaseTxID()
{
...
{
    int ret=FC_ASSET_KEY_VALID;
    int size=strlen(asset_key);
    unsigned char buf[FC_AST_ASSET_REF_SIZE];
    FC_EntityDetails entity;
    const unsigned char *ptr;
        
    if(size == 0)
    {
        return FC_ASSET_KEY_INVALID_EMPTY;
    }
    ...
    if(size == 64)
    {
        if(type)
        {
            *type=FC_ENT_KEYTYPE_TXID;
        }
        hash.SetHex(asset_key);
        if(!FC_gState->m_Assets->FindEntityByTxID(&entity,(unsigned char*)&hash))
        {
            ret=FC_ASSET_KEY_INVALID_TXID;
        }
        else
        {
            if( (entity_type != FC_ENT_TYPE_ANY) && ((int)entity.GetEntityType() != entity_type) )
            {
                ret=FC_ASSET_KEY_INVALID_TXID;
            }
            else
            {
                ...
                {
                    if(hash == FC_GenesisCoinbaseTxID())
                    {
                        ret=FC_ASSET_KEY_INVALID_TXID;                            
                    }
                }
            ...
            {
                if(!FC_gState->m_Assets->FindEntityByFollowOn(&entity,(unsigned char*)&hash))
                {
                    ret=FC_ASSET_KEY_INVALID_TXID;
                }                
            }
    ...
    {
        if(size<=FC_ENT_MAX_NAME_SIZE)
        {
            if(AssetRefDecode(buf,asset_key,size))
            {
                if(type)
                {
                    *type=FC_ENT_KEYTYPE_REF;
                }
                if(!FC_gState->m_Assets->FindEntityByRef(&entity,buf))
                {
                    ret=FC_ASSET_KEY_INVALID_REF;
                }
                else
                {
                    if( (entity_type != FC_ENT_TYPE_ANY) && ((int)entity.GetEntityType() != entity_type) )
            ...
            else
            {
                if(type)
                {
                    *type=FC_ENT_KEYTYPE_NAME;
                }
                if(!FC_gState->m_Assets->FindEntityByName(&entity,(char*)asset_key))
                {
                    ret=FC_ASSET_KEY_INVALID_NAME;
                }    
                else
                {
                    if( (entity_type != FC_ENT_TYPE_ANY) && ((int)entity.GetEntityType() != entity_type) )
        ...
        {
            ret=FC_ASSET_KEY_INVALID_SIZE;
        }
    }

    if(ret == FC_ASSET_KEY_VALID)
    {
        if(txid)
        {
            meFCpy(txid,entity.GetTxID(),32);
        }
        ptr=entity.GetRef();
        if(entity.IsUnconfirmedGenesis())
        {
            ret=FC_ASSET_KEY_UNCONFIRMED_GENESIS;                            
        }
        if(asset_ref)
        {
            meFCpy(asset_ref,ptr,FC_AST_ASSET_REF_SIZE);            
        }
        if(name)
        {
...
int ParseAssetKeyToFullAssetRef(const char* asset_key,unsigned char *full_asset_ref,int *multiple,int *type,int entity_type)
{
    ...
    {
        unsigned char txid[FC_ENT_KEY_SIZE];
        ret=ParseAssetKey(asset_key,txid,NULL,NULL,multiple,type,entity_type);
        if(ret == FC_ASSET_KEY_UNCONFIRMED_GENESIS)
        {
            ret=0;
        }
        meFCpy(full_asset_ref+FC_AST_SHORT_TXID_OFFSET,txid+FC_AST_SHORT_TXID_OFFSET,FC_AST_SHORT_TXID_SIZE);
        
        FC_SetABRefType(full_asset_ref,FC_AST_ASSET_REF_TYPE_SHORT_TXID);        
    }
    else
    {
...
Array AddressEntries(const CTxIn& txin,txnouttype& typeRet,FC_Script *lpScript)
{
    Array addrs;
    ...
    lpScript->Clear();
    lpScript->SetScript((unsigned char*)(&pc2[0]),(size_t)(script2.end()-pc2),FC_SCR_TYPE_SCRIPTSIGRAW);
...
Value PermissionForFieldEntry(FC_EntityDetails *lpEntity)
{
    if(lpEntity->GetEntityType())
    {
        ...
        if(lpEntity->GetEntityType() == FC_ENT_TYPE_ASSET)
        {
            ...
            {
                assetref += itostr((int)FC_GetLE(ptr,4));
                assetref += "-";
                assetref += itostr((int)FC_GetLE(ptr+4,4));
                assetref += "-";
                assetref += itostr((int)FC_GetLE(ptr+8,2));
        ...
        if(lpEntity->GetEntityType() == FC_ENT_TYPE_STREAM)
        {
            ...
            {
                if((int)FC_GetLE(ptr,4))
                {
                    streamref += itostr((int)FC_GetLE(ptr,4));
                    streamref += "-";
                    streamref += itostr((int)FC_GetLE(ptr+4,4));
                    streamref += "-";
                    streamref += itostr((int)FC_GetLE(ptr+8,2));
                }
                else
                {
                    streamref="0-0-0";                
...
Array PermissionEntries(const CTxOut& txout,FC_Script *lpScript,bool fLong)
{
    Array results;
    ...
    for (int e = 0; e < lpScript->GetNumElements(); e++)
    {
        ...
        else                                                        // Not entity element
        {
            ...
            {                
                ...
                full_type=FC_gState->m_Permissions->GetPossiblePermissionTypes(entity.GetEntityType());
                if(full_type & FC_PTP_CONNECT)entry.push_back(Pair("connect", (type & FC_PTP_CONNECT) ? true : false));
                if(full_type & FC_PTP_SEND)entry.push_back(Pair("send", (type & FC_PTP_SEND) ? true : false));
                if(full_type & FC_PTP_RECEIVE)entry.push_back(Pair("receive", (type & FC_PTP_RECEIVE) ? true : false));
                if(FC_gState->m_Features->Streams())
                {
                    if(full_type & FC_PTP_WRITE)entry.push_back(Pair("write", (type & FC_PTP_WRITE) ? true : false));                
                    if(full_type & FC_PTP_CREATE)entry.push_back(Pair("create", (type & FC_PTP_CREATE) ? true : false));                
                }
                if(full_type & FC_PTP_ISSUE)entry.push_back(Pair("issue", (type & FC_PTP_ISSUE) ? true : false));
                if(full_type & FC_PTP_MINE)entry.push_back(Pair("mine", (type & FC_PTP_MINE) ? true : false));
                if(full_type & FC_PTP_ADMIN)entry.push_back(Pair("admin", (type & FC_PTP_ADMIN) ? true : false));
                if(full_type & FC_PTP_ACTIVATE)entry.push_back(Pair("activate", (type & FC_PTP_ACTIVATE) ? true : false));                
...
// 0x0020 creators    
    ...
    if(FC_gState->m_Assets->FindEntityByTxID(&entity,txid))
    {        
        ...
        if(output_level & 0x0004)
        {
            ...
            while(offset>=0)
            {
                ...
                if(value_offset > 0)
                {
                    ...
                    {
                        if(ptr[offset+1] == FC_ENT_SPRM_ISSUER)
                        {
                            if(value_size == 24)
                            {
                                unsigned char tptr[4];
                                meFCpy(tptr,ptr+value_offset+sizeof(uint160),4);
                                if(FC_GetLE(tptr,4) & FC_PFL_IS_SCRIPTHASH)
                                {
        ...
        {
            entStat.Zero();
            meFCpy(&entStat,entity.GetTxID()+FC_AST_SHORT_TXID_OFFSET,FC_AST_SHORT_TXID_SIZE);
            entStat.m_Entity.m_EntityType=FC_TET_STREAM | FC_TET_CHAINPOS;
            if(pwalletTxsMain->FindEntity(&entStat))
            {
                ...
                if(output_level & 0x0010)
                {
                    ...
                    tmp_entity.Zero();
                    meFCpy(tmp_entity.m_EntityID,entity.GetTxID()+FC_AST_SHORT_TXID_OFFSET,FC_AST_SHORT_TXID_SIZE);
                    tmp_entity.m_EntityType=FC_TET_STREAM_KEY | FC_TET_CHAINPOS;
                    entry.push_back(Pair("keys",pwalletTxsMain->GetListSize(&tmp_entity,NULL)));        
                    tmp_entity.m_EntityType=FC_TET_STREAM_PUBLISHER | FC_TET_CHAINPOS;
...
Value DataItemEntry(const CTransaction& tx,int n,set <uint256>& already_seen,uint32_t stream_output_level)
{
    ...
    Array items;
    const unsigned char *ptr;
    unsigned char item_key[FC_ENT_MAX_ITEM_KEY_SIZE+1];
    int item_key_size;
    Value item_value;
    FC_EntityDetails entity;
    uint256 hash;
    ...
    if(FC_gState->m_TmpScript->GetNumElements() == 0)
    {
        return Value::null;
    }
    ...
    if(FC_gState->m_TmpScript->GetEntity(short_txid))           
    {
        return Value::null;
    }
    ...
    elem = FC_gState->m_TmpScript->GetData(2,&elem_size);
...
// 0x0080 put issueqty into qty field
    Object entry;
    FC_EntityDetails entity;
    FC_EntityDetails genesis_entity;
    FC_TxEntityStat entStat;
    unsigned char *ptr;
    ...
    if(FC_gState->m_Assets->FindEntityByTxID(&entity,txid))
    {
        if(entity.IsFollowOn())
        {
            FC_gState->m_Assets->FindEntityByFollowOn(&genesis_entity,txid);
        }
        else
        {
            meFCpy(&genesis_entity,&entity,sizeof(FC_EntityDetails));
        }
        ...
        Array issues;
        int64_t total=0;
        if(( (output_level & 0x0020) !=0 )|| FC_gState->m_Assets->HasFollowOns(txid))
        {
            int64_t qty;
            FC_Buffer *followons;
            followons=FC_gState->m_Assets->GetFollowOns(txid);
            for(int i=followons->GetCount()-1;i>=0;i--)
            {
                Object issue;
                FC_EntityDetails followon;
                if(FC_gState->m_Assets->FindEntityByTxID(&followon,followons->GetRow(i)))
                {
            ...
            FC_gState->m_Assets->FreeEntityList(followons);
        }
        else
        {
        ...
        if( ((output_level & 0x0008) != 0) && ((FC_gState->m_WalletMode & FC_WMD_TXS) != 0) )
        {
            entStat.Zero();
            meFCpy(&entStat,genesis_entity.GetShortRef(),FC_gState->m_NetworkParams->m_AssetRefSize);
            entStat.m_Entity.m_EntityType=FC_TET_ASSET | FC_TET_CHAINPOS;
            if(pwalletTxsMain->FindEntity(&entStat))
            {
                entry.push_back(Pair("subscribed",true));                                            
                if(entStat.m_Flags & FC_EFL_NOT_IN_SYNC)
...
string ParseRawOutputObject(Value param,CAmount& nAmount,FC_Script *lpScript, int *required,int *eErrorCode)
{
    string strError="";
    unsigned char buf[FC_AST_ASSET_FULLREF_BUF_SIZE];
    FC_Buffer *lpBuffer;
    FC_Buffer *lpFollowonBuffer;
    lpBuffer=new FC_Buffer;
    lpFollowonBuffer=new FC_Buffer;
    int assets_per_opdrop=(MAX_STANDARD_TX_SIZE)/(FC_gState->m_NetworkParams->m_AssetRefSize+FC_AST_ASSET_QUANTITY_SIZE);
    ...
    FC_InitABufferDefault(lpFollowonBuffer);
            ...
            if(!parsed && (a.name_ == "issuemore"))
            {
                ...
                BOOST_FOREACH(const Pair& d, a.value_.get_obj()) 
                {
                    ...
                    {                 
                        ...
                        if(asset_name.size())
                        {
                            asset_error=ParseAssetKeyToFullAssetRef(asset_name.c_str(),buf,&multiple,NULL, FC_ENT_TYPE_ASSET);
                            if(asset_error)
                            {
                                goto exitlbl;
                            }                                                    
                ...
                if(lpFollowonBuffer->GetCount())
                {
                    ...
                    {
                        if(meFCmp(buf,lpFollowonBuffer->GetRow(0),FC_AST_ASSET_QUANTITY_OFFSET))
                        {
                ...
                lpScript->SetAssetQuantities(lpFollowonBuffer,FC_SCR_ASSET_SCRIPT_TYPE_FOLLOWON);                
                parsed=true;
            }
            ...
            if(!parsed && (a.name_ == "permissions"))
            {
                ...
                BOOST_FOREACH(const Pair& d, a.value_.get_obj()) 
                {
                    ...
                    {                 
                        ...
                        if(d.value_.type() != null_type && !d.value_.get_str().empty())
                        {
                            ParseEntityIdentifier(d.value_,&entity, FC_ENT_TYPE_ANY);           
                        }
                        field_parsed=true;
                    }                    
                ...
                if(type_string.size())
                {
                    type=FC_gState->m_Permissions->GetPermissionType(type_string.c_str(),entity.GetEntityType());
                    if(entity.GetEntityType() == FC_ENT_TYPE_NONE)
                    {
                        if(required)
                        {
                            *required |= type;
                ...
                if(entity.GetEntityType())                
                {
                    lpScript->SetEntity(entity.GetTxID()+FC_AST_SHORT_TXID_OFFSET);                            
                }
        ...
        else
        {
            if(a.name_.size())
            {
                ...
                if(verify_level & 0x0001)
                {
                    ...
                    {
                        lpScript->SetAssetQuantities(lpBuffer,FC_SCR_ASSET_SCRIPT_TYPE_TRANSFER);                
                        lpBuffer->Clear();
                    }
                }
            }            
    ...
    if(verify_level & 0x0004)
    {
        if(Params().RequireStandard())
        {
            if(lpScript->GetNumElements() > FCP_STD_OP_DROP_COUNT)
            {
...
exitlbl:
    ...
    switch(asset_error)
    {
        ...
        case FC_ASSET_KEY_INVALID_SIZE:
            ...
            break;
        case FC_ASSET_KEY_UNCONFIRMED_GENESIS:
            if(eErrorCode)
            {
...
string ParseRawOutputObject(Value param,CAmount& nAmount,FC_Script *lpScript,int *eErrorCode)
{
...
CScript ParseRawMetadata(Value param,uint32_t allowed_objects,FC_EntityDetails *given_entity,FC_EntityDetails *found_entity)
{
...
//        bool value_is_set=false;
        ...
        {              
            ...
            {
                new_type=-3;
                if( ((allowed_objects & 0x0200) == 0) || (FC_gState->m_Features->CachedInputScript() == 0) )
                {
            ...
            {                 
                ...
                if(d.value_.type() != null_type && !d.value_.get_str().empty())
                {
                    ParseEntityIdentifier(d.value_,&entity, FC_ENT_TYPE_STREAM);       
                    if(found_entity)
                    {
                        meFCpy(found_entity,&entity,sizeof(FC_EntityDetails));
                    }                        
                }                
                new_type=-1;
            ...
            {                 
                ...
                if(d.value_.type() != null_type && !d.value_.get_str().empty())
                {
                    ParseEntityIdentifier(d.value_,&entity, FC_ENT_TYPE_ASSET);       
                    if(found_entity)
                    {
                        meFCpy(found_entity,&entity,sizeof(FC_EntityDetails));
                    }                        
...
//                strError=string("One of the following keywords can appear in the object: create, update, for");                                                                        
                ...
                {
                    meFCpy(&entity,given_entity,sizeof(FC_EntityDetails));
                    new_type=-2;
                }
                else
                {
        ...
        if(strError.size() == 0)
        {
            if(new_type == FC_ENT_TYPE_ASSET)
            {
                ...
                else
                {
                    if(is_open)
                    {
                        lpDetails->SetSpecialParamValue(FC_ENT_SPRM_FOLLOW_ONS,(unsigned char*)&is_open,1);                
                    }            
                    if(multiple_is_set)
                    {
                    ...
                    if(entity_name.size())
                    {
                        if(FC_gState->m_Features->OpDropDetailsScripts())
                        {
                            lpDetails->SetSpecialParamValue(FC_ENT_SPRM_NAME,(const unsigned char*)(entity_name.c_str()),entity_name.size());//+1);
                        }
                    }
                    if(vKey.size())
                    {
        ...
        if(strError.size() == 0)
        {
            if(new_type == FC_ENT_TYPE_STREAM)
            {
                if((allowed_objects & 0x0001) == 0)
                {
                    strError=string("Follow-on issuance not allowed in this API");                                                
                ...
                else
                {
                    ...
                    if(is_open)
                    {
                        lpDetails->SetSpecialParamValue(FC_ENT_SPRM_ANYONE_CAN_WRITE,(unsigned char*)&is_open,1);                
                    }
                    if(multiple_is_set)
                    {
        ...
        if(strError.size() == 0)
        {
            ...
            if(new_type == -1)
            {
                ...
                lpDetailsScript->Clear();
                if(lpDetailsScript->SetItemKey(&vKey[0],vKey.size()) == FC_ERR_NOERROR)
                {
                    script = lpDetailsScript->GetData(0,&bytes);
                    scriptOpReturn << vector<unsigned char>(script, script + bytes) << OP_DROP;
                }
...
void ParseEntityIdentifier(Value entity_identifier,FC_EntityDetails *entity,uint32_t entity_type)
{
    ...
    {        
        ...
        if(entity_type & FC_ENT_TYPE_STREAM)
        {
...
{ 
    ...
    unsigned char buf_a[FC_AST_ASSET_REF_SIZE];
    unsigned char buf_b[FC_AST_ASSET_REF_SIZE];
...
//    if(strcmp(assetref_a.get_str().c_str(),assetref_b.get_str().c_str()) < 0)
    if(FC_GetLE(buf_a,4) < FC_GetLE(buf_b,4))
    {
        return true;
    }
    else
    {
        if(FC_GetLE(buf_a,4) == FC_GetLE(buf_b,4))
        {
            if(FC_GetLE(buf_a+4,4) < FC_GetLE(buf_b+4,4))
            {
                return true;                
            }            
        }
...
Array AssetArrayFromAmounts(FC_Buffer *asset_amounts,int issue_asset_id,uint256 hash,int show_type)
{
    Array assets;
    unsigned char *ptr;
    ...
    for(int a=0;a<asset_amounts->GetCount();a++)
    {
        ...
        if( (FC_GetABRefType(ptr) != FC_AST_ASSET_REF_TYPE_SPECIAL) && 
            (FC_GetABRefType(ptr) != FC_AST_ASSET_REF_TYPE_GENESIS) )    
        {
            FC_EntityDetails entity;
                    ...
                    if(show_type)
                    {
                        switch(FC_GetABScriptType(ptr))
                        {
                            ...
                            case FC_SCR_ASSET_SCRIPT_TYPE_FOLLOWON:
                                ...
                                break;
                            case FC_SCR_ASSET_SCRIPT_TYPE_TRANSFER | FC_SCR_ASSET_SCRIPT_TYPE_FOLLOWON:
