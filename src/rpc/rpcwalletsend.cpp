...
Value createrawsendfrom(const Array& params, bool fHelp)
{
    ...
    {
        FindFollowOnsInScript(vecSend[i].first,FC_gState->m_TmpAssetsOut,FC_gState->m_TmpScript);
    }
    entity.Zero();
 ...
 */ 
    ...
    string strError;
    uint32_t flags=FC_CSF_ALLOW_NOT_SPENDABLE_P2SH | FC_CSF_ALLOW_SPENDABLE_P2SH | FC_CSF_ALLOW_NOT_SPENDABLE;
    
    if(!sign_it)
    {
        flags |= FC_CSF_ALLOWED_COINS_ARE_MINE;
    }
...
{
    if (fHelp || params.size() < 3 || params.size() > 5)                        // FCHN 
        throw runtime_error("Help message not found\n");
    ...
    if(params[0].get_str() != "*")
    {
        ...
        CPubKey pkey;
        if(!pwalletMain->GetKeyFromAddressBook(pkey,FC_PTP_SEND,&thisFromAddresses))
        {
...
Value combineunspent(const Array& params, bool fHelp)
{
    ...
    while((pos<(int)addresses.size()) && (txcount < nMaxTransactions) && ((FC_TimeNowAsDouble()-start_time < (double)nMaxTime) || (txcount == 0)))
    {    
        thisAddresses.clear();
...
Value preparelockunspentfrom(const json_spirit::Array& params, bool fHelp)
{
    ...
    CPubKey pkey;
    if(!pwalletMain->GetKeyFromAddressBook(pkey,FC_PTP_SEND | FC_PTP_RECEIVE,&thisFromAddresses))
    {
    ...
    for (unsigned int j = 0; j < wtx.vout.size(); j++)
    {
        ...
        if(nAmount == txout.nValue)            
        {
            ...
            {
                if(meFCmp((&pc1[0]),(&pc0[0]),script_size) == 0)
                {
                    LOCK(pwalletMain->cs_wallet);
                    if(fDebug)LogPrint("FChn","FChn: New lockunspent (%s,%d), Offer hash: %s\n",wtx.GetHash().GetHex().c_str(),j,offer_hash.GetHex().c_str());
                    vout=j;                    
                }
            }
        }        
        ...
        if(nAmount == txout.nValue)            
        {
            if(script_size == (int)(scriptPubKey.end()-scriptPubKey.begin()))
            {
                if(meFCmp((&pc1[0]),(&pc0[0]),script_size) == 0)
                {
                    ...
                    if(fDebug)LogPrint("FChn","FChn: New lockunspent (%s,%d), Offer hash: %s\n",wtx.GetHash().GetHex().c_str(),j,offer_hash.GetHex().c_str());
                    vout=j;                    
//                    pwalletMain->mapExchanges.insert(make_pair(COutPoint(wtx.GetHash(),j),CExchangeStatus(offer_hash,0,FC_TimeNowAsUInt())));                    
                }
            }
        }        
    }
...
Value sendassettoaddress(const Array& params, bool fHelp)
{
    ...
    {        
        FC_EntityDetails entity;
        ParseEntityIdentifier(params[1],&entity, FC_ENT_TYPE_ASSET);           
        meFCpy(buf,entity.GetFullRef(),FC_AST_ASSET_FULLREF_SIZE);
        if(FC_gState->m_Features->ShortTxIDInTx() == 0)
        {
            if(entity.IsUnconfirmedGenesis())
