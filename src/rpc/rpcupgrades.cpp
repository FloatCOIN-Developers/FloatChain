...
Value createupgradefroFCmd(const Array& params, bool fHelp)
{
    ...
    {
        ret=ParseAssetKey(upgrade_name.c_str(),NULL,NULL,NULL,NULL,&type,FC_ENT_TYPE_ANY);
    ...
    if(upgrade_name.size())
    {        
        lpDetails->SetSpecialParamValue(FC_ENT_SPRM_NAME,(unsigned char*)(upgrade_name.c_str()),upgrade_name.size());//+1);
    }
    ...
    if(params[4].type() == obj_type)
    {
        ...
        {  
            if(s.name_ == "protocol-version")
            {
                ...
                if(protocol_version > FC_gState->GetProtocolVersion())
                {
                    throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid protocol version, cannot upgrade to future version");                                                                                    
                }
                lpDetails->SetSpecialParamValue(FC_ENT_SPRM_UPGRADE_PROTOCOL_VERSION,(unsigned char*)&protocol_version,4);        
            }
            else
            {
                if(s.name_ == "start-block")
                {
                    ...
                    lpDetails->SetSpecialParamValue(FC_ENT_SPRM_UPGRADE_START_BLOCK,(unsigned char*)&start_block,4);        
                }                    
                else
                {
    ...
    err=lpDetailsScript->SetNewEntityType(FC_ENT_TYPE_UPGRADE,0,script,bytes);
    if(err)
    {
    ...
    if(params[0].get_str() != "*")
    {
        ...
        CPubKey pkey;
        if(!pwalletMain->GetKeyFromAddressBook(pkey,FC_PTP_CREATE | FC_PTP_ADMIN,&thisFromAddresses))
        {
    ...
    else
    {
        BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
        {
            ...
            if(address.GetKeyID(keyID))
            {
                ...
                {
                    if(FC_gState->m_Permissions->CanCreate(NULL,(unsigned char*)(&keyID)))
                    {
...
Value approvefrom(const json_spirit::Array& params, bool fHelp)
{
    ...
    entity.Zero();
    ParseEntityIdentifier(entity_identifier,&entity, FC_ENT_TYPE_UPGRADE);           
    ...
    LogPrintf("FChn: %s upgrade %s (%s) from address %s\n",(approval != 0) ? "Approving" : "Disapproving",
...
*/
    ...
    {
        {
            ...
            for(int is=0;is<(int)inputStrings.size();is++)
            {
                ...
                FC_EntityDetails upgrade_entity;
                ParseEntityIdentifier(param,&upgrade_entity, FC_ENT_TYPE_UPGRADE);           
                
                upgrades=FC_gState->m_Permissions->GetUpgradeList(upgrade_entity.GetTxID() + FC_AST_SHORT_TXID_OFFSET,upgrades);
            }
        }
    }
    else
    {        
        {
            LOCK(cs_main);
            upgrades=FC_gState->m_Permissions->GetUpgradeList(NULL,upgrades);
        }
    }
    ...
    for(int i=0;i<upgrades->GetCount();i++)
    {
        ...
        plsRow=(FC_PermissionDetails *)(upgrades->GetRow(i));
        if(plsRow->m_Type == FC_PTP_UPGRADE)
        {
            meFCpy(&hash,plsRow->m_Address,sizeof(uint160));
...
//    for(int i=0;i<upgrades->GetCount();i++)
    {
        ...
        upgrade_entity.Zero();
        FC_gState->m_Assets->FindEntityByShortTxID(&upgrade_entity,plsRow->m_Address);
...
//                        pend_obj.push_back(Pair("endblock", (int64_t)block_to));                        
    ...
    FC_gState->m_Permissions->FreePermissionList(upgrades);
