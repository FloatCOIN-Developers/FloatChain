...
int GetInputOffer(const CWalletTx& wtx, const set<CTxDestination>* addresses,const isminefilter& filter,CAmount& nAmount,FC_Buffer *amounts,FC_Script *lpScript,set<CTxDestination>* from_addresses,set<CTxDestination>* my_addresses)
{
    int nIsMineCount=0;
    amounts->Clear();
    nAmount=0;
    ...
    BOOST_FOREACH(const CTxIn& txin, wtx.vin)
    {
        ...
        if (prevout_found)
        {
            if(IsTxOutMineAndInList(pwalletMain,prevout,addresses,filter))
            {
                ...
                if(ParseFloatchainTxOutToBuffer(txin.prevout.hash,prevout,amounts,lpScript,NULL,NULL,strFailReason))
                {
...
Object ListWalletTransactions(const CWalletTx& wtx, bool fLong, const isminefilter& filter,const set<CTxDestination>* addresses,FC_Buffer *amounts,FC_Script *lpScript)
{
    Object entry;
    ...
    {
        uint64_t quantity=FC_GetABQuantity(amounts->GetRow(i));
        quantity=-quantity;
        FC_SetABQuantity(amounts->GetRow(i),quantity);        
    }
    ...
    for (int i = 0; i < (int)wtx.vout.size(); ++i)
    {
        ...
        if(!txout.scriptPubKey.IsUnspendable())
        {
            ...
            if(IsTxOutMineAndInList(pwalletMain,txout,addresses,filter))
            {
                fIsMine=true;
                nIsToMeCount++;
                ParseFloatchainTxOutToBuffer(wtx.GetHash(),txout,amounts,lpScript,NULL,&required,strFailReason);
...
//            ParseFloatchainTxOutToBuffer(wtx.GetHash(),txout,amounts,lpScript,NULL,&required,strFailReason);
            CreateAssetBalanceList(txout,amounts,lpScript,&required);
            if(required & (FC_PTP_ADMIN | FC_PTP_ACTIVATE))
            {
...
Value listaddresstransactions(const Array& params, bool fHelp)
{
    ...
    if(FC_gState->m_WalletMode & FC_WMD_ADDRESS_TXS)
    {
        ...
        if(lpKeyID)
        {
            meFCpy(address_by_time.m_EntityID,lpKeyID,FC_TDB_ENTITY_ID_SIZE);
            address_by_time.m_EntityType=FC_TET_PUBKEY_ADDRESS | FC_TET_TIMERECEIVED;
        }
        if(lpScriptID)
        {
            meFCpy(address_by_time.m_EntityID,lpScriptID,FC_TDB_ENTITY_ID_SIZE);
            address_by_time.m_EntityType=FC_TET_SCRIPT_ADDRESS | FC_TET_TIMERECEIVED;
        }
...
{
    if (fHelp || params.size() < 1 || params.size() > 3)                        // FCHN
        throw runtime_error("Help message not found\n");
