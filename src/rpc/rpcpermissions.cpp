...
Value grantoperation(const Array& params)
{
    ...
    if(((type & FC_PTP_RECEIVE) == 0) || (from >= to))
    {        
        if(nAmount > 0)
        {
    ...
    {        
        ParseEntityIdentifier(entity_identifier,&entity, FC_ENT_TYPE_ANY);           
        LogPrintf("FChn: Granting %s permission(s) to address %s (%ld-%ld), Entity TxID: %s, Name: %s\n",permission_type,params[1].get_str(),from,to,
                ((uint256*)entity.GetTxID())->ToString().c_str(),entity.GetName());
        
        type=FC_gState->m_Permissions->GetPermissionType(permission_type.c_str(),entity.GetEntityType());
    ...
    {
        type=FC_gState->m_Permissions->GetPermissionType(permission_type.c_str(),FC_ENT_TYPE_NONE);
        
        if(type == 0)
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid permission");
        
        LogPrintf("FChn: Granting %s permission(s) to address %s (%ld-%ld)\n",permission_type,params[1].get_str(),from,to);
    }
    ...
    if(fromaddresses.size() == 1)
    {
        ...
        if(lpKeyID != NULL)
        {
            if(entity.GetEntityType())
            {
                ...
                {
                    if(FC_gState->m_Permissions->CanActivate(entity.GetTxID(),(unsigned char*)(lpKeyID)) == 0)
                    {
                ...
                {
                    if(FC_gState->m_Permissions->CanAdmin(entity.GetTxID(),(unsigned char*)(lpKeyID)) == 0)
                    {
            ...
            else
            {
                if(FC_gState->m_Permissions->IsActivateEnough(type))
                {
                    if(FC_gState->m_Permissions->CanActivate(NULL,(unsigned char*)(lpKeyID)) == 0)
                    {
                ...
                {
                    if(FC_gState->m_Permissions->CanAdmin(NULL,(unsigned char*)(lpKeyID)) == 0)
                    {
        ...
        if(found_entity.GetEntityType() == FC_ENT_TYPE_STREAM)
        {
    ...
    else
    {
        ...
        BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, CAddressBookData)& item, pwalletMain->mapAddressBook)
        {
            ...
            if(address.GetKeyID(keyID))
            {
                ...
                if(found_entity.GetEntityType() == FC_ENT_TYPE_STREAM)
                {
                    if((found_entity.AnyoneCanWrite() == 0) && (FC_gState->m_Permissions->CanWrite(found_entity.GetTxID(),(unsigned char*)(&keyID)) == 0))
                    {
                ...
                if(valid_publisher)
                {
                    if(entity.GetEntityType())
                    {
                        if(FC_gState->m_Permissions->IsActivateEnough(type))
                        {
                            if(FC_gState->m_Permissions->CanActivate(entity.GetTxID(),(unsigned char*)(&keyID)))
                            {
                                admin_found=true;
                            }                                                 
                        }
                        ...
                        {
                            if(FC_gState->m_Permissions->CanAdmin(entity.GetTxID(),(unsigned char*)(&keyID)))
                            {
                                admin_found=true;
                            }                                                                     
                        }
                    ...
                    else
                    {
                        if(FC_gState->m_Permissions->IsActivateEnough(type))
                        {
                            if(FC_gState->m_Permissions->CanActivate(NULL,(unsigned char*)(&keyID)))
                            {
                                admin_found=true;
                            }                                                 
                        }
                        ...
                        {
                            if(FC_gState->m_Permissions->CanAdmin(NULL,(unsigned char*)(&keyID)))
                            {
                                admin_found=true;
                            }                                                                     
                        }                
...
Value grantfroFCmd(const Array& params, bool fHelp)
{
...
Value revokefroFCmd(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error("Help message not found\n");
...
//        int period_pos=permission_type.find_last_of(".",permission_type.size());
            ...
            ((params[1].type() != str_type) || ((params[1].get_str() !="*"))))// && (params[1].get_str() !="") )) )
    {
        ...
        BOOST_FOREACH(CTxDestination& dest, addresses) 
        {
            ...
            
            {
                ...
                if(lpKeyID != NULL)
                {
                    permissions=FC_gState->m_Permissions->GetPermissionList(lpEntity,(unsigned char*)(lpKeyID),type,permissions);
                }
                else
                {
                    permissions=FC_gState->m_Permissions->GetPermissionList(lpEntity,(unsigned char*)(lpScriptID),type,permissions);                
                }
            }
        }
    }
    ...
    {        
        {
            LOCK(cs_main);
            permissions=FC_gState->m_Permissions->GetPermissionList(lpEntity,NULL,type,NULL);
        }
    }
    ...
    for(int i=0;i<permissions->GetCount();i++)
    {
        ...
        FC_PermissionDetails *plsDet;
        FC_PermissionDetails *plsPend;
        bool take_it;
        ...
        if(flags & FC_PFL_IS_SCRIPTHASH)
        {
            uint160 addr;
            meFCpy(&addr,plsRow->m_Address,sizeof(uint160));
        ...
        {
            case FC_PTP_CONNECT:entry.push_back(Pair("type", "connect"));break;
            case FC_PTP_SEND   :entry.push_back(Pair("type", "send"));break;
            case FC_PTP_RECEIVE:entry.push_back(Pair("type", "receive"));break;
            case FC_PTP_WRITE  :entry.push_back(Pair("type", "write"));break;
            case FC_PTP_CREATE :entry.push_back(Pair("type", "create"));break;
            case FC_PTP_ISSUE  :entry.push_back(Pair("type", "issue"));break;
            case FC_PTP_MINE   :entry.push_back(Pair("type", "mine"));break;
            case FC_PTP_ADMIN  :entry.push_back(Pair("type", "admin"));break;
            case FC_PTP_ACTIVATE  :entry.push_back(Pair("type", "activate"));break;                
        ...
        if( (plsRow->m_BlockFrom >= plsRow->m_BlockTo) && 
                (((flags & FC_PFL_HAVE_PENDING) == 0) || !verbose) )
        {
            take_it=false;
        }
        if(take_it)
        {
            ...
            if((flags & FC_PFL_HAVE_PENDING) || (consensus>1))
            {
                details=FC_gState->m_Permissions->GetPermissionDetails(plsRow);                            
            }
            else
            {
                details=NULL;
            ...
            if(details)
            {
                ...
                {
                    plsDet=(FC_PermissionDetails *)(details->GetRow(j));
                    remaining=plsDet->m_RequiredAdmins;
                    if(remaining > 0)
                    {
                        uint160 addr;
                        meFCpy(&addr,plsDet->m_LastAdmin,sizeof(uint160));
                        CKeyID lpKeyID=CKeyID(addr);
                ...
                for(int j=0;j<details->GetCount();j++)
                {
                    ...
                    if(remaining == 0)
                    {
                        ...
                        {
                            plsPend=(FC_PermissionDetails *)(details->GetRow(k));
                            ...
                            if(remaining == 0)
                            {
                                if(block_from == plsPend->m_BlockFrom)
                                {
                                    ...
                                    {
                                        uint160 addr;
                                        meFCpy(&addr,plsPend->m_LastAdmin,sizeof(uint160));
                                        CKeyID lpKeyID=CKeyID(addr);
//                                        CKeyID lpKeyID=CKeyID(*(uint160*)((void*)(plsPend->m_LastAdmin)));
                ...
                FC_gState->m_Permissions->FreePermissionList(details);                    
            }
            else
            {
                uint160 addr;
                meFCpy(&addr,plsRow->m_LastAdmin,sizeof(uint160));
                CKeyID lpKeyID=CKeyID(addr);
                admins.push_back(CBitcoinAddress(lpKeyID).ToString());                    
            }
            if(verbose)
