...
Object ExchangeAssetEntry(uint256 hash,const CTxOut txout,FC_Script *lpScript,FC_Buffer *asset_amounts,FC_Buffer *total_amounts,bool& can_disable,bool allow_sighashall,string& strError)
{
    Object result;
    ...
    if(is_input)
    {
        if(lpScript->GetNumElements() == 2)
        {
            ...
            if(elem_size>1)
            {
                ...
                if(FC_gState->m_Permissions->CanSend(NULL,pubkey_hash) == 0)
                {
                    if(strError.size() == 0)
                    {
    ...
    else
    {
        for(int a=0;a<asset_amounts->GetCount();a++)
        {
            ...
            quantity=(int64_t)FC_GetABQuantity(ptr);
            ...
            if(total_amounts)
            {
                ...
                if(row >= 0)
                {
                    total+=(int64_t)FC_GetABQuantity(total_amounts->GetRow(row));
                    FC_SetABQuantity(total_amounts->GetRow(row),total);
                }
                else
                {
                    meFCpy(buf,ptr,FC_AST_ASSET_QUANTITY_OFFSET);
...
Object DecodeExchangeTransaction(const CTransaction tx,int verbose,int64_t& native_balance,FC_Buffer *lpAssets,bool& is_complete,bool allow_last_sighashall,string& strError)
{
    Object result;
    strError="";
    native_balance=0;
    ...
    for(int i=0;i<(int)input_txouts.size();i++)
    {
        ...
        lpScript->Clear();
        lpScript->SetScript((unsigned char*)(&pc2[0]),(size_t)(script2.end()-pc2),FC_SCR_TYPE_SCRIPTSIG);
...
//                        ParseAssetKey(hash.GetHex().c_str(),NULL,buf,NULL,&multiple,NULL, FC_ENT_TYPE_ASSET);
                        ParseAssetKeyToFullAssetRef(hash.GetHex().c_str(),buf,&multiple,NULL, FC_ENT_TYPE_ASSET);
...
*/
        ...
        {
            nBytes+=4+36+1+73+1+65+4+8+1+24+4+ask.size()*(FC_gState->m_NetworkParams->m_AssetRefSize+FC_AST_ASSET_QUANTITY_SIZE)+1+32;
            required_fee=pwalletMain->GetMinimumFee(nBytes,1,mempool);
            dust=-1;
            if(FC_gState->m_NetworkParams->IsProtocolFloatchain())
            {
                dust=FCP_MINIMUM_PER_OUTPUT;
            }            
            if(dust<0)
            {
...
//        result.push_back(Pair("tocomplete", tocomplete));
        ...
        {    
            CPubKey pkey;
            if(!pwalletMain->GetKeyFromAddressBook(pkey,FC_PTP_SEND | FC_PTP_RECEIVE))
            {
                result.push_back(Pair("cancomplete", false));            
            }
            else
...
string FindExchangeOutPoint(const json_spirit::Array& params,int first_param,COutPoint& offer_input,CAmount& nAmount,FC_Script *lpScript,int *eErrorCode)
{
    string strError="";
...
Value completerawexchange(const json_spirit::Array& params, bool fHelp)
{
    ...
    {    
        ...
        {        
            ...
            if(aptr)
            {
                if((found_entity.AnyoneCanWrite() == 0) && (FC_gState->m_Permissions->CanWrite(found_entity.GetTxID(),aptr) == 0))
                {
                    throw JSONRPCError(RPC_INSUFFICIENT_PERMISSIONS, "Publishing in this stream is not allowed from this address");                                                                        
                }                                                 
                if(FC_gState->m_Permissions->CanSend(NULL,aptr) == 0)
                {
