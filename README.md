...
[FloatChain](http://www.floatcoin.com/) is an open source platform for private blockchains, which offers a rich set of features including extensive configurability, rapid deployment, permissions management, native assets and data streams. Although it is designed to enable private blockchains, FloatChain provides maximal compatibility with the bitcoin ecosystem, including the peer-to-peer protocol, transaction/block formats and [Bitcoin Core](https://bitcoin.org/en/bitcoin-core/) APIs/runtime parameters.
FloatChain is licensed under the GPLv3 open source license. We also offer commercial licenses for those wishing to integrate FloatChain into a proprietary software solution.
...
C++ compilers are memory-hungry, so it is recommended to have at least 1 GB of memory available when compiling FloatChain. With less memory, compilation may take much longer due to swapfile thrashing.
...
Compile FloatChain for Ubuntu (64-bit)
-----------------------------
...
* This will build `floatchaind`, `floatchain-cli` and `floatchain-util` in the `src` directory.

* The release is built with GCC after which `strip floatchaind` strings the debug symbols, which reduces the executable size by about 90%.
...
Compile FloatChain for Windows (64-bit)
------------------------------
...
* This will build `floatchaind.exe`, `floatchain-cli.exe` and `floatchain-util.exe` in the `src` directory.
...
To create a statically linked FloatChain which only depends on default MacOS dylibs, the following steps are taken:
...
Compile FloatChain for Mac (64-bit)
--------------------------
